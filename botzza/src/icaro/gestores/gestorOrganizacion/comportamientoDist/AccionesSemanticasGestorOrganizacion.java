...
public class AccionesSemanticasGestorOrganizacion extends
		...
		try {
			...
			itfUsoPropiadeEsteAgente = (ItfUsoAgenteReactivo) itfUsoRepositorio
					.obtenerInterfazUso(NombresPredefinidos.ITF_USO
							+ nombreAgente);
			tiempoParaNuevaMonitorizacion = Integer.parseInt(config
					.getValorPropiedadGlobal("intervaloMonitorizacion"));
			// tiempoParaNuevaMonitorizacion =
			// Integer.parseInt(config.getValorPropiedadGlobal("gestores.comun.intervaloMonitorizacion"));
			maxNumIntentosCreacionCompGestionados = Integer.parseInt(config
			...
			// Si la organizacion tiene varios nodos de despliegue debo crear el
			// Registro RMI
	...
	public void crearGestores() {
		try {
			...
			config.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION);
			...
			if (nodoDestino.equals(identEsteNodo)) {
				...
				// // ok = true;
				// // } catch (Exception e) {
				// // trazas.aceptaNuevaTraza(new
				// InfoTraza("GestorOrganizacion",
	...
	public void crearGestoreEnEsteNodo(String identAgente)
			throws RemoteException {

		try {
			DescInstanciaGestor gestorAgentes = config
					.getDescInstanciaGestor(identAgente);
			String nodoDestino = gestorAgentes.getNodo().getNombreUso();
			// int intentos = 0;
			if (nodoDestino.equals(identEsteNodo)) {
				FactoriaAgenteReactivo.instancia().crearAgenteReactivo(
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						...
						"Error al crear agente "
								...
								+ " en el nodo local "
								+ identEsteNodo
								+ "\t El nodo del gestor a crear es diferente El Nodo definido es : "
								+ nodoDestino, InfoTraza.NivelTraza.error));
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Error al crear agente " + identAgente
							+ " en el nodo local " + identEsteNodo + "\t  ",
					InfoTraza.NivelTraza.error));
			logger.error("Error al crear agente " + identAgente
					+ " en el nodo local " + identEsteNodo + "\t ");
			// return false;
		}
	}
				...
				try {
					...
					trazas.aceptaNuevaTraza(new InfoTraza(
							...
							"Error al crear el agente "
									...
									+ " en un nodo remoto. Se volver� a intentar en "
									+ intentos + " segundos...\n nodo origen: "
									+ identEsteNodo + "\t nodo destino: "
									+ nodoDestino, InfoTraza.NivelTraza.error));
					logger.error("Error al crear el agente "
							+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES
							+ " en un nodo remoto. Se volver� a intentar en "
							+ intentos + " segundos...\n nodo origen: "
							+ identEsteNodo + "\t nodo destino: " + nodoDestino);
					Thread.sleep(1000 * intentos);
					ok = false;
				}
			}
			if (!ok) {
				trazas.aceptaNuevaTraza(new InfoTraza(
						...
						"No se pudo crear el agente "
								+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES
								+ " en un nodo remoto. Se supero el numero de intentos definido "
								+ intentos + identEsteNodo
								+ "\t nodo destino: " + nodoDestino,
						InfoTraza.NivelTraza.error));
				logger.error("Error al crear el agente "
						+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES
						+ " en un nodo remoto. Se supero el numero de intentos definido "
						+ intentos + identEsteNodo + "\t nodo destino: "
						+ nodoDestino);
			}
			logger.debug("GestorOrganizacion: Gestor de agentes creado.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
		...
		try {
			...
			itfGestorAgentes = (InterfazUsoAgente) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
			itfGestorAgentes.aceptaEvento(new EventoRecAgte("ordenTerminacion",
					nombreAgente, itfGestorAgentes.getIdentAgente()));
	...
	public void terminarGestorRecursos() {
		...
		try {
			// gestorRecursos = (ItfGestionAgenteReactivo)
			...
			gestorRecursos = (InterfazUsoAgente) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ NombresPredefinidos.NOMBRE_GESTOR_RECURSOS);
			gestorRecursos.aceptaEvento(new EventoRecAgte("ordenTerminacion",
					nombreAgente, gestorRecursos.getIdentAgente()));
