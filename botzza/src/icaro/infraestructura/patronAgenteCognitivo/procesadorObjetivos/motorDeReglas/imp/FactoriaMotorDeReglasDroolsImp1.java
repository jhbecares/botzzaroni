...
public class FactoriaMotorDeReglasDroolsImp1 extends FactoriaMotorDeReglas {
	...
	@Override
	public synchronized ItfMotorDeReglas crearMotorDeReglas(
			AgenteCognitivo agent, InputStream reglas, String ficheroReglas) {
		// Solo se crea el motor si la compilacion de las reglas es correcta
		// MotorDeReglasDroolsImp5 motorDrools = new
		// MotorDeReglasDroolsImp5(agent);
		...
		if (kbuilder == null) {
			trazas.aceptaNuevaTraza(new InfoTraza(
					...
					"Motor de reglas Drools: ERROR en la compilacion de las reglas al crear el agente ",
					InfoTraza.NivelTraza.error));
			return null;
		}
		MotorDeReglasDroolsImp5 motorDrools = new MotorDeReglasDroolsImp5(
				agent, this);
	...
	public synchronized KnowledgeBuilder compilarReglas(String agentId,
			InputStream fichero, String ficheroReglas) {
		...
		if (indiceFicheroEnArray >= 0) {
			...
			try {
				...
				if (kbuilder.hasErrors()) {
					...
					// int ultimoIndiceInsercion =
					// ficheroReglasCompilados.size();
				...
				trazas.aceptaNuevaTraza(new InfoTraza(agentId,
						"Motor de Reglas Drools: ERROR al compilar las reglas. "
								+ ficheroReglas, InfoTraza.NivelTraza.error));
				e.printStackTrace();
			}
			return null;
		}
