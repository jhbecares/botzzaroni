 ...
 */
public class ProcesadorInfoReactivoAutInObjImp0 extends
		ProcesadorInfoReactivoAbstracto implements Serializable {
	...
	public ProcesadorInfoReactivoAutInObjImp0(ItfUsoAutomataEFE interpAutom,
			GestorAccionesAgteReactivoImp gestAcciones,
	...
	public synchronized void procesarInfoControlAgteReactivo(
		...
		NombresPredefinidos.RECURSO_TRAZAS_OBJ.trazar(identAgte,
				"Se procesa el  input " + infoParaProcesar,
		...
		if (this.estado == InterfazGestion.ESTADO_ACTIVO) {
			if (infoParaProcesar instanceof InfoContEvtMsgAgteReactivo) {
				...
				resultadoProcesoInput = automataControl.procesaInput(
						infoParaAutomata.getInput(),
				...
				resultadoProcesoInput = automataControl
						.procesaInput(infoParaProcesar);
			}
		}
		if (!resultadoProcesoInput) {
			NombresPredefinidos.RECURSO_TRAZAS_OBJ.trazar(identAgte,
					"No hay transicion asociada al input " + infoParaProcesar,
					InfoTraza.NivelTraza.debug);
	...
	public synchronized void procesarInput(Object input, Object... paramsAccion) {
		NombresPredefinidos.RECURSO_TRAZAS_OBJ.trazar(identAgte,
				"Se procesa el  input " + input, InfoTraza.NivelTraza.debug);
		...
		if (input == null) {
			NombresPredefinidos.RECURSO_TRAZAS_OBJ
					.aceptaNuevaTraza(new InfoTraza(this.getClass()
							.getSimpleName(),
		...
		if (paramsAccion == null) {
			...
			// valoresParametrosAccion);
			// numParametros = 1;
		} else {
			valoresParametrosAccion = new Object[(paramsAccion).length
					+ posparametros];
			if (posparametros == 1) {
	...
	public void inicializarInfoGestorAcciones(String identAgte,
		...
		if (gestorAcciones != null) {
			...
			gestorAcciones.inicializarInfoAccionesAgteReactivo(identAgte,
					percepcionProductor, this);
		}
	}
