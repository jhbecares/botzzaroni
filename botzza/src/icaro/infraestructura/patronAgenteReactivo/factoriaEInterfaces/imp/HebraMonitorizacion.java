...
public class HebraMonitorizacion extends Thread {
	 ...
	 *
	 * @param milis
	 *            Milisegundos a esperar entre cada monitorizacin
	 * @param eventoAProducir
	 *            Elemento que se produce en la interfaz cada cierto tiempo
	 */
	public HebraMonitorizacion(long milis, ItfUsoAgenteReactivo agente,
			String eventoAProducir) {
	...
	public void run() {
		...
		while (!this.finalizar) {
			...
			if (!this.finalizar) {
				try {
					this.agente.aceptaEvento(new EventoRecAgte(this.evento,
							null, null));
				} catch (Exception e) {
