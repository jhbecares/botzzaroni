...
public class FactoriaPercepcionImp extends FactoriaPercepcion {
	...
	public PercepcionAbstracto crearPercepcion() {
		...
		// En esta version la factoria crea el procesador de items y la clase
		// que implmenta las interfaces de la percepcion
		return new PercepcionAgenteReactivoImp();
		// elijo la unica implementacin posible (aunque podra haber mas)

	}

	@Override
	public PercepcionAbstracto crearPercepcion(AgenteReactivoAbstracto agente,
			ItfControlAgteReactivo itfControl) {

		return new PercepcionAgenteReactivoImp(agente,
				new ProcesadorItemsPercepReactivo(agente, itfControl));
	}
}