...
public class Focus {
	 ...
	 *
	 * @param obj
	 *            Objetivo al cual apuntar el foco
	 */
	public void setFoco(Objetivo obj) {
		// trazas.aceptaNuevaTraza(new
		// InfoTraza("Focalizaciones","Foco: Focalizando el objetivo "+obj.getID(),InfoTraza.NivelTraza.debug));

		// Añadido para depurar los objetivos: informo del identificador y la
		// clase de objetivo
		if (obj == null) {
			this.foco = null;
		} else {
			...
			// trazas.aceptaNuevaTraza(new
			// InfoTraza("Focalizaciones","Foco: Focalizando el objetivo "+obj.getID()
			// + " , class -> " + claseobjetivo,InfoTraza.NivelTraza.debug));
	...
	public String toString() {
		return "(FOCO: focoActual= "
				+ this.foco
				+ "  focoAnterior= "
				+ this.focosAnteriores[((this.indice + TAM_COLA_FOCOS) - 1)
						% TAM_COLA_FOCOS] + " )";
	}
	...
	public String toStringCola() {
		...
		res += "Objetivo focalizado en posicion " + this.indice + ": "
				+ this.focosAnteriores[this.indice];
