...
public class AccionesSemanticasAgenteAplicacionAcceso extends
	...
	public void arranque() {
		...
		try {
			// visualizacion = (ItfUsoVisualizadorAcceso)
			// itfUsoRepositorio.obtenerInterfaz
			// (NombresPredefinidos.ITF_USO+"VisualizacionAcceso1");

			visualizacion = (ItfUsoVisualizadorAcceso) obtenerIntfRecurso(
					identRecursoVisualizacionAcceso,
					NombresPredefinidos.ITF_USO);
			if (visualizacion != null) {
				visualizacion.mostrarVisualizadorAcceso(this.nombreAgente,
						NombresPredefinidos.TIPO_REACTIVO);
				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
						"Se acaba de mostrar el visualizador",
						InfoTraza.NivelTraza.debug));
			} else {
				trazas.aceptaNuevaTraza(new InfoTraza(
						this.nombreAgente,
						"No se puede acceder al visualizador porque no se encuentra su interfaz de uso--"
								+ identRecursoVisualizacionAcceso,
	...
	public void valida(InfoAccesoSinValidar infoUsuario) {
		...
		// InfoAccesoSinValidar datos = new
		// InfoAccesoSinValidar(username,password);
		try {
			// Persistencia1 = (ItfUsoPersistenciaAccesoSimple)
			// itfUsoRepositorio.obtenerInterfaz
			// (NombresPredefinidos.ITF_USO+"Persistencia1");
			Persistencia1 = (ItfUsoPersistenciaAccesoSimple) obtenerIntfRecurso(
					identRecursoPersistencia, NombresPredefinidos.ITF_USO);
			if (Persistencia1 != null) {
				ok = Persistencia1.compruebaUsuario(infoUsuario.tomaUsuario(),
						infoUsuario.tomaPassword());
				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
						"Comprobando usuario...", InfoTraza.NivelTraza.debug));
			} else {
				trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
						"No se ha podido obtener la interfaz de uso del recurso"
		...
		try {
			// agenteAcceso = (ItfUsoAgenteReactivo)
			// itfUsoRepositorio.obtenerInterfaz
			// (NombresPredefinidos.ITF_USO+this.nombreAgente);
			Object[] datosEnvio = new Object[] { infoUsuario.tomaUsuario(),
					infoUsuario.tomaPassword() };
			if (ok) {
				...
				// itfUsoPropiadeEsteAgente.aceptaEvento(new
				// EventoRecAgte("usuarioNoValido",
				// datosEnvio,this.nombreAgente,this.nombreAgente));
			}
	...
	public void validaConRecRemoto(InfoAccesoSinValidar infoUsuario) {
		...
		// InfoAccesoSinValidar(username,password);
		try {
			ItfUsoConfiguracion config = (ItfUsoConfiguracion) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ NombresPredefinidos.CONFIGURACION);
			String nombreRec = "Persistencia1";
			String identHostRecurso = config
					.getDescInstanciaRecursoAplicacion(nombreRec).getNodo()
					.getNombreUso();
			...
			// registrado
			{
				...
				//
				ok = itfUsoRec.compruebaUsuario(infoUsuario.tomaUsuario(),
						infoUsuario.tomaPassword());
				try {
					ItfUsoRecursoTrazas trazas = (ItfUsoRecursoTrazas) ClaseGeneradoraRepositorioInterfaces
							.instance()
		...
		try {
			agenteAcceso = (ItfUsoAgenteReactivo) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ this.nombreAgente);
			Object[] datosEnvio = new Object[] { infoUsuario.tomaUsuario(),
					infoUsuario.tomaPassword() };
			if (ok) {
				agenteAcceso
						.aceptaEvento(new EventoRecAgte("usuarioValido",
								datosEnvio, this.nombreAgente,
								NombresPredefinidos.NOMBRE_AGENTE_APLICACION
										+ "Acceso"));
			} else {
				agenteAcceso.aceptaEvento(new EventoRecAgte("usuarioNoValido",
						datosEnvio, this.nombreAgente, this.nombreAgente));
			}
		} catch (Exception e) {
			try {
	...
	public void mostrarUsuarioNoAccede(String username, String password) {
		...
		try {
			...
			// itfUsoRepositorio.obtenerInterfaz
			// (NombresPredefinidos.ITF_USO+"VisualizacionAcceso1");
			visualizacion.mostrarMensajeError("Acceso Incorrecto",
					"El usuario " + dav.tomaUsuario()
							+ " no ha accedido al sistema.");
		}
	...
	public void terminacion() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
				"Terminando agente: "
						+ NombresPredefinidos.NOMBRE_AGENTE_APLICACION
						+ "Acceso1", InfoTraza.NivelTraza.debug));
		// try {
		...
		try {
			...
			// Buscar al Gestor de Agentes y si falla el GO y decirle que el
			// agente de identificador agxx ha terminado
	...
	public void clasificaError() {
		/*
		 * A partir de esta funci�n se debe decidir si el sistema se puede
		 * recuperar del error o no.En este caso la pol�tica es que todos los
	...
	public void pedirTerminacionGestorAgentes() {
		try {
			/*
			 * ItfUsoAgenteReactivo itfUsoGestorOrgan =
			 * (ItfUsoAgenteReactivo)itfUsoRepositorio.obtenerInterfaz
			 * ("Itf_Uso_Gestor_Organizacion");
			 * itfUsoGestorOrgan.aceptaEvento(new
			 * EventoRecAgte("terminar_gestores_y_gestor_organizacion"
			 * ,"AgenteAccesoUso","AgenteAccesoUso"));
			 */
			// this.itfUsoGestorAReportar.aceptaEvento(new
			// EventoRecAgte("peticion_terminar_todo",this.nombreAgente,NombresPredefinidos.NOMBRE_GESTOR_AGENTES));
			visualizacion.cerrarVisualizadorAcceso();
			//
			String identGestorAReportar = NombresPredefinidos.NOMBRE_GESTOR_AGENTES;
			itfUsoGestorAReportar = (ItfUsoAgenteReactivo) obtenerIntfAgente(
					identGestorAReportar, NombresPredefinidos.ITF_USO);
			if (itfUsoGestorAReportar == null) {
				identGestorAReportar = NombresPredefinidos.NOMBRE_GESTOR_NODO;
				itfUsoGestorAReportar = (ItfUsoAgenteReactivo) itfUsoRepositorio
						.obtenerInterfaz(NombresPredefinidos.ITF_USO
								+ identGestorAReportar);

			}
			this.itfUsoGestorAReportar.aceptaMensaje(new MensajeSimple(
					new InfoContEvtMsgAgteReactivo("peticion_terminar_todo"),
					this.nombreAgente, identGestorAReportar));
		} catch (Exception e) {
			logger.error("Error al mandar el evento de peticion_terminar_todo",
					e);
			trazas.aceptaNuevaTraza(new InfoTraza(this.nombreAgente,
			...
			try {
				...
				// itfUsoPropiadeEsteAgente.aceptaEvento(new
				// EventoRecAgte("error",this.nombreAgente,this.nombreAgente));
			} catch (Exception exc) {
				try {
					ItfUsoRecursoTrazas trazas = (ItfUsoRecursoTrazas) ClaseGeneradoraRepositorioInterfaces
							.instance()
	...
	public Object obtenerIntfAgente(String identAgente, String tipoInterfaz) {
		...
		try {
			resultadoBusqueda = itfUsoRepositorio.obtenerInterfaz(tipoInterfaz
					+ identAgente);
			...
			Logger.getLogger(
					AccionesSemanticasAgenteAplicacionAcceso.class.getName())
					.log(Level.SEVERE,
							"Se prujo un error al buscar   la interfaz del agente : "
									+ identAgente, ex);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Se produjo un error al buscar   la interfaz del agente : "
							+ identAgente, InfoTraza.NivelTraza.error));
		}
