...
public class AccionesSemanticasAgenteAplicacionAcceso extends
	...
	public void arranque() {
		...
		try {
			visualizacion = (ItfUsoVisualizadorAcceso) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ "VisualizacionAcceso1");
			persistencia1 = (ItfUsoPersistenciaAccesoSimple) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO
							+ "Persistencia1");
	 ...
	 *
	 * @param infoUsuario
	 *            contien la información introducida por el usuario y enviada
	 *            por el recurso de visualizacion
	 */
	public void valida(InfoAccesoSinValidar infoUsuario) {
		...
		try {
			if (ok) {
				this.informaraMiAutomata("usuarioValido",
						infoUsuario.tomaUsuario(), infoUsuario.tomaPassword());
			} else {
				this.informaraMiAutomata("usuarioNoValido",
						infoUsuario.tomaUsuario(), infoUsuario.tomaPassword());
			}
		} catch (Exception e) {
			try {
	...
	public void clasificaError() {
		/*
		 * A partir de esta funcion se debe decidir si el sistema se puede
		 * recuperar del error o no.En este caso la politica es que todos los
		 * errores son cri ticos.
		 */
		try {
	...
	public void pedirTerminacionGestorAgentes() {
		try {
			...
			// new MensajeSimple (new
			// InfoContEvtMsgAgteReactivo("peticion_terminar_todo"),
