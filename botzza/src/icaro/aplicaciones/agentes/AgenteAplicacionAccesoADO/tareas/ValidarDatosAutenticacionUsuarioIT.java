...
public class ValidarDatosAutenticacionUsuarioIT extends Tarea {
	...
	public void ejecutar(Object... params) {
		...
		Object[] parametrosAccion = infoUsuario.getvaloresParametrosAccion();
		InfoAccesoSinValidar infoAcceso = (InfoAccesoSinValidar) parametrosAccion[0];
		try {
			// Se obtienen las interfaces de los recursos. Si no se pueden
			// obtener las interfaces se debe generar un informe de tarea
			...
			if (visualizadorAcceso == null) {
				this.generarInformeConCausaTerminacion(identTarea,
						contextoEjecucionTarea, identAgenteOrdenante,
						"Error-Al Obtener la interfaz de uso de :"
								+ IdentRecursoVisualizacionAcceso,
						CausaTerminacionTarea.ERROR);
			} else {
				visualizadorAcceso.mostrarVisualizadorAcceso(
			...
			if (itfUsoPersistencia == null) {
				this.generarInformeConCausaTerminacion(
						...
						identAgenteOrdenante,
						"Error-Al Obtener:Interfaz_" + IdentRecursoPersistencia,
						CausaTerminacionTarea.ERROR);
			} else {
				boolean resultadoValidacion = itfUsoPersistencia
						.compruebaUsuario(infoAcceso.tomaUsuario(),
				...
				this.generarInformeOK(identTarea, contextoEjecucionTarea,
						identAgenteOrdenante, contenidoInformeTarea);
			}
		} catch (Exception e) {
			e.printStackTrace();
			this.generarInformeConCausaTerminacion(this.getIdentTarea(),
					contextoEjecucionTarea, identAgenteOrdenante,
					"Error-AlObtener:Interfaz_Recurso_Persistencia",
					CausaTerminacionTarea.ERROR);
		}
	}
}
