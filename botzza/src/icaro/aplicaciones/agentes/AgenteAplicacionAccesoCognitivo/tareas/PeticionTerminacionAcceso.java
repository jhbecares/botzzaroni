...
public class PeticionTerminacionAcceso extends Tarea {
		...
		try {
			...
			if (visualizadorAcceso == null) {
				...
				if (gestorAgentes == null) {
					gestorAgentes = (ItfUsoAgenteReactivo) AdaptadorRegRMI
							.getItfAgenteRemoto(
									NombresPredefinidos.NOMBRE_GESTOR_AGENTES,
									NombresPredefinidos.ITF_USO);
				}
				if (gestorAgentes == null) {
					...
					gestorAgentes.aceptaEvento(new EventoRecAgte(
							"peticion_terminar_todo", this.getAgente()
									.getIdentAgente(), null));
