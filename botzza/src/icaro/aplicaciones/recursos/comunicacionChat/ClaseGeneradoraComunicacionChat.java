...
public class ClaseGeneradoraComunicacionChat extends ImplRecursoSimple
	...
	private String chanel = "#kiwiirc-botzza";
	private String identInterlocutorPruebas = "pacopa22";
	...
	public ClaseGeneradoraComunicacionChat(String idInstanciaRecurso)
		...
		// repoIntfaces.obtenerInterfaz(NombresPredefinidos.ITF_USO+NombresPredefinidos.CONFIGURACION);
		// DescInstanciaRecursoAplicacion descRecurso =
		// config.getDescInstanciaRecursoAplicacion(idInstanciaRecurso);
		...
		try {
			...
			// this.wait(1000);
			// itfAgteControlador=(InterfazUsoAgente)
			// this.repoIntfaces.obtenerInterfazUso(identAgenteAReportar);
			// this.generarErrorCreacionComponente("itfAgteControlador es null");
			// }
			// ItfUsoExtractorSemantico
			// itfExtractorSem=(ItfUsoExtractorSemantico)
			// this.repoIntfaces.obtenerInterfazUso(identExtractorSem);
			// if (itfExtractorSem == null){
			// this.generarErrorCreacionComponente("itfExtractorSemantico es null");
			// }
			// if (itfExtractorSem == null ||itfAgteControlador == null )throw
			// new Exception();
	...
	public void comenzar(String identAgteControlador) throws Exception {
		...
		try {
			itfAgteControlador = (InterfazUsoAgente) this.repoIntfaces
					.obtenerInterfazUso(VocabularioGestionPizzeria.IdentAgenteIdentificadorBotzza);
			if (itfAgteControlador == null) {
				this.generarErrorCreacionComponente("itfAgteControlador es null");
			} else {
				interpreteMsgIrc
			...
			if ((itfExtractorSem == null) || (itfAgteControlador == null)) {
				...
				interpreteMsgIrc
						.setIdentAgenteGestorDialogo(VocabularioGestionPizzeria.IdentAgenteIdentificadorBotzza);
				interpreteMsgIrc
						.setIdentConexion(VocabularioGestionPizzeria.IdentConexionAgte);
	...
	@Override
	public void enviarMensagePrivado(String interlocutor, String mensaje)
			throws Exception {
