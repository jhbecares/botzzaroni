 ...
 * @author     Felipe Polo
 */
public class AccionesSemanticasGestorAgentes extends
	...
	public void configurarGestor() {
		try {
			...
			// NombresPredefinidos.ITF_USO + NombresPredefinidos.CONFIGURACION);
			// tiempoParaNuevaMonitorizacion =
			// Integer.parseInt(config.getValorPropiedadGlobal(NombresPredefinidos.INTERVALO_MONITORIZACION_ATR_PROPERTY));
			config = (ItfUsoConfiguracion) itfUsoRepositorio
	...
	public void crearAgentes() {
		...
		try {
			...
			listaDescripcionesAgtesACrear = (ArrayList<DescInstancia>) config
					.getDescInstanciaGestor(
							NombresPredefinidos.NOMBRE_GESTOR_AGENTES)
					.getComponentesGestionados();

			// Iterator<DescInstancia> iterador = null;

			if (listaDescripcionesAgtesACrear.isEmpty()) {

				this.informaraMiAutomata("listaAgentesGestionadosVacia",
						sinParametros);
			...
			//
			// DescInstanciaAgenteAplicacion instanciaActual =
			// (DescInstanciaAgenteAplicacion)iterador.next();
			// String identAgenteAcrear = instanciaActual.getId();
			//
			// logger.debug("GestorAgentes: Creando agente " + nombreAgente +
			// ".");
			// // Si el agente es remoto damos la orden al gestor de nodo para
			// que lo cree
			//
			...
			// // tipoAgenteaCrear =
			// instanciaActual.getDescComportamiento().getTipo();
			// if (instanciaActual.getDescComportamiento().getTipo() ==
			// TipoAgente.REACTIVO) {
			// crearAgenteReactivoEnEsteNodo (instanciaActual);
			// } else
			//
			// if (instanciaActual.getDescComportamiento().getTipo() ==
			// TipoAgente.COGNITIVO) {
			// crearUnAgenteCognitivo(identAgenteAcrear);
			// }
			// else { trazas.aceptaNuevaTraza(new InfoTraza(
			// "GestorAgentes",
			// "El subtipo del agente " + identAgenteAcrear +
			// " definido en la configuraci�n no es correcto.",
			// InfoTraza.NivelTraza.error));
			//
			//
			...
			// " definido en la configuraci�n no es correcto.");
			//
			// throw new Exception("El subtipo del agente " + identAgenteAcrear
			// + " definido en la configuraci�n no es correcto.");
			// }
			//
			...
			//
			// trazas.aceptaNuevaTraza(new InfoTraza("GestorAgentes",
			// "Añadiendo agente " + identAgenteAcrear +
			// " a la lista de objetos gestionados.",
			// InfoTraza.NivelTraza.debug));
			//
			//
			...
			try {
				this.informaraMiAutomata("error_en_creacion_agentes",
						sinParametros);
				// this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
	...
	private void crearAgenteReactivoEnEsteNodo(
			DescInstanciaAgenteAplicacion descInstAgente) throws Exception {
		String identAgenteACrear = descInstAgente.getId();
		try {

			FactoriaComponenteIcaro.instanceAgteReactInpObj()
					.crearAgenteReactivo(descInstAgente);
			...
			((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ identAgenteACrear))
					.setGestorAReportar(NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
	...
	private void crearAgenteCognitivoEnEsteNodo(
			DescInstanciaAgenteAplicacion descInstAgente) throws Exception {
		String identAgenteACrear = descInstAgente.getId();
		try {
			...
			// + identAgenteACrear)).setGestorAReportar(
			// NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
	...
	private void crearUnAgenteReactivo(String nombreAgente) throws Exception {
		try {
			...
			// ClaseGeneradoraRepositorioInterfaces.instance().obtenerInterfaz(
			// NombresPredefinidos.ITF_USO + NombresPredefinidos.CONFIGURACION);
			// logger.debug("GestorAgentes: Construyendo agente reactivo " +
			// nombreAgente + ".");
			//
			// trazas.aceptaNuevaTraza(new InfoTraza("GestorAgentes",
			...
			// trazas.aceptaNuevaTraza(new InfoTraza(
			// "GestorAgentes",
			// "Error al crear el agente " + nombreAgente +
			// " en un nodo remoto. Se volver� a intentar en " + intentos +
			// " segundos...\n nodo origen: " + esteNodo + "\t nodo destino: " +
			// nodoDestino,
			// InfoTraza.NivelTraza.error));
			//
			//
			// logger.error("Error al crear el agente " + nombreAgente +
	...
	private void crearUnAgenteEnNodoRemotoConRMI(
			DescInstanciaAgenteAplicacion descAgenteAcrear) throws Exception {
		...
		if (!misInterfacesEstanEnElRegistroRMILocal) { // Las añado -por ahora
			...
			if (!AdaptadorRegRMI.addElement2LocalRegRMI(
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						"GestorRecursos",
						"Error al añadir mi interfaz al registro RMI local. "
								+ nombreAgente
								+ "No se ha podido añadir  la itf de uso del Gestor de nodo en el registro RMI local "
								+ identNodoAgente, InfoTraza.NivelTraza.error));
			}
		}
		...
		try {
			int intentos = 0;
			boolean ok = false;
			// DescInstanciaGestor gestorAgentes =
			// config.getDescInstanciaGestor(identAgente);
			...
			while (!ok & (intentos <= maxNumIntentosCreacionCompGestionados)) {
				...
				try {
					...
					// Se ordena al Gestor de Nodo que cree al Gestor. Se debe
					// esperar la confirmacion
					// String[] lista = nodeRegistry.list();
					// Object obj =
					// nodeRegistry.lookup(NombresPredefinidos.NOMBRE_GESTOR_NODO);
					// ItfUsoAgenteReactivo itfUsoGestorNodo =
					// (ItfUsoAgenteReactivo)nodeRegistry.lookup(NombresPredefinidos.NOMBRE_GESTOR_NODO);

					ItfUsoAgenteReactivo itfUsoGestorNodo = AdaptadorRegRMI
							.getItfAgteReactRemoto(identNodoAgente,
									NombresPredefinidos.NOMBRE_GESTOR_NODO);
					if (itfUsoGestorNodo != null) {
						// Se crea una descripcion del GN . El GA crea y guarda
						// informacion sobre los GN con los que interactúa

						// DescInstanciaAgente descGN = new
						// DescInstanciaAgente();
						...
						// añado el gestor de nodo a la lista de descriptores
						// de GN
						// listaDescripcionesGestoresNodo.add(descGN);
						// Creamos el mensaje para ordenarle al GN que cree el
						// recurso en su nodo.
						// Deberíamos esperar una confirmación de la creacion.
						// Registro la interfaz de uso del gestor en el registro
						// RMI de la organizacion para que pueda ser localizado
						// y recibir informacion

						itfUsoGestorNodo.aceptaMensaje(new MensajeSimple(
								new InfoContEvtMsgAgteReactivo(
										"peticion_crearAgente",
										identAgenteAcrear), this.nombreAgente,
								NombresPredefinidos.NOMBRE_GESTOR_NODO));
						...
						trazas.aceptaNuevaTraza(new InfoTraza(
								...
								"Error al crear el recurso. "
										+ identAgenteAcrear
										+ "No se ha podido obtener la itf de uso del Gestor de nodo en el nodo remoto "
										+ identNodoAgente,
								InfoTraza.NivelTraza.error));
					}
				} catch (Exception e) {
					trazas.aceptaNuevaTraza(new InfoTraza(
							...
							"Error al crear el recurso:  "
									+ identAgenteAcrear
									+ " en un nodo remoto. Se volveraa intentar en "
									+ intentos
			...
			if (!ok) {
				trazas.aceptaNuevaTraza(new InfoTraza(
						...
						"No se pudo crear el agente "
								...
								+ intentos, InfoTraza.NivelTraza.error));
				logger.error("Error al crear el recurso "
			...
			logger.debug("GestorRecursos : Recurso :" + identAgenteAcrear
					+ "creado.");
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Recurso :"
					+ identAgenteAcrear + "creado.", InfoTraza.NivelTraza.debug));
		} catch (Exception e) {
			logger.error("GestorAgentes: Error al crear el agente "
					+ identAgenteAcrear + "en el nodo remoto "
					+ identNodoAgente, e);
			trazas.aceptaNuevaTraza(new InfoTraza(identAgenteAcrear,
					"Error al crear el agente " + identAgenteAcrear
							+ "en el nodo remoto " + identNodoAgente,
					InfoTraza.NivelTraza.error));
			e.printStackTrace();
			try {
	...
	public void addDescripcionGN(String identNodoGN,
			InterfazUsoAgente itfUsoGestorNodo) {
	...
	public void arrancarAgentes() {
		...
		while (enumAgentes.hasMoreElements()) {
			try {
				...
				trazas.aceptaNuevaTraza(new InfoTraza("GestorAgentes",
						"Es necesario arrancar el agente " + nombre
				...
				if (itfGesAg != null) { // arrancamos el agente que puede ser
					...
					trazas.aceptaNuevaTraza(new InfoTraza("GestorAgentes",
							...
							InfoTraza.NivelTraza.debug));
					logger.debug("GestorAgentes: Arrancando el agente "
							+ nombre + ".");
					itfGesAg.arranca();
					logger.debug("GestorAgentes: Orden de arranque ha sido dada al agente "
							+ nombre + ".");
					trazas.aceptaNuevaTraza(new InfoTraza("GestorAgentes",
							"Orden de arranque ha sido dada al agente "
									+ nombre + ".", InfoTraza.NivelTraza.debug));

					// si el agente es remoto almacenamos sus interfaces en el
					// repositorio de interfaces
					if (esAgenteRemoto) {
						itfUsoRepositorio.registrarInterfaz(
								NombresPredefinidos.ITF_GESTION + nombre,
								itfGesAg);
						Remote itfUsoAg = AdaptadorRegRMI.getItfAgenteRemoto(
								nombre, NombresPredefinidos.ITF_USO);
						itfUsoRepositorio.registrarInterfaz(
								NombresPredefinidos.ITF_USO + nombre, itfUsoAg);
					}
				} else { // la interf de gestion es null
					trazas.aceptaNuevaTraza(new InfoTraza(
							"GestorAgentes",
							"No se pudo arrancar  el agente "
									+ nombre
									+ ". Porque su interfaz no esta registrada en el repositorio",
							InfoTraza.NivelTraza.error));
					logger.debug("GestorAgentes: No se pudo arrancar  el agente  "
							+ nombre
							+ ". Porque  su interfaz no esta registrada en el repositorio");
					// Le debo enviar la posicion del agente en la lista de
					// agentes a arrancar para que se pueda recuperar
	...
	public void generarInformeErrorIrrecuperable() {
		...
		try {
			...
			// this.itfUsoPropiadeEsteAgente.aceptaEvento(new
			// EventoRecAgte("informe_generado",
			// NombresPredefinidos.NOMBRE_GESTOR_AGENTES,
			// NombresPredefinidos.NOMBRE_GESTOR_AGENTES));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean crearAgente(Integer indice) throws Exception {
		listaDescripcionesAgtesACrear.iterator();
		...
		// Si el agente es remoto damos la orden al gestor de nodo para que lo
		// cree
		...
		if (esAgtRemoto) {
			...
			// crearlos uno a uno dependiendo de su tipo
			tipoAgenteaCrear = instanciaActual.getDescComportamiento()
					.getTipo();
			if (tipoAgenteaCrear == TipoAgente.REACTIVO) {
				crearAgenteReactivoEnEsteNodo(instanciaActual);
			} else

			if ((tipoAgenteaCrear == TipoAgente.COGNITIVO)
					|| (tipoAgenteaCrear == TipoAgente.ADO)
		...
		// gestionados por el gestor
		// Suponemos que todo ha ido bien y lo añadimos a la lista de agentes
		// gestionados

		// Si es un agente remoto debemos esperar a que se reciba una
		// confirmación de su creación
		if (!esAgtRemoto) {
	...
	public void addAgenteAListaAgtesGestionados(String identAgente) {
		...
		logger.debug("GestorAgentes: Añadiendo agente " + identAgente
				+ " a la lista de objetos gestionados.");

		trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
				"Añadiendo agente " + identAgente
						+ " a la lista de objetos gestionados.",
				InfoTraza.NivelTraza.debug));
	...
	public Boolean esAgenteRemoto(String identAgente) {
		String nodoAgente;
		try {
			nodoAgente = config.getDescInstanciaAgenteAplicacion(identAgente)
			...
			Logger.getLogger(AccionesSemanticasGestorAgentes.class.getName())
	...
	public Boolean esRecursoRemoto(String identRecurso) {
		...
		try {
			nodoRecurso = config
					.getDescInstanciaRecursoAplicacion(identRecurso).getNodo()
					.getNombreUso();
	...
	public void monitorizarAgentes() {
		...
		while (enume.hasMoreElements() && !errorEncontrado) {
			...
			try {
				InterfazGestion itfGes = (InterfazGestion) this.itfUsoRepositorio
						.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
								+ nombre);
				int monitoriz = itfGes.obtenerEstado();
				if ((monitoriz == InterfazGestion.ESTADO_ERRONEO_IRRECUPERABLE)
						|| (monitoriz == InterfazGestion.ESTADO_ERRONEO_RECUPERABLE)
						|| (monitoriz == InterfazGestion.ESTADO_TERMINADO)
						|| (monitoriz == InterfazGestion.ESTADO_TERMINANDO)) {
					errorEncontrado = true;
					logger.debug("GestorAgentes:Agente " + nombre
							+ " est� en estado err�neo o terminado.");
	...
	public void terminarAgentesActivos() {
		...
		while (enumAgentes.hasMoreElements()) {
			try {
				...
				// para cada agente, recuperamos su itf de gestion
				logger.debug("GestorAgentes: Recuperando Itf Gestion del agente "
						+ nombre + ".");
				...
				if (itfGesAg != null) {
					itfGesAg.termina();
					logger.debug("GestorAgentes: Orden de terminacion ha sido dada al agente "
							+ nombre + ".");

					trazas.aceptaNuevaTraza(new InfoTraza("GestorAgentes",
							"Orden de terminacion ha sido dada al agente "
									+ nombre + ".", InfoTraza.NivelTraza.debug));
				}
			} catch (Exception ex) {
				logger.error("GestorAgentes: Hubo un problema al acceder a un interfaz remoto mientras se daba orden de terminaci�n al agente "
						+ nombre + ".");

				trazas.aceptaNuevaTraza(new InfoTraza(
						"GestorAgentes",
						"Hubo un problema al acceder a un interfaz remoto mientras se daba orden de terminacion al agente "
								+ nombre + ".", InfoTraza.NivelTraza.error));
	...
	public void peticionTerminacionGestoresNodo() {
		...
		if (listaDescripcionesGestoresNodo == null) {
			...
			while (posInstGN < numGNConocidos) {
				...
				// InterfazUsoAgente itfUsosAg = (InterfazUsoAgente)
				// descGN.getIntfUsoAgente();
				String identGN = descGN.getId();
				// finalizamos el agente
				logger.debug("GestorAgentes: Se envia mensaje de peticion de terminacion al GN  "
						+ identGN + ".");
				try {
					descGN.getIntfUsoAgente().aceptaMensaje(
							new MensajeSimple(new InfoContEvtMsgAgteReactivo(
									"peticion_terminar_todo", null),
									this.nombreAgente,
									NombresPredefinidos.NOMBRE_GESTOR_NODO));
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Se envia mensaje de peticion de terminacion al GN"
									+ identGN + ".", InfoTraza.NivelTraza.debug));

				} catch (Exception ex) {
					logger.error("GestorAgentes: Hubo un problema al enviar un mensaje al GEstor de Nodo  "
							+ identGN + ".");

					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Hubo un problema al enviar un mensaje al GEstor de Nodo  "
									+ identGN + ".", InfoTraza.NivelTraza.error));
					ex.printStackTrace();
				}
				posInstGN++;
			}
	...
	public void terminarGestorAgentes() {
		...
		try {
			...
			// NombresPredefinidos.NOMBRE_GESTOR_AGENTES,
			// NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
			this.comunicator.enviarInfoAotroAgente("gestor_agentes_terminado",
					NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION);
		} catch (Exception e) {
			e.printStackTrace();
		}
