...
public class PercepcionAgenteCognitivoImp extends PercepcionAgenteCognitivo {
	...
	public PercepcionAgenteCognitivoImp(int CapacidadBuzon,
			ProcesadorItems prItems, AgenteCognitivo agente) {
	...
	@Override
	public void SetParametrosPercepcionAgenteCognitivoImp(
			LinkedBlockingQueue<Object> colaEvtosyMsgs,
			ProcesadorItems prItems, AgenteCognitivo agente) {
	...
	public void SetProcesadorEvidencias(
			ItfProcesadorObjetivos itfProcesEvidencias) {
		if (itfProcesEvidencias != null) {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(this.agente.getIdentAgente(),
					"Interfaz del Procesador de Objetivos  == NULL!!!!!",
	...
	public synchronized void aceptaMensaje(MensajeSimple mensaje) {
		...
		// InfoTraza(this.agente.getIdentAgente(),"Percepcion: Ha llegado un nuevo mensaje desde "+mensaje.getEmisor(),InfoTraza.NivelTraza.debug));
		// trazas.aceptaNuevaTrazaMuestraMensaje(new
		// InfoTraza(this.agente.getIdentAgente()," Emisor: "+mensaje.getEmisor()+". Tipo Contenido: "+
		// mensaje.getContenido().getClass().getSimpleName()
		...
		if (mensaje instanceof MensajeSimpleConInfoTemporal) {
			...
			mensajeTemporizado.putMomentoRecepcionMsg(System
					.currentTimeMillis());
	...
	// JM: Nuevo metodo para tener acceso al procesador de items de la
	// percepcion
