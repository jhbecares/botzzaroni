...
public class ProcesadorObjetivosImp extends ProcesadorObjetivos {
	...
	public ProcesadorObjetivosImp(ItfMotorDeReglas ruleEngine,
			ItfGestorTareas taskManager) {
	...
	public boolean procesarCreencia(Creencia cre) {
		...
		if ((cre != null) & (cre.getContenido() != null)) {
			...
			// Se intenta introducir en el motor una creencia vacio o de
			// contenido vacio
			return false;
		}
	}
	...
	public boolean procesarEvidencia(Evidencia evidence) {
		Creencia believe;
		// verificamos la cardinalidad del contenido de la evidencia. Si es
		// mayor que uno se trata de una
	...
	public boolean procesarExtractedInfo(ExtractedInfo extrInfo) {
		// verificamos la cardinalidad del contenido de la informacion extraida.
		// Si es mayor que uno se trata de una
		...
		// Opcional se puede anadir al motor el InfoExtracted o solo su
		// contenido. Esto se podria configurar
		// ruleEngine.assertFact(extrInfo);
		return true;
	}
	...
	public Creencia asimilarEvidencia(Evidencia ev) {
		// A partir de las evidencias se generan creencias. En este caso se
		// trata de una sola evidencia a partir de la cual generamos una
		// creencia
		// Se pueden utilizar varios modelos e incluso delegar la creación a
		// una clase exterior
		// dependiente del dominio. Aqui nos limitamos a creernos la evidencia
		// recibida. Tranformamos la evidencia en creencia
	...
	public Creencia nuevaCreencia(Object emisor, Object creador,
			Object contenido) {
		// A partir de las evidencias se crean creencias
		// Se pueden utilizar varios modelos e incluso delegar la creación a
		// una clase exterior
		// dependiente del dominio. Aqui nos limitamos a creernos todas las
	...
	public ArrayList<Creencia> asimilarEvidencias(Evidencia evidence) {
		...
		while (it.hasNext()) {
			...
			if (objetoActual != null) {
				Creencia nuevaCrencia = nuevaCreencia(origen, creador,
						objetoActual);
