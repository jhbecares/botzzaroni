...
public class FactoriaProcesadorObjetivosImp extends FactoriaProcesadorObjetivos {
	...
	public ProcesadorObjetivos crearProcesadorObjetivos(AgenteCognitivo agente,
		...
		InputStream reglas;
		// Path rutaReglas =
		// FileSystems.getDefault().getPath(ficheroResolucionObjetivos).getFileSystem().;
		...
		try {
			if (reglas != null) {
				...
				ItfMotorDeReglas motorDeReglas = FactoriaMotorDeReglas
						.instance().crearMotorDeReglas(agente);
				...
				motorDeReglas.addGlobalVariable(
						NombresPredefinidos.TASK_MANAGER_GLOBAL, taskManager);
				motorDeReglas
						.addGlobalVariable(
								NombresPredefinidos.ITFUSO_RECURSOTRAZAS_GLOBAL,
								trazas);
				motorDeReglas.addGlobalVariable(
						NombresPredefinidos.AGENT_ID_GLOBAL, agentName);

				int index = getNumberStartIndex(agentName);
				getNumber(agentName, index);
			...
			throw new ExcepcionEnComponente("FactoriaMotorDeReglas",
					"Errors compiling rules for agent " + agentName,
					"MotorDeReglas", "Creacion del Motor de Reglas");
		}
	}
