...
public abstract class FactoriaMotorDeReglas {
	...
	// "icaro.infraestructura.PatronAgenteCognitivo.procesadorObjetivos.motorDeReglas.imp";
	...
	public static FactoriaMotorDeReglas instance() {
		if (instance == null) {
			String c = System.getProperty(
					NombresPredefinidos.RUTA_FACTORIA_MOTOR_REGLAS,
					FactoriaMotorDeReglasDroolsImp1.class.getName());
			try {
				instance = (FactoriaMotorDeReglas) Class.forName(c)
						.newInstance();
			} catch (Exception ex) {
	...
	// public abstract ItfMotorDeReglas crearMotorDeReglas(AgenteCognitivo
	// agent, InputStream reglas);
	public abstract ItfMotorDeReglas crearMotorDeReglas(AgenteCognitivo agent,
			InputStream reglas, String ficheroReglas);
}
