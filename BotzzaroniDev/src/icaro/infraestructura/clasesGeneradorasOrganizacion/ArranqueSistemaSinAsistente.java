		...
		if (args.length == 0) {
			...
			try {
				ItfUsoRepositorioInterfaces repositorioInterfaces = ClaseGeneradoraRepositorioInterfaces
						.instance();
				configuracionExterna = ClaseGeneradoraConfiguracion
						.instance(args[0]);
				...
				System.err
						.println("Error. No se ha podido interpretar o registrar la descripcion.");
				e.printStackTrace();
				JOptionPane
						.showConfirmDialog(
								new JFrame(),
			...
			if (!herramientaArrancada) {
				try {
					DescInstanciaAgente descGestor = configuracionExterna
					...
					FactoriaAgenteReactivo.instancia().crearAgenteReactivo(
							descGestor);
					gestorOrg = (ItfGestionAgenteReactivo) ClaseGeneradoraRepositorioInterfaces
							.instance()
							.obtenerInterfaz(
									NombresPredefinidos.ITF_GESTION
					...
					System.err
							.println("Error. No se ha podido crear el gestor de organizacion con nombre "
									+ NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION);
					System.exit(1);
				} catch (Exception e) {
					System.err
