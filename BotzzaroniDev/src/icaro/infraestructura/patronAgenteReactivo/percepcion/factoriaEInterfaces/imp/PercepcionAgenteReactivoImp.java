...
public class PercepcionAgenteReactivoImp extends PercepcionAbstracto {
	...
	public PercepcionAgenteReactivoImp() {
		// Se crea un objeto vacio. Es necesario definir los parametros para que
	...
	public PercepcionAgenteReactivoImp(AgenteReactivoAbstracto agente) {
		...
		this.agente = agente;
		this.procesador = new ProcesadorItemsPercepReactivo(agente,
				agente.getItfControl());
		this.envioItems = new EnvioItemsThread();

	}

	public PercepcionAgenteReactivoImp(int CapacidadBuzon,
			ProcesadorItemsPercepReactivo prItems,
			AgenteReactivoAbstracto agente) {
	...
	public PercepcionAgenteReactivoImp(AgenteReactivoAbstracto agente,
			ProcesadorItemsPercepReactivo procesador) {
		this.agente = agente;
	...
	public void SetParametrosPercepcionAgenteReactivoImp(
			LinkedBlockingDeque<Object> colaEvtosyMsgs,
			ProcesadorItemsPercepReactivo prItems,
			AgenteReactivoAbstracto agente) {
	...
	public synchronized void aceptaEvento(EventoSimple evento)
		...
		trazas.aceptaNuevaTraza(new InfoTraza(this.agente.getIdentAgente(),
				"Percepcion: Ha llegado un nuevo evento desde "
						+ evento.getOrigen(), InfoTraza.NivelTraza.debug));
		trazas.aceptaNuevaTrazaEventoRecibido(this.agente.getIdentAgente(),
				evento);
		buzon.offer(evento);
	}
	...
	public synchronized void aceptaMensaje(MensajeSimple mensaje)
			throws RemoteException {
		trazas.aceptaNuevaTraza(new InfoTraza(this.agente.getIdentAgente(),
				"Percepcion: Ha llegado un nuevo mensaje desde "
						+ mensaje.getEmisor(), InfoTraza.NivelTraza.debug));
	...
	private class EnvioItemsThread extends Thread {
			...
			while (!termina) {
				...
				try {
					...
					java.util.logging.Logger.getLogger(
							PercepcionAgenteReactivoImp.class.getName()).log(
		...
		public void termina() {
			...
			// envioItems.finalize();
			// } catch (Throwable ex) {
			// java.util.logging.Logger.getLogger(PercepcionAgenteReactivoImp.class.getName()).log(Level.SEVERE,
			// null, ex);
			// }
	...
	public Object consumeConTimeout(int tiempoEnMilisegundos)
		...
		try {
			Object obj = buzon
					.poll(tiempoEnMilisegundos, TimeUnit.MILLISECONDS);
			return obj;

		} catch (InterruptedException e) {
			throw new ExcepcionSuperadoTiempoLimite(
					"Percepcion: Interrumpida la espera de nuevo item en el buzon de items");
		}
	}
	 ...
	 *
	 * @param evento
	 *            Evento que se consumir el primero
	 */
	@Override
