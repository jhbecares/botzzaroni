...
public class ProcesadorItemsPercepReactivo {
	...
	public ProcesadorItemsPercepReactivo(AgenteReactivoAbstracto agente,
		...
		try {
			...
			java.util.logging.Logger.getLogger(
					ProcesadorItemsPercepReactivo.class.getName()).log(
	...
	private boolean filtrarItem() {
		...
		if (item instanceof EventoSimple) {
			...
			// (this.agente.getLocalName(),"Percepcin: El evento" +
			// evento.toString()
			// + " ha sido filtrado por el agente " + nombreAgente
			...
			if (!mensaje.getReceptor().equals(nombreAgente)) {
				...
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
						"Percepcion: El mensaje " + mensaje.toString()
								+ " no tiene como receptor el agente "
								+ nombreAgente
								+ "\n Destinatario del mensaje: "
								+ mensaje.getReceptor(),
						InfoTraza.NivelTraza.debug));
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Percepcin: Item " + item + " no reconocido",
					InfoTraza.NivelTraza.debug));
			return false;
		}
	...
	// Extrae del evento o del mensaje la información que necesita el control
	// para transitar
	private InfoContEvtMsgAgteReactivo extraerInfoControl() {
		...
		if (item instanceof EventoSimple) {
			...
			infoControlExtraida = (InfoContEvtMsgAgteReactivo) evento
					.getContenido();
			// infoControlExtraida.setInput(evento.getMsg());
			// infoControlExtraida.setvaloresParametrosAccion(evento.getMsgElements());

		} else if (item instanceof MensajeSimple) {
			// En el contenido del mensaje se debe poner un objeto del tipo
			// InfoControlAgteReactivo
			MensajeSimple mensaje = (MensajeSimple) item;
			infoControlExtraida = (InfoContEvtMsgAgteReactivo) mensaje
					.getContenido();
	...
	public synchronized void procesarItem(Object item) throws RemoteException {
		...
		if (item instanceof EventoInternoAgteReactivo) {
			itfControlReactivo.procesarInput(
					((EventoInternoAgteReactivo) item).input,
					((EventoInternoAgteReactivo) item).valoresParametrosAccion);
		} else {
			if (item instanceof EventoSimple) {
			...
			if (infoExtraida != null) {
				itfControlReactivo
						.procesarInfoControlAgteReactivo(infoExtraida);
			} else {
				log.error("No se ha podido extraer informacion valida del  "
						+ item + " no reconocido");
				trazas.aceptaNuevaTraza(new InfoTraza(this.agente
						.getIdentAgente(), "Percepcion: Item " + item
						+ " no se ha podido extraer informacion valida",
						InfoTraza.NivelTraza.debug));
			}
		}
