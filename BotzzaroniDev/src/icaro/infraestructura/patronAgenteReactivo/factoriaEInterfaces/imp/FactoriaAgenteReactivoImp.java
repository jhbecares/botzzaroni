...
public class FactoriaAgenteReactivoImp extends FactoriaAgenteReactivo {
	...
	/*
	 * Crea una instancia del patr�n que crea un agente reactivo a partir de
	 * las acciones sem�nticas, el aut�mata que define el comportamiento y
	...
	public void crearAgenteReactivo(DescInstanciaAgente descInstanciaAgente)
		...
		DescComportamientoAgente agente = descInstanciaAgente
		...
		if ((accionesSemanticasEspecificas != null) & (rutaTabla != null)) {
			...
			try {
				...
				// Paso 0 se obtienen las interfaces de los recursos externos :
				// trazas e interfaces

				RepositorioIntfaces = ClaseGeneradoraRepositorioInterfaces
						.instance();

				// Se obtienen los parametros y se crean los subcomponentes
				// utilizando las factorias
				...
				// itfGesControl = (InterfazGestion)
				// control.getProcesadorEventos();
				...
				AgenteReactivoAbstracto patron = new AgenteReactivoImp(
						nombreInstanciaAgente, itfAutomata,
						itfControlAgteReactivo, itfProductorPercepcion,
						itfConsumidorPercepcion);
	...
	/*
	 * Este m�todo crea un agente reactivo a partir de un fichero donde se
	 * especifica su configuraci�n
	 */
	...
	// (ItfGestionAgenteReactivo)patron);
	// RepositorioInterfaces.instance().registrarInterfaz(
	...
	// }
	@Override
	public void crearAgenteReactivo(String nombreInstanciaAgente,
			String rutaComportamiento) throws ExcepcionEnComponente {
		...
		try {
			...
			AccionesSemanticasAgenteReactivo accionesSemanticasEspecificas = (AccionesSemanticasAgenteReactivo) AccionesSemanticas
					.newInstance();

			accionesSemanticasEspecificas
					.setNombreAgente(nombreInstanciaAgente);
			...
			ProcesadorInfoReactivoAbstracto control = FactoriaControlAgteReactivo
					.instancia().crearControlAgteReactivo(
							accionesSemanticasEspecificas, RutaAutomata,
							nombreInstanciaAgente, itfConsumidorPercepcion,
							itfProductorPercepcion);
			itfControlAgteReactivo = control;
			...
			AgenteReactivoAbstracto patron = new AgenteReactivoImp(
					nombreInstanciaAgente, itfAutomata, itfControlAgteReactivo,
					itfProductorPercepcion, itfConsumidorPercepcion);
			...
			RepositorioIntfaces.registrarInterfaz(
					...
					patron);
			RepositorioIntfaces.registrarInterfaz(NombresPredefinidos.ITF_USO
					+ nombreInstanciaAgente, patron);
	...
	private String obtenerRutaValidaAutomata(String rutaComportamiento)
		...
		rutaComportamiento = File.separator
				+ rutaComportamiento.replace(".", File.separator);
		 ...
		 * nombreFichero = ficherosRuta[ i ]; if
		 * (nombreFichero.startsWith(NombresPredefinidos.PREFIJO_AUTOMATA ) ){
		 * ficheroEncontrado = true; } } /
		 */
		if (ficheroEncontrado) {
			return rutaComportamiento.replace(File.separator, "/") + "/"
					+ nombreFichero;
		}
		{

			throw new ExcepcionEnComponente("PatronAgenteReactivo",
					"No se encuentra el fichero del automata en la ruta :"
							+ rutaComportamiento,
					"Factoria del Agente Reactivo",
	...
	private String normalizarRuta(String ruta) {
		/*
		 * Esta funci�n cambia la primera letra del nombre y la pone en
		 * min�sculas
		 */
	...
	private Class obtenerClaseAccionesSemanticas(DescInstanciaAgente instAgente)
			throws ExcepcionEnComponente {

		DescComportamientoAgente comportamiento = instAgente
				.getDescComportamiento();
		...
		try {
			...
			java.util.logging.Logger.getLogger(
					FactoriaAgenteReactivoImp.class.getName()).log(
					Level.SEVERE, null, ex);
			throw new ExcepcionEnComponente("PatronAgenteReactivo",
					"No se encuentra la clase de acciones semanticas en la ruta :"
							+ ruta, "Factoria del Agente Reactivo",
					"Class obtenerClaseAccionesSemanticas(DescInstanciaAgente instAgente)");
		}
	}

	private Class obtenerClaseAccionesSemanticas(String rutaComportamiento)
			throws ExcepcionEnComponente {
		// La ruta del comportamiento no incluye la clase
		// Obtenemos la clase de AS en l aruta
		String rutaClases = rutaComportamiento;
		rutaComportamiento = "src" + File.separator
				+ rutaComportamiento.replace(".", File.separator);
		...
		for (int i = 0; (i < ficherosRuta.length) & !ClaseASEncontrada; i++) {
			...
			if (nombreClase
					.startsWith(NombresPredefinidos.PREFIJO_CLASE_ACCIONES_SEMANTICAS)
	...
	private AccionesSemanticasAgenteReactivo obtenerAcciones(
			DescInstanciaAgente instAgente) {
		...
		// DescComportamientoAgenteReactivo comportamientoReactivo =
		// (DescComportamientoAgenteReactivo) comportamiento;
		String ruta = comportamiento.getLocalizacionComportamiento() + "."
				+ NombresPredefinidos.PREFIJO_CLASE_ACCIONES_SEMANTICAS
				+ comportamiento.getNombreComportamiento();
		// } else
		...
		try {
			Class claseAcciones = Class.forName(ruta);
			AccionesSemanticasAgenteReactivo acciones = (AccionesSemanticasAgenteReactivo) claseAcciones
					.newInstance();
	...
	private String obtenerRutaTablaTransiciones(DescInstanciaAgente instAgente) {
		...
		/*
		 * String ruta = "./automatas/TablaEstados"+nombreAgente+".xml"; return
		 * ruta;
		 */
	}
