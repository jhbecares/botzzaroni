...
public abstract class AccionSincrona {
	...
	// public AccionSincrona(ItfAutomataEFconGestAcciones envioInputs,
	// AgenteCognitivo agente) {
	// public AccionSincrona() {
	...
	// this.repoInterfaces = NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ;
	// }
	public void inicializar(String identAgte,
			ItfProductorPercepcion iftPercepAgte) {
	...
	// public void generarInformeConCausaTerminacion (String idAccion,Objetivo
	// contxtGoal,String idAgenteOrdenante,Object contenido,
	// CausaTerminacionAccion causaTerminacion ){
	//
	// String identGoal = null;
	// if (contxtGoal!= null) identGoal = contxtGoal.getgoalId();
	// InformeDeAccion resultadoAccion = new InformeDeAccion
	// (idAccion,identGoal,idAgenteOrdenante,contenido, causaTerminacion );
	...
	public void generarInputAutomata(Object input) {
		if (itfEnvioEventosInternos != null) {
			itfEnvioEventosInternos
					.produceParaConsumirInmediatamente(new EventoInternoAgteReactivo(
							identAccion, input, null));
			// envioHechos.insertarHecho(informe.getContenidoInforme());
		}
	}

	// public void generarInformeOK (String idTarea,Objetivo contxtGoal,String
	// idAgenteOrdenante, Object contenido){
	// String goalId = null ;
	// if (contxtGoal!=null){
	// goalId= contxtGoal.getgoalId();
	// }
	// InformeDeTarea resultadoTarea = new InformeDeTarea
	// (idTarea,goalId,idAgenteOrdenante, contenido );
	// itfProcObjetivos.insertarHecho(resultadoTarea);
	// // envioHechos.insertarHecho(contenido);
	// }

	public void generarInputTemporizador(long milis, String idAccion,
			String msgTimeout) {
		// String goalId = null ;
		// if (contxtGoal!=null){
		// goalId= contxtGoal.getgoalId();
		...
		InformeTemporizacion informeTemp = new InformeTemporizacion(idAccion,
				msgTimeout);
		GeneracionInputTimeout informeTemporizado = new GeneracionInputTimeout(
				milis, itfUsoAgente, informeTemp);
		informeTemporizado.start();
	}

	public void generarInformeTemporizadoFromConfigProperty(
			String identproperty, Objetivo contxtGoal,
			String idAgenteOrdenante, String msgTimeout) {
		try {
			// el nombre de la propiedad es el nombre de la tarea y debe estar
			// definido en la configuracion. El valor debe ser un entero
			// identproperty =
			// NombresPredefinidos.PREFIJO_PROPIEDAD_TAREA_TIMEOUT+identproperty;
			int valorTimeout = this.getItfUsoConfiguracion()
					.getValorNumericoPropiedadGlobal(
							NombresPredefinidos.PREFIJO_PROPIEDAD_TAREA_TIMEOUT
									+ identproperty);
			if (valorTimeout <= 0) {
				trazas.trazar(
						"Accion",
						"Se ejecuta la accion "
								...
								+ " Verifique el nombre de la propiedad en la descripcion de la organizacion :  "
								+ NombresPredefinidos.PREFIJO_PROPIEDAD_TAREA_TIMEOUT
								+ identproperty, NivelTraza.error);
				valorTimeout = this
						.getItfUsoConfiguracion()
						.getValorNumericoPropiedadGlobal(
								NombresPredefinidos.PROPERTY_TIME_TIMEOUT_POR_DEFECTO);
				if (valorTimeout <= 0) {
					trazas.trazar(
							"Accion",
							"Se ejecuta la tarea "
									...
									+ " Defina el nombre de la propiedad :"
									+ NombresPredefinidos.PROPERTY_TIME_TIMEOUT_POR_DEFECTO
			...
			this.generarInputTemporizador(valorTimeout, identproperty,
					msgTimeout);
			trazas.trazar("Accion",
					"Se ejecuta la accion " + this.getIdentAccion()
							+ " Se activa un informe temporizado :  "
							+ msgTimeout, NivelTraza.debug);
			...
			trazas.trazar(
					...
					"Se ejecuta la accion "
							...
							+ " Verifique el nombre de la propiedad en la descripcion de la organizacion :  "
							+ identproperty, NivelTraza.error);
			Logger.getLogger(AccionSincrona.class.getName()).log(Level.SEVERE,
					null, ex);
	...
	public ItfUsoConfiguracion getItfUsoConfiguracion() {
		if (itfConfig == null) {
			try {
				itfConfig = (ItfUsoConfiguracion) repoInterfaces
						.obtenerInterfaz(NombresPredefinidos.ITF_USO
								+ NombresPredefinidos.CONFIGURACION);
			} catch (Exception ex) {
				Logger.getLogger(AccionSincrona.class.getName()).log(
	...
	public void procesarInput(Object input, Object[] infoComplementaria) {
		if (itfEnvioEventosInternos == null) {
			trazas.trazar(identAgte,
					" El interfaz para el envio de eventos internos no esta definida "
							+ " El input:  " + input + " No sera procesado ",
					InfoTraza.NivelTraza.error);
		} else {
			itfEnvioEventosInternos
					.produceParaConsumirInmediatamente(new EventoInternoAgteReactivo(
							identAgte, input, infoComplementaria));
		}
	}
