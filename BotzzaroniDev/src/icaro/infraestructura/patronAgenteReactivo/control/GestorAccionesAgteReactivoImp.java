 ...
 * @author F Garijo
 */
public class GestorAccionesAgteReactivoImp extends GestorAccionesAbstr
		implements ItfGestorAcciones {
	...
	// public GestorAccionesImp(AgenteCognitivo agente,ItfProcesadorObjetivos
	// envioHechos){
	public GestorAccionesAgteReactivoImp() {
		...
		accionesSemAgteReactivoSimpleName = AccionesSemanticasAgenteReactivo.class
				.getSimpleName();
	...
	// public void setItfAutomataEFconGestAcciones(ItfUsoAutomataEFE
	// itfautomata){
	// envioInputs = itfautomata;
	// }
	// private void
	// setInfoParaAccionesSemanticasAgteReactivo(ItfControlAgteReactivo
	// itfCtrlAgte){
	...
	public void inicializarInfoAccionesAgteReactivo(String identAgte,
			ItfProductorPercepcion itfEvtosInternos,
			ItfControlAgteReactivo itfControl) {
		try {
			identPropietario = identAgte;
			...
			java.util.logging.Logger.getLogger(
					GestorAccionesAgteReactivoImp.class.getName()).log(
					Level.SEVERE, null, ex);
			this.trazas
					.trazar("GestorAccionesAgteReactivoImp",
							"Operacion de inicializar Accion. Error en los objetos de inicializacion: Revisar ",
							InfoTraza.NivelTraza.error);
		}
	}

	@Override
	public void inicializarInfoAcciones(Class claseAccionAinicializar,
			Object... paramsInicializacion) {
	...
	public Accion crearAccion(Class clase) throws Exception {
		...
		// identAccion =
		// identAccionLong.substring(identAccionLong.lastIndexOf(".")+1);
	...
	public AccionesSemanticasAgenteReactivo getInstanceASagteReactivo(
			Class accionClass) {

		if (accionesSemAgteReactivo == null) {
			identAccion = accionClass.getSimpleName();
		}
		// if (accionesCreadas.containsKey(identAccion)) return
		// (AccionesSemanticasAgenteReactivo) accionesCreadas.get(identAccion);
		// AccionesSemanticasAgenteReactivo accion;
		try {
			accionesSemAgteReactivo = (AccionesSemanticasAgenteReactivo) accionClass
					.newInstance();
			accionesSemAgteReactivo.inicializarAcciones(this.identPropietario,
					itfControlAgte, envioInputs);
			// accionesSemAgteReactivo.itfAutomataControl = this.envioInputs;
			accionesCreadas.put(identAccion, accionesSemAgteReactivo);
			// accionesSemAgteReactivo.trazas =
	...
	public void ejecutar(Object... paramsEjecucion) throws Exception {
		...
		if (paramsEjecucion == null) {
			this.trazas
					.trazar(this.getClass().getSimpleName(),
							"Error en  la ejecucion de la accion: "
									+ " No se han especificado parametros de ejecucion",
							InfoTraza.NivelTraza.error);
		}
		...
		if (claseAccionEjecutar == null) {
			this.trazas
					.trazar(this.getClass().getSimpleName(),
							"Error en  la ejecucion de la accion: "
									+ " No se ha especificado la clase de las acciones a ejecutar",
							InfoTraza.NivelTraza.error);
		}
		...
		if (superclase.equals(accionSincSimpleName)
				|| superclase.equals(accionesSemAgteReactivoSimpleName)) {
			...
			this.trazas
					.trazar(this.getClass().getSimpleName(),
							"Error en  la ejecucion de la accion: "
									+ claseAccionEjecutar.getSimpleName()
									+ " debe extender a AccionSincrona o a AccionAsincrona ",
							InfoTraza.NivelTraza.error);
		}
	}

	@Override
	public void ejecutarAccion(Class claseAccionEjecutar,
			Object... paramsEjecucion) throws Exception {
		// Class claseAccionEjecutar = (Class)paramsEjecucion[0];
		// Extraccion de parametros y verificacion de la clase
		...
		// accionesSemAgteReactivo =
		// getInstanceASagteReactivo(claseAccionEjecutar);
		// }
		if (superclase.equals(accionSincSimpleName)) {
	...
	@Override
	public synchronized void ejecutarMetodo(Class claseAccionEjecutar,
			String identMetodo, Object[] paramsEjecucion) throws Exception {
		// protected synchronized void ejecutarAccionBloqueante(String nombre,
		// Object[] parametros) throws ExcepcionEjecucionAcciones {
		String superclase = claseAccionEjecutar.getSuperclass().getSimpleName();
		...
		if ((paramsEjecucion == null)
				|| ((paramsEjecucion.length == 1) && (paramsEjecucion[0] == null))) {
			params = new Class[0];
		...
		try {
			...
			Method thisMethod = claseAccionEjecutar.getMethod(identMetodo,
					params);
			thisMethod.invoke(accionesSemAgteReactivo, paramsObj);
		} catch (IllegalAccessException iae) {
			System.out
					.println("ERROR en los privilegios de acceso (no es publico) para en metodo: "
							+ identMetodo
							+ " de la clase: "
							+ accionesSemAgteReactivo.getClass().getName());
			iae.printStackTrace();
			throw new ExcepcionEjecucionAcciones("AccionesSemanticasImp",
					"error al ejecutar un metodo" + identMetodo
							+ " de la clase: "
							+ accionesSemAgteReactivo.getClass().getName(),
					"se ha producido un IlegalAccessIAE");
		} catch (NoSuchMethodError nsme) {
			System.out.println("ERROR (NO EXISTE) al invocar el metodo: "
					+ identMetodo + " en la clase: "
					+ accionesSemAgteReactivo.getClass().getName());
			nsme.printStackTrace();
			throw new ExcepcionEjecucionAcciones(
					"AccionesSemanticasImp",
					"error al ejecutar un metodo" + identMetodo
							+ " de la clase: "
							+ accionesSemAgteReactivo.getClass().getName(),
			...
			System.out
					.println("Invocando metodo con parametros de sus superclases correspondientes");
			throw new ExcepcionEjecucionAcciones(
					"AccionesSemanticasImp",
					"error al ejecutar un metodo" + identMetodo
							+ " de la clase: "
	...
	@Override
	public synchronized void ejecutarMetodoThread(Class claseAccionEjecutar,
			String identMetodo, Object... paramsEjecucion) throws Exception {
		throw new ExcepcionEjecucionAcciones(
				"GestorAccionesImp",
				"error al ejecutar un metodo" + identMetodo + " de la clase: "
						+ accionesSemAgteReactivo.getClass().getName(),
