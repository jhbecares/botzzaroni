...
public class TablaEstadosControl {
	...
	/**
	 * Devuelve la operacion a realizar desde el estado Qi con el input i PRE:
	 * El estado actual no es final. El input pertenece a los aceptados por ese
	 * estado
	 *
	 * @param estadoActual
	...
	public Operacion dameOperacion(String estadoActual, String input) {
		Hashtable operaciones = inputsDeEstados.get(estadoActual);
	...
	public boolean esEstadoFinal(String estado) {
		return (clasificacionEstados.get(estado).intValue() == TablaEstadosControl.TIPO_DE_ESTADO_FINAL);
	}
	...
	public boolean esInputValidoDeEstado(String estado, String input) {
		Hashtable inp = inputsDeEstados.get(estado);
		if (inp != null) {
	 ...
	 */
	public void putTransicion(String estado, String input, String accion,
			String estadoSiguiente, String modo) {
		boolean modoBloqueante = modo.equalsIgnoreCase("bloqueante");
		Operacion operacion = new Operacion(accion, estadoSiguiente,
				modoBloqueante);

		// obtenemos la tabla de inputs y operaciones
		Hashtable<String, Operacion> inputsDeUnEstado = inputsDeEstados
				.get(estado);
	...
	/**
	 * Aade la transicin indicada como parmetro a todos los estados del autmata
	 * PRE: El autmata debe estar completamente creado ( todos los estados
	 * aadidos ) NOTA: En caso de existir inputs repetidos, se dejar intacto el
	 * input ya existente
	 *
	 * @param input
	 *            input de la transicin universal
	 * @param accion
	 *            accin a ejecutar en la transicin universal
	 * @param estadoSig
	 *            estado al que llegamos tras la transicin universal
	 * @param modo
	 *            modo de ejecucin de la accin
	 */
	public void putTransicionUniversal(String input, String accion,
			String estadoSig, String modo) {
		...
		while (iteradorClaves.hasMoreElements()) {
			...
			// aadimos la transicin si no es estado final y el input no estaba
			// en ese estado
			if ((!esEstadoFinal(estadoPivote))
					&& (!esInputValidoDeEstado(estadoPivote, input))) {
	...
	public String toString() {
		...
		while (nombres.hasMoreElements()) {
			...
			Hashtable inp = inputsDeEstados.get(id);
			if (!inp.isEmpty()) {
