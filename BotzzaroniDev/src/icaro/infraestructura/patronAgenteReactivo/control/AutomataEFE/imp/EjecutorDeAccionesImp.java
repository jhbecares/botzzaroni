...
public class EjecutorDeAccionesImp extends EjecutorDeAccionesAbstracto {
	 ...
	 *            Clase que contiene las acciones ejecutables java
	 */
	public EjecutorDeAccionesImp(
			AccionesSemanticasAgenteReactivo accionesSemanticas) {
	...
	/**
	 * @param accionesSemanticas
	 *            The accionesSemanticas to set.
	 * @uml.property name="accionesSemanticas"
	 */
	public synchronized void setAccionesSemanticas(
			AccionesSemanticasAgenteReactivo accionesSemanticas) {
		this.accionesSemanticas = accionesSemanticas;
	}
	 ...
	 *
	 * @param accion
	 *            Nombre del mtodo a ejecutar
	 * @param modoBloqueante
	 *            Marca la espera o no por el resultado, en caso de ser no
	 *            bloqueante se ejecuta en una nueva hebra
	 */
	@Override
	public synchronized void ejecutarAccion(String accion, Object[] parametros,
			boolean modoBloqueante) throws ExcepcionEjecucionAcciones {
		/**/
		if (accion != null) {
			if (modoBloqueante) {
			...
			System.out
					.println("AVISO: Accion Ignorada.La accion a ejecutar era null");
		}
	}
	...
	public void run() {
		try {
			...
			Logger.getLogger(EjecutorDeAccionesImp.class.getName()).log(
		...
		try {
			...
			System.out
					.println("ERROR en los privilegios de acceso (no es publico) para en metodo: "
							+ nombre
							+ " de la clase: "
							+ accionesSemanticas.getClass().getName());
			iae.printStackTrace();
			throw new ExcepcionEjecucionAcciones("AccionesSemanticasImp",
					"error al ejecutar un metodo" + nombre + " de la clase: "
							+ accionesSemanticas.getClass().getName(),
			...
			System.out.println("ERROR en la ejecucion del metodo: " + nombre
					+ " en la clase: "
					+ accionesSemanticas.getClass().getName());
			ite.printStackTrace();
	...
	/**
	 * Mtodo que ejecuta la accin dinmicamente esperando su vuelta. Con el orden
	 * se va evaluando distintos niveles de herencia en los parmetros teniendo
	 * en cuenta que pueden pasarse infinitos parmetros. El modo de operacin
	 * consiste en probar llamando a mtodos con las clases finales en la
	 * jerarqua de herencia y en caso de no encontrar el mtodo solicitado, subir
	 * un nivel de herencia para el primer parmetro manteniendo el resto hasta
	 * llegar a la clase Object (primer nivel de la jeranqua). En ese momento se
	 * sube un nivel en la jerarqua del segundo parmetro...
	 *
	 * @param nombre
	 *            Nombre del mtodo a ejecutar
	 */
	protected void ejecutarAccionBloqueantePolimorfica(String nombre,
			Object[] parametros, int nivel) {
		...
		try {
			...
			System.out
					.println("ERROR en los privilegios de acceso (no es pblico) para en mtodo: "
							+ nombre
							+ " de la clase: "
							+ accionesSemanticas.getClass().getName());
			iae.printStackTrace();
		} catch (NoSuchMethodError nsme) {
			System.out.println("ERROR (NO EXISTE) al invocar el mtodo: "
					+ nombre + " en la clase: "
					+ accionesSemanticas.getClass().getName());
			nsme.printStackTrace();
		} catch (NoSuchMethodException nsmee) {
			...
			nsmee.printStackTrace();
			System.out
					.println("Invocando mtodo con parmetros de sus superclases correspondientes");
			if (!salir) {
				ejecutarAccionBloqueantePolimorfica(nombre, parametros, ++nivel);
			}
		} catch (InvocationTargetException ite) {
			System.out.println("ERROR en la ejecucin del mtodo: " + nombre
					+ " en la clase: "
					+ accionesSemanticas.getClass().getName());
			ite.printStackTrace();
	 ...
	 *            Nombre del mtodo a ejecutar
	 * @param parametros
	 *            Parmetros del mtodo a ejecutar
	 */
