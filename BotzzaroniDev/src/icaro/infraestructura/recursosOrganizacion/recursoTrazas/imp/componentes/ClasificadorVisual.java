...
public class ClasificadorVisual implements Serializable {
	...
	public ClasificadorVisual(NotificacionesRecTrazas notifEventos) {
		...
		// pasamos la lista de elementos trazables a la interfaz grfica para
		// permitir su traza
		panelTrazasNiveles = new PanelTrazasClasificadas(notificador,
				tablaPanelesEspecificos);
		 ...
		 * InfoPanelEspecifico panel = new
		 * InfoPanelEspecifico(listaElementosTrazables.get(i).toString(),"");
		 * //panel.setVisible(false); listaInfoPanelesEspecificos.add(panel); }
		 * 
		 * listaPanelesEspecificos = new LinkedList<PanelTrazasEspecificas>();
		 */
	}

	public void visualizarElementosTrazables(
			List<String> listaElementosTrazables) {
		 ...
		 * while(iterador.hasNext()){ String identEntidad = iterador.next();
	...
	// public void setArrayInfoPaneles(HashMap
	// tabla){this.tablaInfoPanelesEspecificos = tabla;}
	// public void setArrayPaneles(HashMap tabla){this.tablaPanelesEspecificos =
	// tabla;}
	...
	// public HashMap getArrayInfoPaneles(){return
	// this.tablaInfoPanelesEspecificos;}
	...
	public void cerrarVentanas() {
		...
		if (tablaPanelesEspecificos != null) {
			...
			while (iter.hasNext()) {
				...
				PanelTrazasAgteReactivo panel = (PanelTrazasAgteReactivo) tablaPanelesEspecificos
						.getPanelEspecifico(identPanel);
				panel.dispose();
			}
		}
	}

	private InfoPanelesEspecificos construirPanelesEspecificos(
			List<String> listaElementosTrazables) {
		// Se crea una lista de paneles y una tabla con las descripciones de
		// los paneles donde el identificador del panel es el identificador que
		// se pasa en la lista
		// HashMap tablaInfoPaneles = null;
		if (listaElementosTrazables != null) {
			// construimos los paneles especificos para que se pueda trazar en
			// ellos
			...
			while (iterador.hasNext()) {
				...
				InfoPanelEspecifico panel = new InfoPanelEspecifico(
						identEntidad, "");
				panel.setPanelEspecifico(new PanelTrazasAgteReactivo(
						identEntidad, ""));
				// panel.setVisible(false);
	...
	public void muestraTrazaEnviarMensaje(MensajeSimple trazaMensaje) {
		// muestraTraza(traza);
		panelActual = tablaPanelesEspecificos.getPanelEspecifico(trazaMensaje
				.getEmisor().toString());
	...
	public void muestraTrazaEnviarEvento(EventoSimple trazaEvento) {
		// muestraTraza(traza);
		panelActual = tablaPanelesEspecificos.getPanelEspecifico(trazaEvento
				.getOrigen().toString());
		// panelActual.muestraMensaje(traza);
		// Se debe mostrar en el panel del agente emisor del mensaje
	...
	public void muestraTrazaMensajeEvento(String id, EventoSimple ev) {
		// muestraTraza(traza);
		panelActual = tablaPanelesEspecificos.getPanelEspecifico(id);
		// panelActual.muestraMensaje(traza);
	...
	public void muestraTrazaActivacionReglas(String entityId, String infoAtrazar) {
		// muestraTraza(traza);
		panelActual = tablaPanelesEspecificos.getPanelEspecifico(entityId);
		// panelActual.muestraMensaje(traza);
	...
	public synchronized void muestraTraza(InfoTraza traza) {
		...
		InfoPanelEspecifico panelInfoActual = tablaPanelesEspecificos
				.getInfoPanel(identElementTraza);
		if (panelInfoActual != null) {
			// panelInfoActual.setContenido(panelInfoActual.getContenido()
			// + getMensajeNormalizado(traza) + "\n");
			panelActual = tablaPanelesEspecificos
					.getPanelEspecifico(identElementTraza);
			// panelActual.muestraMensaje(traza);
		}
		/*
		 * if (tablaInfoPanelesEspecificos.containsKey(identElementTraza)){
		 * InfoPanelEspecifico panelInfoActual = (InfoPanelEspecifico)
		 * tablaInfoPanelesEspecificos.get(identElementTraza);
		 * 
		 * panelInfoActual.setContenido(panelInfoActual.getContenido() +
		 * getMensajeNormalizado(traza) + "\n"); PanelTrazasEspecificas
		 * panelActual =
		 * (PanelTrazasEspecificas)tablaPanelesEspecificos.get(identElementTraza
		 * ); panelActual.muestraMensaje(traza); }
		 */
		else {
			// creamos el info panel y visualizamos la ventana

			// InfoPanelEspecifico infoPanel = new
			// InfoPanelEspecifico(identElementTraza,"");
			// PanelTrazasEspecificas panelEsp = new
			// PanelTrazasEspecificas(identElementTraza, "");
	...
	private PanelTrazasAbstracto crearPanelparaEntidad(String entityId) {
		...
		try {
			...
			if ((itfConfig == null)
					&& (NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ != null)) {
				itfConfig = (ItfUsoConfiguracion) NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
						.obtenerInterfaz("Itf_Uso_Configuracion");
			}
		} catch (Exception ex) {
			Logger.getLogger(ClasificadorVisual.class.getName()).log(
		...
		try {
			if (itfConfig == null) {
				...
				DescInstancia descComponente = itfConfig
						.getDescInstancia(entityId);
				if (descComponente == null) {
					nuevoPanel = new PanelTrazasGenerico(entityId, "");
				} else if (descComponente.getCategoriaComponente().equals(
						NombresPredefinidos.NOMBRE_ENTIDAD_AGENTE)) {
					if (descComponente.getTipoComponente().equalsIgnoreCase(
							"Cognitivo")) {
						...
						NombresPredefinidos.NOMBRE_ENTIDAD_RECURSO)) {
	...
	public void cierraVentanaEspecifica(String nombreVentana) {
		...
		InfoPanelEspecifico panelActual = tablaPanelesEspecificos
				.getInfoPanel(nombreVentana);
		if (panelActual != null) {
			panelActual.getPanelEspecifico().setVisible(false);
			// if (tablaPanelesEspecificos.containsKey(nombreVentana)){
			// PanelTrazasEspecificas panelActual =
			// (PanelTrazasEspecificas)tablaPanelesEspecificos.get(nombreVentana);
