...
public class VisualizacionTrazasContr {
	...
	public synchronized void visualizarNuevaTraza(InfoTraza traza) {
		...
		if (activacionPanelTrazas) {
			...
			panelActual = this.getpanelParaVisualizar(tipoEntidadEmisora,
					idEntidad);
	...
	private PanelTrazasAbstracto getpanelParaVisualizar(
			NombresPredefinidos.TipoEntidad tipoEnti, String idEntidad) {
		if (!idEntidad.equals(identUltimaEntidad)) {
			panelActual = (PanelTrazasAbstracto) tablaPanelesEspecificos
					.get(idEntidad);
			identUltimaEntidad = idEntidad;
			if (panelActual == null) {
				// if
				// (tipoEnti.equals(NombresPredefinidos.TipoEntidad.noDefinido))
				// panelActual = new PanelTrazasGenerico(idEntidad, "");
				// else if
				// (tipoEnti.equals(NombresPredefinidos.TipoEntidad.Reactivo
				// ))panelActual = new PanelTrazasAgteReactivo(idEntidad, "");
				// else
				// if(tipoEnti.equals(NombresPredefinidos.TipoEntidad.Reactivo))
				// panelActual =new PanelTrazasAgteCognitivo(idEntidad, "");
				// tablaPanelesEspecificos.put(idEntidad, panelActual);
				panelActual = this.infoPanels.crearPanelparaEntidad(idEntidad,
						tipoEnti);
	...
	public synchronized void visualizarEnvioEvento(
	...
	public synchronized void visualizarEnvioMensaje(
			NombresPredefinidos.TipoEntidad tipoEnt, MensajeSimple trazaMensaje) {
		String identEntidad = trazaMensaje.getEmisor().toString();
		panelActual = this.getpanelParaVisualizar(tipoEnt, identEntidad);
		panelActual.muestraMensajeEnviado(trazaMensaje);
	}

	public synchronized void visualizarRecibirMensaje(
			NombresPredefinidos.TipoEntidad tipoEnt, MensajeSimple trazaMensaje) {
	...
	public synchronized void visualizarRecibirEvento(
			NombresPredefinidos.TipoEntidad tipoEnt, EventoSimple trazaEvento) {
		String identEntidad = trazaEvento.getOrigen();
		panelActual = this.getpanelParaVisualizar(tipoEnt, identEntidad);
	...
	public synchronized void visualizarTrazaEjecReglas(String entityId,
			String infoAtrazar) {
		panelActual = this.getpanelParaVisualizar(
				NombresPredefinidos.TipoEntidad.Cognitivo, entityId);
		panelActual.muestraTrazaEjecucionReglas(infoAtrazar);
	}

	public synchronized void visualizarTrazaActivacionReglas(String entityId,
			String infoAtrazar) {
		panelActual = this.getpanelParaVisualizar(
				NombresPredefinidos.TipoEntidad.Cognitivo, entityId);
	...
	public void visualizarIdentFicheroDescrOrganizacion() {
		panelTrazasNiveles
				.visualizarInfoGeneral("Fichero Descripcion organizacion : "
						+ NombresPredefinidos.DESCRIPCION_XML_POR_DEFECTO);
	}

	public synchronized void pedirConfirmacionTerminacionAlUsuario() {
		int res = JOptionPane.showConfirmDialog(null, "Confirmar terminacion",
				"Confirmar terminacion", JOptionPane.YES_NO_OPTION,
		...
		case JOptionPane.YES_OPTION: {
			try {
				...
				// itfUsoAgente.aceptaEvento(new
				// EventoRecAgte("terminacion_confirmada",NombresPredefinidos.RECURSO_TRAZAS,NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
				notificador.confirmarTerminacionOrganizacion();
			} catch (Exception e) {
				System.out
						.println("Ha habido un error al enviar el evento terminacion_confirmada al gestor de organizacin");
				e.printStackTrace();
		...
		case JOptionPane.NO_OPTION: {
			try {
				...
				// .obtenerInterfaz(NombresPredefinidos.ITF_USO+NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION);
				// itfUsoAgente.aceptaEvento(new
				// EventoRecAgte("terminacion_anulada",NombresPredefinidos.RECURSO_TRAZAS,NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));
				notificador.anularTerminacionOrganizacion();
			} catch (Exception e) {
				System.out
						.println("Ha habido un error al enviar el evento terminacion_anulada al gestor de organizacin");
				e.printStackTrace();
				// this.itfAutomata.transita("error");
			}
		}
