...
public class PanelTrazasClasificadas extends javax.swing.JFrame {
	...
	public PanelTrazasClasificadas(NotificacionesRecTrazas notif,
			InfoPanelesEspecificos infoPaneles) {
		initComponents();
	...
	/*
	 * public void visualizar_componentes_trazables(List<String>
	 * listaElementosaTrazar) { listaElementosTrazables =
	 * listaElementosaTrazar.toArray();
	...
	public void visualizarElementoTrazable(String elemento) {
		...
		if (!listaElementosTrazables.contains(elemento)) {
			...
			this.listaComponentes
					.setListData(listaElementosTrazables.toArray());
		}
	...
	public void muestraMensaje(InfoTraza traza) {
		...
		// En el panel principal solo mostramos la informacion de las entidades
		// que envian informacion

		Boolean identEntidadIgualAnterior = identEntidadEmisora
				.equals(ultimaEntidadEmisora);

		/*
		 * if (traza.getNivel() == InfoTraza.NivelTraza.debug ){ if (
		 * !identEntidadIgualAnterior ) { nivel = "DEBUG"; c = Color.LIGHT_GRAY;
		 * 
		 * areaDebugMensaje.setFont(f); areaDebugMensaje.setForeground(c);
		 * //areaDebugMensaje
		 * .append(traza.getEntidadEmisora()+"\t"+traza.getMensaje()+"\n");
		 * areaInfoMensaje.append(traza.getEntidadEmisora()+"\t"+
		 * "Nueva informacion: ver detalles en ventana especifica"+"\n");
		 * 
		 * } else if (traza.getNivel() == InfoTraza.NivelTraza.info){ if (
		 * !identEntidadIgualAnterior ) { nivel = "INFO"; c = Color.BLUE;
		 * areaInfoMensaje.setFont(f); areaInfoMensaje.setForeground(c);
		 * //areaInfoMensaje
		 * .append(traza.getEntidadEmisora()+"\t"+traza.getMensaje()+"\n"); //
		 * areaInfoMensaje.append(traza.getEntidadEmisora()+"\t"+
		 * "Nueva informacion: ver detalles en ventana especifica"+"\n");
		...
		if (traza.getNivel() == InfoTraza.NivelTraza.error) {
			...
			areaErrorMensaje.append(traza.getEntidadEmisora() + "\t"
					+ traza.getMensaje() + "\n");
			areaGeneralMensaje.append(traza.getEntidadEmisora() + "\t"
					+ traza.getMensaje() + "\n");
		 ...
		 * nivel = "FATAL"; c = Color.DARK_GRAY; areaFatalMensaje.setFont(f);
		 * areaFatalMensaje.setForeground(c);
		 * areaFatalMensaje.append(traza.getEntidadEmisora
		 * ()+"\t"+traza.getMensaje()+"\n"); //
		 * areaGeneralMensaje.append(traza.getEntidadEmisora
		 ...
		 * c = Color.BLACK;
		 * 
		 * areaGeneralMensaje.setFont(f); areaGeneralMensaje.setForeground(c);
	...
	private void initComponents() {
		...
		areaGeneralMensaje.setCursor(new java.awt.Cursor(
	...
	private void listaComponentesMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_listaComponentesMouseClicked
		if (evt.getClickCount() == 2) {
			...
			PanelTrazasAbstracto panel = infoPanelesEspecifics
					.getPanelEspecifico(listaComponentes.getSelectedValue()
							.toString());
	...
	// java.util.logging.Logger.getLogger(PanelTrazasClasificadas1.class.getName()).log(java.util.logging.Level.SEVERE,
	// null, ex);
