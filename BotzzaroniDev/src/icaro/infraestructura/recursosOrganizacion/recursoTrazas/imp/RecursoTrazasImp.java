 ...
 */
public class RecursoTrazasImp extends ClaseGeneradoraRecursoTrazas implements
		Serializable {
	...
	@Override
	public synchronized void trazar(String entidad, String infotraza,
	...
	@Override
	public void visualizarComponentesTrazables(
			List<String> listaElementosaTrazar) {
	...
	public void termina() {
		...
		/*
		 * visual.getPanelPrincipal().dispose(); HashMap tablaPaneles =
		 * visual.getArrayPaneles(); if (tablaPaneles !=null ){ String
		 * identPanel=""; Set conjIdentPanels = tablaPaneles.keySet();
		 * Iterator<String> iter = conjIdentPanels.iterator(); while
		 * (iter.hasNext()) { identPanel = iter.next(); PanelTrazasEspecificas
		 * panel = (PanelTrazasEspecificas) tablaPaneles.get(identPanel);
		 * panel.dispose(); } } // for (int i=0;i<listaPaneles.size();i++){ //
	...
	@Override
	public synchronized void aceptaNuevaTrazaEnviarMensaje(
			MensajeSimple trazaMensaje) {
		try {
			if (activacionPanelTrazas) {
				// muestraTraza(traza);
				controlVisulizacionTrazas.visualizarEnvioMensaje(
						NombresPredefinidos.TipoEntidad.noDefinido,
						trazaMensaje);
			}
		} catch (Exception e) {
			logger.fatal(
	...
	@Override
	public synchronized void aceptaNuevaTrazaEnviarEvento(
			EventoSimple trazaEvento) {
		try {
			if (activacionPanelTrazas) {
				// muestraTraza(traza);
				controlVisulizacionTrazas
						.visualizarEnvioEvento(
								NombresPredefinidos.TipoEntidad.noDefinido,
								trazaEvento);
			}
		} catch (Exception e) {
	...
	public synchronized void trazarEnviarEvento(
			NombresPredefinidos.TipoEntidad tipoEnt, EventoSimple trazaEvento) {
		try {
			if (activacionPanelTrazas) {
				// muestraTraza(traza);
				controlVisulizacionTrazas.visualizarEnvioEvento(tipoEnt,
						trazaEvento);
	...
	@Override
	public synchronized void aceptaNuevaTrazaMensajeRecibido(
			MensajeSimple trazaMensaje) {
		try {
			if (activacionPanelTrazas) {
				// muestraTraza(traza);
				controlVisulizacionTrazas.visualizarRecibirMensaje(
						NombresPredefinidos.TipoEntidad.noDefinido,
						trazaMensaje);
				/*
				 * panelActual =
				 * (PanelTrazasAbstracto)tablaPanelesEspecificos.getPanelEspecifico
				 * (trazaMensaje.getReceptor().toString()); if (panelActual ==
				 * null){ String identElementTraza =
				 * trazaMensaje.getEmisor().toString(); panelActual =
				 * tablaPanelesEspecificos.crearPanelparaEntidad
				 * (identElementTraza);
				 * panelTrazasNiveles.visualizarElementoTrazable(
				 * identElementTraza); }
	...
	@Override
	public synchronized void aceptaNuevaTrazaEventoRecibido(String entityId,
			EventoSimple trazaEvento) {
		try {
			if (activacionPanelTrazas) {
				controlVisulizacionTrazas
						.visualizarRecibirEvento(
								NombresPredefinidos.TipoEntidad.noDefinido,
								trazaEvento);
				 ...
				 * panelActual =
				 * (PanelTrazasAbstracto)tablaPanelesEspecificos.getPanelEspecifico
				 * (entityId); if (panelActual == null){
				 * 
				 * panelActual = tablaPanelesEspecificos.crearPanelparaEntidad
				 * (entityId); panelTrazasNiveles.visualizarElementoTrazable(
				 * entityId); } panelActual.muestraEventoEnviado(trazaEvento);
				 */
			}
		} catch (Exception e) {
			logger.fatal(
	...
	public synchronized void trazaEventoRecibido(String entityId,
			NombresPredefinidos.TipoEntidad tipoEnt, EventoSimple trazaEvento) {
		try {
			if (activacionPanelTrazas) {
				controlVisulizacionTrazas
				 ...
				 * panelActual =
				 * (PanelTrazasAbstracto)tablaPanelesEspecificos.getPanelEspecifico
				 * (entityId); if (panelActual == null){ String
				 * identElementTraza = trazaEvento.getOrigen(); panelActual =
				 * tablaPanelesEspecificos.crearPanelparaEntidad
				 * (identElementTraza,tipoEnt.name());
				 * panelTrazasNiveles.visualizarElementoTrazable(
				 * identElementTraza); }
	...
	@Override
	public synchronized void aceptaNuevaTrazaEjecReglas(String entityId,
			String infoAtrazar) {
		try {
			if (activacionPanelTrazas) {
				controlVisulizacionTrazas.visualizarTrazaEjecReglas(entityId,
						infoAtrazar);
				// muestraTraza(traza);
				/*
				 * panelActual =
				 ...
				 * identElementTraza = trazaEvento.getOrigen(); panelActual =
				 * tablaPanelesEspecificos.crearPanelparaEntidad
				 * (entityId,TipoEntidad.Cognitivo.name());
	...
	@Override
	public synchronized void aceptaNuevaTrazaActivReglas(String entityId,
			String infoAtrazar) {
		try {
			if (activacionPanelTrazas) {

				controlVisulizacionTrazas.visualizarTrazaActivacionReglas(
						entityId, infoAtrazar);
				/*
				 * panelActual =
				 * (PanelTrazasAbstracto)tablaPanelesEspecificos.getPanelEspecifico
				 * (entityId); if (panelActual == null){ // String
