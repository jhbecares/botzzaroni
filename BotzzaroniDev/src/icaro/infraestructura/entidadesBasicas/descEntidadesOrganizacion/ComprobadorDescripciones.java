...
import icaro.infraestructura.entidadesBasicas.descEntidadesOrganizacion.jaxb.DescripcionComponentes;
...
public class ComprobadorDescripciones {
	...
	public ComprobadorDescripciones() {
		 ...
		 * NombresPredefinidos.RECURSO_TRAZAS); InfoTraza traza = new
		 * InfoTraza(NombresPredefinidos.CONFIGURACION,
		 * "Comprobando descripciones de los comportamientos...",
		 * NivelTraza.info); trazas.aceptaNuevaTraza(traza);
		 * 
		 * 
		 * 
		 * } catch (Exception e) {
	...
	public boolean comprobar(DescOrganizacion descOrganizacion)
			throws Exception {
		...
		// Verificamos que existe un gestor del NODO que debe ser un gestor de
		// organización o un Gestor de Nodo
		// NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION
		String identGestorNodo = descOrganizacion.getDescInstancias()
				.getGestores().getInstanciaGestor().get(0).getId();
		comprobarExistenciaGestorNodo(identGestorNodo);
		// if
		// (!(identGestorNodo.startsWith(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION))||!(identGestorNodo.startsWith(NombresPredefinidos.NOMBRE_GESTOR_NODO))
		// ){
	...
	public void comprobarExistenciaGestorNodo(String identGestorNodo) {
		...
		if (!(identGestorNodo
				.startsWith(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION))
				&& (!(identGestorNodo
						.startsWith(NombresPredefinidos.NOMBRE_GESTOR_NODO)))) {
			logger.fatal("Error al verificar los nombres predefinidos del GO y del GN. Compruebe que exista,un GO o un GN  en la descripcion de la organizacion,"
					+ " y que los nombres asignados comiencen por GestroOrganizacion o GestorNodo");
			errores.add(new InfoTraza(
	...
	public DescComportamientoAgente comprobarGestor(
			DescComportamientoAgente agente) {
		...
		if (comportamientoEspecificado == null) {
			agente.setLocalizacionComportamiento(comportamientoPorDefecto);
		}
		if (tipo == TipoAgente.REACTIVO) {
			agente = comprobarAgenteReactivo(agente);
		} else if ((tipo == TipoAgente.COGNITIVO) || (tipo == TipoAgente.ADO)
				|| (tipo == TipoAgente.DIRIGIDO_POR_OBJETIVOS)) {
			agente = comprobarAgenteADO(agente);
		}
		return agente;
	}

	public DescComportamientoAgente comprobarAgenteAplicacion(
			DescComportamientoAgente agente) {
		agente.getNombreComportamiento();
		TipoAgente tipo = agente.getTipo();
		agente.getRol();
		if (tipo == TipoAgente.REACTIVO) {
	...
	// public boolean comprobarAgenteReactivo(String rutaComportamiento,String
	// nombreAgente, String rolAgente) {
	public DescComportamientoAgente comprobarAgenteReactivo(
			DescComportamientoAgente comptAgente) {
		String rutaDirectorioCompt = comptAgente
				.getLocalizacionComportamiento();
		String rutaClaseAcciones = comptAgente.getLocalizacionClaseAcciones();
		String rutaFicheroAutomata = comptAgente
		...
		rutaClaseAcciones = verificarRutaClasesAccion(rutaClaseAcciones,
				rutaDirectorioCompt);
		rutaFicheroAutomata = validarRutaEntidadComportamiento(
				rutaFicheroAutomata, rutaDirectorioCompt, identComportmto);
	...
	public DescComportamientoAgente comprobarAgenteADO(
			DescComportamientoAgente comptAgente) {
		...
		if (rutaFicheroReglas == null) {// se valida la ruta y luego que exista
			...
			rutaFicheroReglas = rutaDirectorioCompt + "."
					+ NombresPredefinidos.CARPETA_REGLAS + "."
	...
	public boolean validarRutaReglasAgteADO(String rutaEntidad,
			String identDescComptoEntidad) {
		boolean ficheroValido = false;
		String msgInfoUsuario = null;
		String rutaDirectorioCompt = NombresPredefinidos.RUTA_AGENTES_APLICACION
				+ "."
		...
		if (!rutaEntidad.startsWith(rutaDirectorioCompt)) {
			...
			InfoTraza traza = new InfoTraza(NombresPredefinidos.CONFIGURACION,
					msgInfoUsuario, NivelTraza.error);
		...
		if (!rutaEntidad.endsWith(".drl")) {
			rutaEntidad = rutaEntidad + ".drl";
		}
		if (!verificarNombradoEntidad(rutaEntidad,
				NombresPredefinidos.NOMBRE_FICHERO_PDFTO_REGLAS)) {
			msgInfoUsuario = "Error en el nombre del fichero del Reglas. \n"
					+ "El nombre del comportamiento es : "
					+ identDescComptoEntidad
					+ ": "
					+ "En la ruta: "
					+ rutaEntidad
	...
	public String validarRutaClaseEntidad(String rutaEntidadEspecificada,
			String rutaEntidadPorDfto, String identDescComptoEntidad) {
		String msgInfoUsuario = null;
		// String rutaValidada = rutaEntidadEspecificada;
		// se construye la ruta por defecto y se valida que la ruta especificada
		// cumple con el patron de ruta
		...
		if (posicion < 0) {
			posicion = rutaEntidadEspecificada.indexOf(".java");
		}
		if (posicion >= 0) {
			rutaEntidadEspecificada = rutaEntidadEspecificada.substring(0,
					posicion);
		}
	...
	/*
	 * public boolean validarRutaClaseGeneradoraRecursos (String rutaEntidad,
	 * String identDescComptoEntidad){ boolean especCorrecta=true; String
	 * msgInfoUsuario = null; // se construye la ruta por defecto y se valida
	 * que la ruta especificada cumple con el patron de ruta String
	 * rutaclaseGeneradoraPorDefecto =
	 * NombresPredefinidos.RUTA_RECURSOS_APLICACION + "." +
	 * primeraMinuscula(identDescComptoEntidad) + ".imp." +
	 * NombresPredefinidos.PREFIJO_CLASE_GENERADORA_RECURSO +
	 * identDescComptoEntidad; if (rutaEntidad == null) rutaEntidad =
	 * rutaclaseGeneradoraPorDefecto; else // se valida que cumple el patron de
	 * ruta if (!rutaEntidad.startsWith(rutaclaseGeneradoraPorDefecto)){
	 * msgInfoUsuario =
	 * "Error en la especificacion de la clase generadora del recurso :  " +
	 * identDescComptoEntidad + "\n  La ruta especificada es :  " + rutaEntidad
	 * + " \n La ruta debe comenzar por " + rutaclaseGeneradoraPorDefecto +
	 * ". Compruebe la ruta definida  en la descripcion de la organizacion.";
	 * InfoTraza traza = new
	 * InfoTraza(NombresPredefinidos.CONFIGURACION,msgInfoUsuario,
	 * NivelTraza.error); ItfUsoRecTrazas.aceptaNuevaTraza(traza);
	 * logger.fatal(msgInfoUsuario); especCorrecta=false; } if
	 * (!validarRutaClaseEntidad(rutaEntidad, rutaEntidad,
	 * identDescComptoEntidad)){ msgInfoUsuario =
	 * "Error no se encuentra la  clase especificada \n"+
	 * "Para el comportamiento:" + identDescComptoEntidad + "En la ruta: " +
	 * rutaEntidad + "\n" +
	 * "Verifique la existencia del fichero en el directorio src \n"; InfoTraza
	 * traza = new InfoTraza(NombresPredefinidos.CONFIGURACION,msgInfoUsuario,
	 * NivelTraza.error); ItfUsoRecTrazas.aceptaNuevaTraza(traza);
	 * logger.fatal(msgInfoUsuario); }else if (especCorrecta) return true;
	 * return false; }
	 */
	public String validarRutaEntidadComportamiento(String rutaEntidad,
			String rutaPdftoDirEntidad, String identDescComptoEntidad) {
		// se valida la ruta del directorio
		Boolean rutaValida = false, ficheroValido = false;
		...
		if (rutaEntidad.lastIndexOf(extensionFicheroRuta) < 0) {
			rutaEntidad = rutaEntidad + extensionFicheroRuta;
		}

		if (esCompAgteReactivo) {
			if (!verificarNombradoEntidad(rutaEntidad,
					NombresPredefinidos.NOMBRE_FICHERO_PDFTO_AUTOMATA)) {
		...
		if (rutaValida && ficheroValido) { // se valida la existencia del
			...
			InputStream input = this.getClass()
					.getResourceAsStream(rutaEntidad);
			logger.debug(rutaEntidad + "?"
					+ ((input != null) ? "  OK" : "  null"));
			if (input != null) {
				return rutaEntidad;
			} else {
				...
				InfoTraza traza = new InfoTraza(
						NombresPredefinidos.CONFIGURACION, msgInfoUsuario,
						NivelTraza.error);
				// ItfUsoRecTrazas.aceptaNuevaTraza(traza);
				errores.add(traza);
				logger.fatal(msgInfoUsuario);
	...
	public boolean verificarNombradoEntidad(String rutaEntidad,
			String nombreFicheroPorDefecto) {
		String msgInfoUsuario;
		boolean extensionCorrecta = false;
		Integer posicionPunto = nombreFicheroPorDefecto.indexOf(".");
		String extensionFicheroPD = nombreFicheroPorDefecto
				.substring(posicionPunto);
		String nombreFicheroPD = nombreFicheroPorDefecto.substring(0,
				posicionPunto);
		...
		nombreficheroEnlaRuta = nombreficheroEnlaRuta
				.substring(nombreficheroEnlaRuta.lastIndexOf(".") + 1);
		if (!extensionFicheroPD.equals(extensionFicheroRuta)) {
			msgInfoUsuario = "Error extension del fichero incorrecta \n "
					+ " La extension del  fichero especificado  debe ser :   "
					+ extensionFicheroPD
					+ "  \n Fichero especificado :"
					+ nombreficheroEnlaRuta
					+ ": "
					+ "En la ruta:"
					+ rutaEntidad
					+ "\n"
		...
		if (!nombreficheroEnlaRuta.startsWith(nombreFicheroPD)) {
			msgInfoUsuario = "Error nombre de fichero incorrecto /n "
					+ " El  fichero especificado  debe comenzar por :   "
					+ nombreFicheroPD
					+ "fichero especificado :"
					+ nombreficheroEnlaRuta
					+ ": "
	...
	public DescRecursoAplicacion comprobarRecursoAplicacion(
		...
		String rutaClaseGeneradoraEspecificada = recursoAplicacion
				.getLocalizacionClaseGeneradora();
		if (rutaClaseGeneradoraEspecificada == null) {
			rutaclaseGeneradoraPorDefecto = NombresPredefinidos.RUTA_RECURSOS_APLICACION
					+ "."
					+ primeraMinuscula(nombreRecurso)
					+ ".imp."
					+ NombresPredefinidos.PREFIJO_CLASE_GENERADORA_RECURSO
					+ nombreRecurso;
			...
			rutaclaseGeneradoraPorDefecto = rutaClaseGeneradoraEspecificada;
		}
		recursoAplicacion
				.setLocalizacionClaseGeneradora(validarRutaClaseEntidad(
						rutaClaseGeneradoraEspecificada,
						rutaclaseGeneradoraPorDefecto, nombreRecurso));
	...
	public boolean existeClase(String rutaClase) {
		...
		if (posicion < 0) {
			posicion = rutaClase.indexOf(".java");
		}
		if (posicion >= 0) {
	...
	/*
	 * public boolean comprobarRecursoAplicacion1( DescRecursoAplicacion
	 * recursoAplicacion) { boolean encontrada = false; String nombreRecurso =
	 * recursoAplicacion.getNombre(); String claseGeneradoraPorDefecto =
	 * NombresPredefinidos.RUTA_RECURSOS_APLICACION + "." +
	 * primeraMinuscula(nombreRecurso) + ".imp." +
	 * NombresPredefinidos.PREFIJO_CLASE_GENERADORA_RECURSO + nombreRecurso;
	 * String claseGeneradoraEspecificada =
	 * recursoAplicacion.getLocalizacionClaseGeneradora(); if
	 * (claseGeneradoraEspecificada != null) { if
	 * (existeClase(claseGeneradoraEspecificada)) { encontrada = true;
	 * 
	 * logger.debug(NombresPredefinidos.CONFIGURACION +
	 * ": Clase generadora leida para el recurso " + nombreRecurso + ": " +
	 * claseGeneradoraEspecificada); } else { encontrada = false; InfoTraza
	 * traza = new InfoTraza( NombresPredefinidos.CONFIGURACION,
	 * "Error al leer la clase generadora " + claseGeneradoraEspecificada +
	 * " del recurso" + nombreRecurso +
	 * ". Compruebe que la clase existe o especifique otra localizacion del comportamiento del recurso"
	 * +nombreRecurso + " en la descripcion de la organizacion.",
	 * NivelTraza.fatal); logger .fatal(NombresPredefinidos.CONFIGURACION +
	 * ": Error al leer la clase generadora " + claseGeneradoraEspecificada +
	 * " del recurso" + nombreRecurso +
	 * ". Compruebe que la clase existe o especifique otra localizacion del comportamiento del recurso"
	 * +nombreRecurso + " en la descripcion de la organizacion.");
	 * ItfUsoRecTrazas.aceptaNuevaTraza(traza); errores.add(traza); } } if
	 * (claseGeneradoraEspecificada == null || !encontrada) { if
	 * (existeClase(claseGeneradoraPorDefecto)) { encontrada = true;
	 * recursoAplicacion
	 * .setLocalizacionClaseGeneradora(claseGeneradoraPorDefecto);
	 * 
	 * logger.debug(NombresPredefinidos.CONFIGURACION +
	 * ": Clase generadora leida para el recurso " + nombreRecurso + ": " +
	 * claseGeneradoraPorDefecto); } else { encontrada = false; InfoTraza traza
	 * = new InfoTraza( NombresPredefinidos.CONFIGURACION,
	 * "Error al leer la clase generadora " + claseGeneradoraPorDefecto +
	 * "del recurso" + nombreRecurso +
	 * ". Compruebe que la clase existe o especifique la localizacion del comportamiento del recurso"
	 * +nombreRecurso + "en la descripcion de la organizacion.",
	 * NivelTraza.fatal); logger .fatal(NombresPredefinidos.CONFIGURACION +
	 * ": Error al leer la clase generadora " + claseGeneradoraPorDefecto +
	 * "del recurso" + nombreRecurso +
	 * ". Compruebe que la clase existe o especifique la localizacion del comportamiento del recurso"
	 * +nombreRecurso + "en la descripcion de la organizacion.");
	 * ItfUsoRecTrazas.aceptaNuevaTraza(traza); errores.add(traza); } }
	 * 
	 * return encontrada; }
	 * 
	 * 
	 ...
	 * recursoClassPath); logger.debug(recursoClassPath+"?"+ ((input != null) ?
	 * "  OK" : "  null")); return (input != null); }
	 * 
	 * 
	 * 
	 * public Collection<File> lookupClasspathFiles(String name) {
	 * Enumeration<URL> resources; List<File> files = new ArrayList<File>();
	 * 
	 * try { resources =
	 * ComprobadorDescripciones.class.getClassLoader().getResources(name);
	 * 
	 * while (resources.hasMoreElements()) { URI uri =
	 * resources.nextElement().toURI(); File file = new File(uri);
	 * files.add(file); }
	 * 
	 * } catch (Exception e) { throw new
	 * RuntimeException("could not lookup files with name: "+name, e); }
	 * 
	 * return files; } private Class obtenerClaseAccionesSemanticas(String
	 ...
	 * 
	 * Boolean ClaseASEncontrada=false; String nombreClase= ""; for (int i=0;
	 ...
	 * +rutaComportamiento,"Factoria del Agente Reactivo",
	 * "Class obtenerClaseAccionesSemanticas(DescInstanciaAgente instAgente)" );
	 * } private boolean validarAutomatayAccionesSemanticas(String
	 * rutaComportamiento)throws ExcepcionEnComponente { // La ruta del
	 * comportamiento no incluye la clase // Obtenemos la clase de AS en la ruta
	 * String rutaClases = rutaComportamiento; rutaComportamiento =
	 * NombresPredefinidos
	 ...
	 * clase de acciones semanticas en el array con los ficheros
	 * 
	 * Boolean claseASEncontrada=false; Boolean ficheroAutomataEncontrado=false;
	 * String nombreFichero= ""; for (int i=0;
	 * (i<ficherosRuta.length)&(!claseASEncontrada|!ficheroAutomataEncontrado);
	 * i++){ nombreFichero = ficherosRuta[ i ]; if
	 * (nombreFichero.startsWith(NombresPredefinidos
	 * .PREFIJO_CLASE_ACCIONES_SEMANTICAS )&
	 * (nombreFichero.lastIndexOf(".java")!= 0) ){ claseASEncontrada = true;
	 * }else if (nombreFichero.startsWith(NombresPredefinidos.PREFIJO_AUTOMATA
	 * )& (nombreFichero.lastIndexOf(".xml")!= 0) ){ ficheroAutomataEncontrado =
	 * true; } } if (claseASEncontrada && ficheroAutomataEncontrado)return true;
	 * else { return false; // throw new ExcepcionEnComponente (
	 * "PatronAgenteReactivo",
	 * "No se encuentra la clase de acciones semanticas en la ruta :"
	 * +rutaComportamiento,"Factoria del Agente Reactivo",
	 * "Class obtenerClaseAccionesSemanticas(DescInstanciaAgente instAgente)" );
	 * // return false; } }
	 */
	private String verificarRutaClasesAccion(String rutaClaseAS,
