...
public class ComunicacionAgentes {
	...
	public ComunicacionAgentes(String propietario) {
		...
		itfsAgtesConLosQComunico = new IdentityHashMap(
	...
	public boolean existItfAgte(String nombreAgente) {
		try {
			...
			System.err.printf("Error al buscar interfaz del agente: [%s]",
					nombreAgente);
			e.printStackTrace();
			return false;
		}
	...
	public synchronized boolean enviarInfoAotroAgente(Object infoAEnviar,
			String identAgteReceptor) {
		try {
			if (existItfAgte(identAgteReceptor)) {
				MensajeSimple mensajeAenviar = new MensajeSimple(infoAEnviar,
						agentePropietario, identAgteReceptor);
				itfUsoAgente.aceptaMensaje(mensajeAenviar);
				// trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
				// "Se envia  la informacion : " +infoAEnviar +" Al agente: " +
				// identAgteReceptor ,
				// InfoTraza.NivelTraza.debug));
				trazas.aceptaNuevaTrazaEnviarMensaje(mensajeAenviar);
				return true;
			} else {
				logger.error("El agente: " + agentePropietario
						+ "-- no pudo   enviar la informacion : " + infoAEnviar
						+ " Al agente: " + identAgteReceptor
						+ ".No se encontro la interfaz del agente Receptor");
			...
			logger.error("El agente: "
					+ agentePropietario
					+ "-- no pudo   enviar la informacion : "
					+ infoAEnviar
					+ " Al agente: "
					+ identAgteReceptor
					+ ".Hubo un error en la busqueda de la interfaz del agente Receptor");
			trazas.aceptaNuevaTraza(new InfoTraza(
					agentePropietario,
					"No se pudo enviar la informacion : "
							+ infoAEnviar
	...
	public synchronized boolean enviarInfoConMomentoCreacionAotroAgente(
			Object infoAEnviar, String identAgteReceptor) {

		return (enviarMsgaOtroAgente(new MensajeSimpleConInfoTemporal(
				infoAEnviar, agentePropietario, identAgteReceptor)));
	}

	public synchronized boolean enviarMsgaOtroAgente(
			MensajeSimple mensajeAenviar) {
		try {
			if (existItfAgte((String) mensajeAenviar.getReceptor())) {
				...
				logger.error("El agente: " + agentePropietario
						+ "-- no pudo   enviar el mensaje  : " + mensajeAenviar
						+ ".No se encontro la interfaz del agente Receptor");
				trazas.aceptaNuevaTraza(new InfoTraza(
						agentePropietario,
						"No se pudo enviar la informacion : "
	...
	public synchronized void informaraOtroAgenteReactivo(
			InfoContEvtMsgAgteReactivo infoAEnviar, String identAgenteReceptor,
			ItfUsoAgenteReactivo itfUsoAgenteReceptor) {
		...
		try {
			if (itfUsoAgenteReceptor != null) {
				MensajeSimple mensajeAenviar = new MensajeSimple(infoAEnviar,
						agentePropietario, identAgenteReceptor);
				itfUsoAgenteReceptor.aceptaMensaje(mensajeAenviar);
			} else {

				logger.error("El agente: " + agentePropietario
						+ "-- no puedo   enviar la informacion : "
						+ infoAEnviar + " Al agente: " + identAgenteReceptor
						+ " La interfaz del agente Receptor es null ");
				trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
	...
	public synchronized void informaraGrupoAgentes(Object infoAEnviar,
			ArrayList<String> identsAgteReceptores) {
		enviarMsgaGrupoAgentes(new MensajeSimple(infoAEnviar,
				this.agentePropietario, null), identsAgteReceptores);
	}

	public synchronized void informarConMomentoCreacionaGrupoAgentes(
			Object infoAEnviar, ArrayList<String> identsAgteReceptores) {
		enviarMsgaGrupoAgentes(new MensajeSimpleConInfoTemporal(infoAEnviar,
				this.agentePropietario, null), identsAgteReceptores);
	}
	...
	public synchronized void enviarMsgaGrupoAgentes(MensajeSimple msgAEnviar,
		...
		try {
			...
			logger.error("El agente: " + agentePropietario
					+ "-- no pudo   enviar el mensaje : " + msgAEnviar);
			trazas.aceptaNuevaTraza(new InfoTraza(agentePropietario,
