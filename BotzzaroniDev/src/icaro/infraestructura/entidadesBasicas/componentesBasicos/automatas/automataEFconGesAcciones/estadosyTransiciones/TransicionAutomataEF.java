...
public class TransicionAutomataEF {
	 ...
	 *            input debe indicar el nombre de la clase a la que pertenece el
	 *            input
	 * @param idEstadoSiguiente
	 * @param idAccion
	 *            Nombre de la acción definida en el automata , puede ser null
	 *            o "vacia"
	 * @param modalidadAccion
	 *            definida en el automata puede ser ( bloqueante o paralela )
	 */
	public TransicionAutomataEF(String input, Class idAccionClass,
			String idEstadoSiguiente, String modalidadAccion) {
		...
		} else if (modalidadAccion
				.equals(NombresPredefinidos.NOMBRE_MODO_CONCURRENTE_AUTOMATA_EF_SIN_ACCION)) {
			this.tipoTransicion = NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_ACCION_THREAD;
		} else {
			this.tipoTransicion = NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_ACCION_BLOQ;
		}
	}

	public TransicionAutomataEF(String input, Class idAccion,
			String idMetodoClase, String idEstadoSiguiente,
			String modalidadAccion) {
		this.identEstadoOrigen = null;
		this.tipoEstadoOrigen = 2;
		this.input = input;
		...
		if (idAccion == null) {
			this.tipoTransicion = NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_SIN_ACCION;
				...
				.equals(NombresPredefinidos.AUTOMATA_EF_NOMBRE_MODO_CONCURRENTE)) {
			this.tipoTransicion = NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_CONCURR;
		} else {
			this.tipoTransicion = NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_BLOQ;
		}
	}
	 ...
	 *
	 * @param tipoEst
	 *            puede ser estado intermedio o estado final . Habría que
	 *            controlar el tipo
	 */
	...
	/**
	 *
	 * @return Los tipos de transición estan definidos como Integer en
	 *         NombresPredefinidos
	 */
