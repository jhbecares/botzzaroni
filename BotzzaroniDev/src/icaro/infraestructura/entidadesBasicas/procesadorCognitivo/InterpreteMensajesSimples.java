...
public class InterpreteMensajesSimples {
	 ...
	 * evento simple o posiblemente en varios eventos. Por ahora solo
	 ...
	 * 
	 * Object contenido = mensaje.getContenido(); emisorMensaje =
	 * mensaje.getSource(); evidencia = new Evidencia ( );
	 * evidencia.setOrigen(emisorMensaje); evidencia.setCreador(propietario); if
	 * ((mensaje.getNamePerformative() == "REQUEST")&
	 * (mensaje.getSource().contains ("gestor"))){ OrdenDeOtroAgente orden =
	 * transformaMsgInfoEnOrden (contenido) ;
	 * 
	 * evidencia.addElementToContent(orden); } else {
	 * evidencia.addElementToContent(contenido); } return evidencia ; }
	 * 
	 * 
	 * public ExtractedInfo extractInfo(MensajeACLSimple mensaje) {
	 * 
	 * Object contenido = mensaje.getContenido(); emisorMensaje =
	 * mensaje.getSource(); inforExtracted = new ExtractedInfo ( );
	 * inforExtracted.setOrigen(emisorMensaje);
	 * inforExtracted.setCreador(propietario); // if
	 * ((mensaje.getNamePerformative() == "REQUEST")&
	 * (mensaje.getSource().contains ("gestor"))){ // OrdenDeOtroAgente orden =
	 * transformaMsgInfoEnOrden (contenido) ;
	 * 
	 * inforExtracted.addElementToContent(orden); } else {
	 * inforExtracted.addElementToContent(contenido); } return inforExtracted ;
	 * }
	 */
	public ExtractedInfo extractInfo(MensajeSimple mensaje) {
		...
		if (mensaje.isContenidoColection()) {
			inforExtracted
					.setContentCollection(mensaje.getColeccionContenido());
		} else {
			inforExtracted.setContenido(contenido);
		}
