	...
	public void ejecutar(Object... params) {
		...
		try {
		    ...
		    if(fechasPedidos.isEmpty()){
			    ...
			    // Aqu� cambiamos el objetivo para que se ejecute directamente la tarea que inserta el pedido y a�
		    }
		    	...
		    	if (fechasPedidos.size() == 1){
		    	    ...
		    	    if( (timeDifInMilliSec / (60 * 1000)) >= 0){
	    				...
	    				// Si puede ser despu�s del pedido porque est� dentro del horario 
		    	        if( (timeDifInMilliSec / (60 * 1000)) >= 0){
		    	        	sdf.setCalendar(ant);
		    				continuar = false;
		    				System.out.println("Puede ser después del pedido" + timeDifInMilliSec);
		    	        }
	    	        }
			    	...
			    	if(continuar){ // Se me acabaron los pedidos, no encontr� nada
			    		// Compruebo si cabe despu�s del �ltimo pedido si no se pod�a en un hueco
				    	GregorianCalendar gcFin = (GregorianCalendar) gc.clone();
						gcFin.set(Calendar.HOUR_OF_DAY, 23);
					    gcFin.set(Calendar.MINUTE, 59);
	    				ant.add(Calendar.MINUTE, 30);
