 ...
 * @author
 */
public class AccionesSemanticasIniciador extends
	...
	public AccionesSemanticasIniciador() {
		...
		try {
			...
			if (trazas == null) {
				...
				itfUsoRepositorio.registrarInterfaz(NombresPredefinidos.ITF_USO
						+ NombresPredefinidos.RECURSO_TRAZAS, trazas);
	...
	public void verificarExistenciaEntidadesDescripcion() {
		...
		// Se verifican las rutas donde deben encontrarse las entidades de
		// descripcion:
		// El esquema de descripcion de la organizacion, el fichero de
		// descripcion y el paquete jaxb
		...
		// this.trazas.trazar(nombreAgente,
		// "Verificando la existencia de entidadesDescripcion",
		// NivelTraza.debug);
		...
		try {
			...
			ItfUsoRecTrazas
					.aceptaNuevaTraza(new InfoTraza(
							"Iniciador",
							tipoEntidad,
							"No se ha enviar un evento a si mismo notificando la  validacion de las rutas de las entidades de descripcion . ",
							InfoTraza.NivelTraza.error));
	...
	public void crearRecursosNucleoOrganizacion() throws Exception {
		...
		// Intento crear los recursos en orden pero si se producen errores se
		// capturan, se visualizan y
		// se pude intentar dialogar con el usuario para que los corrija

		try {
			// Se crea el recurso de configuración y posteriormente se debería
			// dar orden al recurso
			...
			trazas.trazar(this.nombreAgente,
					"Creacion de recursos nucleo Organizacion", NivelTraza.info);
			configuracionExterna = ClaseGeneradoraConfiguracion.instance();
			// Se crea una configuracion con el fichero de descripcion de la
			// organizacion
			rutaDescripcionOrganizacion = NombresPredefinidos.RUTA_DESCRIPCIONES
					+ NombresPredefinidos.DESCRIPCION_XML_POR_DEFECTO + ".xml";
			configuracionExterna
					.interpretarDescripOrganizacion(rutaDescripcionOrganizacion);
			if (!configuracionExterna.validarDescripOrganizacion()) {
				...
				itfUsoRepositorio.registrarInterfaz(NombresPredefinidos.ITF_USO
						...
						configuracionExterna);
				if (NombresPredefinidos.ACTIVACION_PANEL_TRAZAS_DEBUG
						.startsWith("t")) {
				} else {
			...
			System.err
					.println("Error. La organizacion no se ha compilado correctamente. Compruebe que los ficheros xml de los automatas se encuentren en el classpath.");
			ie.printStackTrace();
			System.exit(1);
	...
	public String localizarComportamientoGestorInicial(String identGestor)
			throws ExcepcionEnComponente, RemoteException {

		String locComportamientoGestor = null;
		// Se obtiene el gestor inicial de la descripción de la organizacion.
		// Puede ser un gestor de organizacion o un gestor de nodo
		...
		try {
			...
			DescInstanciaGestor desCompGestor = configuracionExterna
					.getDescInstanciaGestor(identGestor);
			locComportamientoGestor = desCompGestor.getDescComportamiento()
			...
			if (identGestor
				...
				// El gestor inicial debe ser un gestor de organización o un
				// gestor de nodo y no se ha definido ninguno
				ItfUsoRecTrazas
						.aceptaNuevaTraza(new InfoTraza(
								"Iniciador",
			...
			Logger.getLogger(AccionesSemanticasIniciador.class.getName()).log(
	...
	public void crearGestorInicial() throws Exception {
		...
		if (configuracionExterna.despliegueOrgEnUnSoloNodo()) {
			...
			if (thisHost.equalsIgnoreCase(identNodoGO)) {
				identGestorNodo = NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION;
			} else {
		...
		// if (
		// identGestorNodo.startsWith(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION)){
		// // identGestorACrear =
		...
		// } else
		// if (
		// identGestorNodo.startsWith(NombresPredefinidos.NOMBRE_GESTOR_NODO)){
		// // identGestorACrear = NombresPredefinidos.NOMBRE_GESTOR_NODO;
		// locComportamientoGestor =
		// NombresPredefinidos.COMPORTAMIENTO_PORDEFECTO_GESTOR_NODO;
		// }
		// else{
		// // El gestor inicial debe ser un gestor de organización o un gestor
		// de nodo y no se ha definido ninguno
		// ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador",
		// "No se pudo crear el comportamiento del Gestor Inicial porque no hay un Gestor de Organizacion o un Gestor de Nodo definido."
		// + " Revisar el fichero de Descripcion de la  Organizacion",
		// InfoTraza.NivelTraza.error));
		...
		// NombresPredefinidos.NOMBRE_INICIADOR));
		// }
		locComportamientoGestor = this
				.localizarComportamientoGestorInicial(identGestorNodo);

		if (!locComportamientoGestor.isEmpty()) {
			// Se crea el gestor y se registra en el registro RMI en el caso en
			// que la organizacion sea distribuida
			try {
				// FactoriaAgenteReactivo.instancia().crearAgenteReactivo(identGestorNodo
				// ,locComportamientoGestor);
				DescInstanciaGestor desCompGestor = configuracionExterna
						.getDescInstanciaGestor(identGestorNodo);
				// FactoriaAgenteReactivo.instancia().crearAgenteReactivo(desCompGestor);
				FactoriaComponenteIcaro.instanceAgteReactInpObj()
						.crearAgenteReactivo(desCompGestor);
				itfgestGestorInicial = (ItfGestionAgenteReactivo) itfUsoRepositorio
						.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
								+ identGestorNodo);
	...
	public void arrancarGestorInicialyTerminar() throws Exception {
		...
		try {
			...
			// NombresPredefinidos.NOMBRE_INICIADOR));
			ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador",
					"No se ha podido  arrancar  el Gestor de Organizacion . ",
					InfoTraza.NivelTraza.error));
	...
	public void analizarInformeArranque(InformeArranqueGestor informeRecibido) {
		if (informeRecibido.getContenidoInforme().equals(
			...
			try {
				...
				Logger.getLogger(AccionesSemanticasIniciador.class.getName())
		...
		while (enume.hasMoreElements() && !errorEncontrado) {
			...
			try {
				...
				if ((monitoriz == InterfazGestion.ESTADO_ERRONEO_IRRECUPERABLE)
					...
					try {
						ItfUsoRecursoTrazas trazas = (ItfUsoRecursoTrazas) ClaseGeneradoraRepositorioInterfaces
								.instance()
								.obtenerInterfaz(
	...
	public void terminarPorPropiaVoluntad() throws Exception, Throwable {
		...
		ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador",
				"Terminando el Iniciador.", InfoTraza.NivelTraza.debug));
		try {
			...
			itfUsoRepositorio
					.eliminarRegistroInterfaz(NombresPredefinidos.ITF_USO
							+ this.nombreAgente);
			itfUsoRepositorio
					.eliminarRegistroInterfaz(NombresPredefinidos.ITF_GESTION
							+ this.nombreAgente);
		...
		ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador",
				"Terminando.", InfoTraza.NivelTraza.debug));
		if (this.hebra != null) {
