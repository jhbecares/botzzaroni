...
public class VerificarExistenciaEntidadesDescripcion extends AccionSincrona {
	...
	//
	// try {
	// // Se crea el recurso de configuración y posteriormente se debería dar
	// orden al recurso
	...
	// trazas.trazar(this.nombreAgente,"Creacion de recursos nucleo Organizacion",
	// NivelTraza.info);
	...
	// NombresPredefinidos.CONFIGURACION,configuracionExterna);
	// // Ahora que la configuracion es correcta , interpreto las propiedades
	// globales
	...
	// configuracionExterna.getDescInstanciaGestor(identGestor);
	// locComportamientoGestor =
	// desCompGestor.getDescComportamiento().getLocalizacionComportamiento();
	// if (
	// identGestor.startsWith(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION)){
	// if (locComportamientoGestor.isEmpty())
	// locComportamientoGestor =
	// NombresPredefinidos.PAQUETE_GESTOR_ORGANIZACION;
	//
	// } else
	...
	// }
	// else{
	// // El gestor inicial debe ser un gestor de organización o un gestor de
	// nodo y no se ha definido ninguno
	// ItfUsoRecTrazas.aceptaNuevaTraza(new InfoTraza("Iniciador",
	...
	// else { // Si la organizacion es distribuida el iniciador mira si esta en
	// el nodo del GO o en otro nodo
	// String identNodoGO =
	...
	// if ( thisHost.equalsIgnoreCase(identNodoGO)) // Esta en el nodo del GO se
	// crea un Gestor de Organizacion
	// identGestorNodo = NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION;
	// else identGestorNodo = NombresPredefinidos.NOMBRE_GESTOR_NODO; // Se crea
	// un Gestor de Nodo
	// }
	//
	// // if (
	...
	// // }
	// locComportamientoGestor =
	// this.localizarComportamientoGestorInicial(identGestorNodo);
	//
	// if (!locComportamientoGestor.isEmpty() ) {
	// // Se crea el gestor y se registra en el registro RMI en el caso en que
	// la organizacion sea distribuida
	// try {
	...
	// ItfgestGestorInicial = (ItfGestionAgenteReactivo)
	// itfUsoRepositorio.obtenerInterfaz(
	...
	// * (enume.hasMoreElements()) { Agente item = (Agente)enume.nextElement();
	// if
	// * (nombreAgente.equals(item.getNombre())) return
	// * item.getHayQueArrancarlo(); } logger.error("GestorAgentes: No se
	// encontr�
	// * ning�n agente con nombre "+nombreAgente+" dentro de la lista de
	// objetos
	...
	// monitoriz == InterfazGestion.ESTADO_ERRONEO_RECUPERABLE || monitoriz ==
	// InterfazGestion.ESTADO_TERMINADO || monitoriz ==
	// InterfazGestion.ESTADO_TERMINANDO) {
	...
	//
	// /**
	// * Intenta recuperar los errores detectados en la monitorizaci�n
	// siguiendo
	...
	//
	// /**
	// * destruye los recursos que se crearon a lo largo del ciclo de vida de
	// este
	// * agente.
	// */
