...
public class FactoriaRecursoSimpleImp2 extends FactoriaRecursoSimple {
	...
	// private static final String PAQUETE_RECURSOS_APLICACION =
	// "icaro.aplicaciones.recursos.";
	String msgError;
	/**
	...
	public void crearRecursoSimple(DescInstanciaRecursoAplicacion recurso) {
		...
		try {
			...
			if (objRecurso != null) {
				itfAutomata = ClaseGeneradoraAutomataEFsinAcciones
				...
				// End Logging
				// logger.debug("Factoria de recurso simple: recurso " +
				// recurso+ " creado.");
				trazas.aceptaNuevaTraza(new InfoTraza(idRecurso,
						"Factoria de recurso simple: recurso " + idRecurso
								+ " creado.", InfoTraza.NivelTraza.debug));
				objRecurso
						.setItfUsoRepositorioInterfaces(ClaseGeneradoraRepositorioInterfaces
								.instance());
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						idRecurso,
						idRecurso
								+ ":No se puede crear el recurso. La clase generadora no esta bien definida ",
						NivelTraza.error));
				System.err
						.println(" No se puede crear  el recurso. La clase generadora no esta bien definida.");
			}

		} catch (Exception e) {
			trazas.aceptaNuevaTraza(new InfoTraza(idRecurso,
					"Factoria de recurso simple: Error al crear el recurso "
							+ idRecurso + e, InfoTraza.NivelTraza.error));
			logger.error(
					"Factoria de recurso simple: Error al crear el recurso "
							+ recurso, e);
		}
	...
	private ImplRecursoSimple obtenerInstClaseGeneradora(
			DescInstanciaRecursoAplicacion instRecurso) {

		DescRecursoAplicacion descComportamiento = instRecurso.getDescRecurso();
		if (descComportamiento == null) {
			msgError = "Factoria de recurso simple: Error al crear el recurso "
					+ idRecurso
					+ "la descripcion del comportamiento es null. \n";
			trazas.aceptaNuevaTraza(new InfoTraza(idRecurso, msgError,
					InfoTraza.NivelTraza.error));
			System.err.println(msgError);
		} else {
			String rutaClase = descComportamiento
			...
			try {
				...
				// cause.getMessage());
				msgError = "La invocation de la clase " + rutaClase
						+ " que debe implementar la generacion del recurso "
						+ " produce  un error \n Cuya causa es : "
						+ cause.getMessage() + "\n";
				trazas.aceptaNuevaTraza(new InfoTraza(idRecurso, msgError,
						InfoTraza.NivelTraza.error));
