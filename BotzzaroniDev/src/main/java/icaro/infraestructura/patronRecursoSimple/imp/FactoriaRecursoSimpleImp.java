...
public class FactoriaRecursoSimpleImp extends FactoriaRecursoSimple {
	...
	public void crearRecursoSimple(DescInstanciaRecursoAplicacion recurso) {
		...
		try {
			/*
			 * try{ trazas = (ItfUsoRecursoTrazas)repoIntfaces.obtenerInterfaz(
			...
			repoIntfaces.obtenerInterfaz(NombresPredefinidos.ITF_USO
			...
			String rutaClaseGeneradora = PAQUETE_RECURSOS_APLICACION
					+ nombrePaqueteRecurso + ".imp.ClaseGeneradora"
					+ nombreRecurso;
			...
			try {
				...
				trazas.aceptaNuevaTraza(new InfoTraza(idRecurso,
						"Error al leer la clase generadora del recurso "
								+ nombreRecurso
								+ ". Compruebe que existe la clase "
								+ rutaClaseGeneradora,
						InfoTraza.NivelTraza.error));
				// logger.error("Error al leer la clase generadora "+
				// rutaClaseGeneradora, ex);
			}
			...
			ImplRecursoSimple objRecurso = (ImplRecursoSimple) claseGen
			...
			// End Logging
			// logger.debug("Factoria de recurso simple: recurso " + recurso+
			// " creado.");
			trazas.aceptaNuevaTraza(new InfoTraza(idRecurso,
