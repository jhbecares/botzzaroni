...
public class FactoriaAgenteReactivoImp1 extends FactoriaAgenteReactivo {
	...
	public void crearAgenteReactivo(DescInstanciaAgente descInstanciaAgente)
		...
		if ((accionesSemanticasEspecificas != null) & (rutaTabla != null)) {
			...
			try {
				// En esta version la percepcion envia la informacion procesada
				// al control
				// Se crea en primer lugar el control y postieriormente la
				// percepcion y la clase que implementa
				// las interfaces externas
				// Se crea un objeto que implementa las interfaces de uso y de
				// gestion del agente
				...
				PercepcionAbstracto percepcion = FactoriaPercepcion.instancia()
						.crearPercepcion(agente, itfControlAgteReactivo);
				...
				this.agente.setComponentesInternos(nombreInstanciaAgente,
						itfAutomata, itfControlAgteReactivo,
	...
	public void crearAgenteReactivo(String nombreInstanciaAgente,
		...
		try {
			...
			ProcesadorInfoReactivoAbstracto control = FactoriaControlAgteReactivo
					.instancia()
					.crearControlAgteReactivo(accionesSemanticasEspecificas,
							rutaAutomata, agente);
			itfControlAgteReactivo = control;
			// Se crea la percepcion y se le pasa la interfaz del control
			...
			logger.error(
					"Error AL CREAR LA PERCEPCION. La factoria no puede crear la instancia : "
							+ nombreInstanciaAgente
	...
	private Class obtenerClaseAccionesSemanticas(String rutaComportamiento)
		...
		rutaComportamiento = NombresPredefinidos.RUTA_SRC + File.separator
				+ rutaComportamiento.replace(".", File.separator);
	...
	private AccionesSemanticasAgenteReactivo obtenerAcciones(
		...
		// String rutaAutomata =
		// comportamiento.getLocalizacionFicheroAutomata();
