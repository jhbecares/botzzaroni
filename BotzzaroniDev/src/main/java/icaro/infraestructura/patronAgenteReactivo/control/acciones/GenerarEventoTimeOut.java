...
public class GenerarEventoTimeOut extends Thread {
	...
	public GenerarEventoTimeOut(long milis, String identInput, String origen,
			String destino, ItfUsoRepositorioInterfaces repositorio) {
		super();
		this.milis = milis;
	...
	public void run() {
		try {
			sleep(milis);
			ItfUsoAgenteReactivo destinatario = (ItfUsoAgenteReactivo) repositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + destino);
			destinatario.aceptaEvento(new EventoRecAgte(inputAenviar, origen,
					destino));
		} catch (Exception e) {
