...
/**
 * Clase que define autmatas de estados finitos sin transiciones vacas escritos
 * en XML
 *
...
public class AutomataEFEImp implements ItfUsoAutomataEFE {
	...
	public AutomataEFEImp(String NombreFicheroDescriptor,
			EjecutorDeAccionesAbstracto accionesSem, int nivelTraza,
			String nombreAgente) throws ExcepcionNoSePudoCrearAutomataEFE {
		...
		try {
			XMLParserTablaEstados parser = new XMLParserTablaEstados(
			...
			traza = nivelTraza;
			logger.debug("Usando el automata del fichero: "
					+ NombreFicheroDescriptor);
			...
			if (trazas != null) {
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
						"no se pudo crear el automata EFE porque se produjo un error en "
								+ e.getParteAfectada() + "debido a:"
								+ e.getCausa() + " en el contexto "
								+ e.getContextoExcepcion(),
		...
		if (conjuntoInputs.contains(input)) {
			if (theTablaEstadosControl.esInputValidoDeEstado(estadoActual,
				...
				try {
					...
					logger.info("Transicion usando input:" + input + "  :"
							+ estadoActual + " -> " + op.estadoSiguiente);
					trazas.aceptaNuevaTraza(new InfoTraza(
							nombreAgente,
							"Transicion usando input '" + input
									+ "'. ESTADO ACTUAL: "
									+ estadoAntesdeTransitar + " -> "
									+ "ESTADO SIGUIENTE: " + op.estadoSiguiente,
							InfoTraza.NivelTraza.info));
					return true;
				} catch (ExcepcionEjecucionAcciones ex) {
					java.util.logging.Logger.getLogger(
							AutomataEFEImp.class.getName()).log(Level.SEVERE,
							null, ex);
					// this.estadoActual =
					// this.theTablaEstadosControl.crearEstadoErrorInterno();
					this.estadoActual = "errorInternoIrrecuperable";

					logger.info("Error al procesar el " + input
							+ "Ejecutando accion: " + op.accionSemantica
							+ "se transita al estado" + estadoActual);
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Error al procesar el " + input
									+ "Ejecutando accion: "
									+ op.accionSemantica
									+ "se transita al estado" + estadoActual,
							InfoTraza.NivelTraza.error));
				...
				logger.info("AVISO: Input ignorado.El input: "
						+ input
						+ " no pertenece a los inputs validos para el estado actual: "
						+ estadoActual);
			...
			logger.error("ERROR: Input " + input + " no valido para "
					+ descripcionAutomata);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "ERROR: Input "
					+ input + " no valido para " + descripcionAutomata,
					InfoTraza.NivelTraza.error));
	...
	public synchronized void transita(String input) {
		...
		if (theTablaEstadosControl.esInputValidoDeEstado(estadoActual, input)) {
			Operacion op = theTablaEstadosControl.dameOperacion(estadoActual,
					input);
			logger.info("Transicion Directa input:" + input + "  :"
					+ estadoActual + " -> " + op.estadoSiguiente);
			cambiaEstado(op.estadoSiguiente);
			// cambiar al siguiente estado

		} else {
			logger.info("AVISO: Input de Transicion directa .El input: "
					+ input
					+ " no pertenece a los inputs validos para el estado actual: "
					+ estadoActual);
			/*
	 ...
	 * de control // 1. Creo las acciones semnticas
	 * tid.tecHabla.agentes.componentes
	 * .infraestructura.automata.AutomataControl.funciones fun = new
	 * tid.tecHabla
	 * .agentes.componentes.infraestructura.automata.AutomataControl.
	 * funciones(); // 1.5 Creo objeto de acciones semnticas AccionesSemanticas
	 * as = new AccionesSemanticas(fun); // 2. Creo el autmata AutomataControl
	 * automata = new AutomataControl("TablaEstadosPruebaDeTablaEstados.xml",
	 * as,
	 * tid.tecHabla.agentes.componentes.infraestructura.automata.AutomataControl
	 * .NIVEL_TRAZA_TODO); // 3. Tengo el autmata disponible para usar
	 * 
