...
/**
 * Clase base de la que deben heredar todas las acciones semnticas de los
 * agentes reactivos.
 *
 * @author F Garijo Esta clase depende del Gestor de Acciones del Agte reactivo
 *         que es quien crea las A semanticas especificas Necesita informacion
 *         contextual que debe definirse en la inicializacion. La informacion qe
 *         necesite la pide a su gestor que a su vez la pedira a los elementos
 *         internos de los que dependa
 * @version 4.0
 */
...
public abstract class AccionesSemanticasAgenteReactivo {
	 ...
	 *                     "nombre:java.lang.String java.lang.Object"
	 */
	...
	public void inicializarAcciones(String identAgte,
			ItfControlAgteReactivo itfControl,
			ItfProductorPercepcion itfProductPercept) {
	...
	public void infoParaDesarrollador(String mensaje) {
		if (recursoTrazasDefinido) {
			this.trazas.trazar(identEstaClase, mensaje,
	...
	public ItfGestionAgenteReactivo getIftGestionAgente() {
		if (itfGestionAgte != null) {
			...
			if (this.itfUsoRepositorio != null) {
				try {
					itfGestionAgte = (ItfGestionAgenteReactivo) itfUsoRepositorio
							.obtenerInterfazGestion(nombreAgente);
				} catch (Exception ex) {
					// java.util.logging.Logger.getLogger(AccionesSemanticasAgenteReactivo.class.getName()).log(Level.SEVERE,
					// null, ex);
					this.infoParaDesarrollador("Se ha producido un error al obtener la interfaz de gestion del agente "
							+ ex);
					return null;
				}
			}
	...
	// public void setItfUsoRepositorioInterfaces(ItfUsoRepositorioInterfaces
	// itfUsoR)
	// {
	...
	// public void informaraMiAutomata(String input, Object[]
	// infoComplementaria){
	// // Este método crea un evento con la información de entrada y se le
	// envía a si mismo por medio de
	// // la interfaz de uso
	...
	// }
	// else {
	// eventoaEnviar = new EventoRecAgte(input,infoComplementaria, nombreAgente,
	// nombreAgente);
	// }
	// try {
	...
	// }
	// catch (Exception ex) {
	// logger.error("Ha habido un problema al enviar un  input a este agente "
	// +nombreAgente);
	// trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
	// "Ha habido un problema al procesar el  "+ input +
	// "por el automata que implementa el control",
	// InfoTraza.NivelTraza.error));
	// }
	//
	// }
	...
	// itfEnvioEventosInternos.produceParaConsumirInmediatamente(new
	// EventoInternoAgteReactivo(nombreAgente, input, infoComplementaria));
	// else trazas.trazar(nombreAgente,
	...
	public void informaraMiAutomata(String input, Object... infoComplementaria) {
		...
		if ((infoComplementaria == null)
				|| ((infoComplementaria.length == 1) && (infoComplementaria[0] == null))) {
		...
		if (itfEnvioEventosInternos != null) {
			itfEnvioEventosInternos
					.produceParaConsumirInmediatamente(new EventoInternoAgteReactivo(
							nombreAgente, input, paramsAccion));
		} else {
			trazas.trazar(nombreAgente,
					" El interfaz para el envio de eventos internos no esta definida "
							+ " El input:  " + input + " No sera procesado ",
					InfoTraza.NivelTraza.error);
		}
	...
	public void procesarInput(Object input, Object[] infoComplementaria) {
		if (itfEnvioEventosInternos == null) {
			...
			itfEnvioEventosInternos
					.produceParaConsumirInmediatamente(new EventoInternoAgteReactivo(
							nombreAgente, input, infoComplementaria));
	...
	public void generarTimeOut(long milis, String identInput, String origen,
			String destino) {
		GenerarEventoTimeOut timeout = new GenerarEventoTimeOut(milis,
				identInput, origen, destino, this.itfUsoRepositorio);
		logger.debug("Generando evento de timeout de " + milis
				+ " milisegundos");
		timeout.start();
	}
