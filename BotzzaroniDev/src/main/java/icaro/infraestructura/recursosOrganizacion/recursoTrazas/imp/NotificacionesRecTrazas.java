...
public class NotificacionesRecTrazas implements Serializable {
	 ...
	 *                     "controlador:icaro.infraestructura.recursosOrganizacion.recursoTrazas.imp.componentes.ClasificadorVisual"
	 */
	...
	public synchronized void pedirTerminacionOrganizacion() {
		try {
			if (gestoraReportar != null) {
				gestoraReportar.aceptaEvento(new EventoRecAgte(
						"peticion_terminar_todo_usuario",
						NombresPredefinidos.RECURSO_TRAZAS, gestoraReportar
								.getIdentAgente()));
			} else {
			...
			e.printStackTrace();
			logger.fatal(
					"Error al obtener la interfaz del gestor Inicial . Hay un problema con el recurso de trazas,",
					e);
		}
	}
	...
	public synchronized void confirmarTerminacionOrganizacion() {
		try {
			if (gestoraReportar != null) {
				gestoraReportar.aceptaEvento(new EventoRecAgte(
						"terminacion_confirmada",
						NombresPredefinidos.RECURSO_TRAZAS, gestoraReportar
								.getIdentAgente()));
				NombresPredefinidos.RECURSO_TRAZAS_OBJ
						.aceptaNuevaTraza(new InfoTraza(
								NombresPredefinidos.RECURSO_TRAZAS,
								"Se envia el evento con input :"
										+ "terminacion_confirmada"
										+ " al   agente "
										+ gestoraReportar.getIdentAgente(),
								InfoTraza.NivelTraza.debug));
			} else {
				logger.fatal("Error al pedirTerminacionOrganizacion. No se puede obtener la interfaz del gestor a reportar : "
			...
			e.printStackTrace();
			logger.fatal(
					"Error al enviar eventos al  gestor Inicial . Hay un problema con el recurso de trazas,",
					e);
		}
	}
	...
	public void setIdentGestoraReportar(String identGestor) {
		try {
			if (identGestor != null) {
				...
				gestoraReportar = (ItfUsoAgenteReactivo) NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
