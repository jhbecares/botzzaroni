...
/**
 * Clase que define automatas de estados finitos, sin acciones semanticas a
 * ejecutar en las transiciones, escritos en XML
 *
 * @author Francisco J Garijo
 * @modified 02 de Marzo de 2014
 */

// public abstract class InterpreteAutomataEFconGestAcciones implements
// ItfAutomataEFconGestAcciones,Serializable{
public class InterpreteAutomataEFconGestAcciones implements ItfUsoAutomataEFE,
		Serializable {
	 ...
	 *            en esta clase)
	 */
	public InterpreteAutomataEFconGestAcciones(
			TablaEstadosAutomataEFinputObjts intpTablaEstados,
			Boolean activarTrazas) {

		// XMLParserTablaAutomataSinAcciones parser = new
		// XMLParserTablaAutomataSinAcciones();
		// String ruta =
		...
		// interpTablaEstados =
		// parser.extraeTablaEstadosDesdeFicheroXML(NombreFicheroDescriptor);
		...
		if (NombresPredefinidos.RECURSO_TRAZAS_OBJ == null) {
			try {
				...
				java.util.logging.Logger.getLogger(
						InterpreteAutomataEFconGestAcciones.class.getName())
		...
		//
		// traza = nivelTraza;
		// String NombreFicheroDescriptor =
		// interpTablaEstados.dameFicheroDefEstados();

		// logger.debug("Usando el automata de ciclo de vida del fichero: " +
		// NombreFicheroDescriptor);
		// logger.debug(this.toString());
		/*
		 * try{ trazas =
		 * (ItfUsoRecursoTrazas)RepositorioInterfaces.instance().obtenerInterfaz
	...
	// @Override
	// public abstract boolean ejecutarTransicion(Object input, Object[]
	// params);
	//
	// Si el input es de tipo String Tomamos como input la cadena que representa
	// el input

	// }else {// tomamos como input el nombre de la clase del input
	//
	// }
	// // if( tablaEstadosAutomata.esInputValidoDeEstado(estadoActual,
	// inputAutomata)){
	// TransicionAutomataEF transicion =
	// tablaEstadosAutomata.getTransicion(estadoActual()+inputAutomata);
	// if (transicion == null){
	// // mensaje de error el inputno es valido en el estado actual
	// }else {
	// // obtenemos el tipo de transción ************* Revisar
	// // if (tipoTransicion==
	// NombresPredefinidos.TRANSICION_AUTOMATA_EF_SIN_ACCION){
	...
	public synchronized boolean ejecutarTransicion(String input) {
		...
		//
		// if( input instanceof String ){ // Si el input es de tipo String
		// Tomamos como input la cadena que representa el input
		...
		if (!tablaEstadosAutomata.esInputValidoDeEstado(estadoActual, input)) {
			this.trazas.trazar(this.getClass().getSimpleName(), " El input : "
					+ input
					+ " No coincide con los inputs definidos en el automata "
					+ " en el estado  :" + estadoActual,
					InfoTraza.NivelTraza.error);
			return false;
		}
		TransicionAutomataEF transicion = tablaEstadosAutomata
				.getTransicion(estadoActual() + input);
		if (transicion == null) {
			// mensaje de error el inputno es valido en el estado actual
			this.trazas.trazar(this.getClass().getSimpleName(),
					" No existe transicion asociada al input : " + input
							+ " en el estado  :" + estadoActual,
					InfoTraza.NivelTraza.error);
			return true;
		}
		...
		// if
		// (claseAEjecutar.getSimpleName().startsWith(NombresPredefinidos.NOMBRE_ACCIONES_SEMANTICAS)
		// ){
		if (tipoTransicion == NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_BLOQ) {
			try {
				this.itfGestAcciones.ejecutarMetodo(claseAEjecutar,
						transicion.getIdentMetodoAccion(), (Object) null);
				this.estadoActual = transicion.getidentEstadoSiguiente();
				return true;
			} catch (Exception ex) {
				java.util.logging.Logger.getLogger(
	...
	public synchronized void transita(String input) {
		...
		// if (tablaEstadosAutomata.esInputValidoDeEstado(estadoActual, input))
		// {
		// estadoActual = tablaEstadosAutomata.dameEstadoSiguiente(estadoActual,
		// input);
		// // cambiar al siguiente estado
		// // logger.info("Transicion en el ciclo de vida usando input:" + input
		...
		//
		//
		// logger.info("AVISO: Input de ciclo de vida ignorado.El input: " +
		// input + " no pertenece a los inputs vlidos para el estado actual: " +
		// estadoActual);
		/*
		 * trazas.aceptaNuevaTraza(new InfoTraza("AutomataCicloVidaRecurso",
		 * "AVISO: Input de ciclo de vida ignorado.El input: " + input +
		 * " no pertenece a los inputs vlidos para el estado actual: " +
		 * estadoActual, InfoTraza.NivelTraza.info));
		 */
	...
	// //
	// // if( inputAutomata.equals("String")){ // Si el input es de tipo String
	// Tomamos como input la cadena que representa el input
	// // inputAutomata = (String)input;
	// // }
	// // if( tablaEstadosAutomata.esInputValidoDeEstado(estadoActual,
	...
	// TransicionAutomataEF transicion =
	// this.tablaEstadosAutomata.getTransicion(estadoActual()+input);
	// if (transicion == null){
	// // mensaje de error el input no es valido en el estado actual
	// this.trazas.trazar
	// (this.getClass().getSimpleName()," No existe transicion asociada al input : "
	// + input +
	...
	// Integer tipoTransicion = transicion.getTipoTransicion();
	// if (tipoTransicion ==
	// NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_SIN_ACCION){
	// return true;
	// }
	// Class accion = transicion.getClaseAccion();
	// if (tipoTransicion ==
	// NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_BLOQ ||
	// tipoTransicion ==
	// NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_CONCURR){
	// String metodoId = transicion.getIdentMetodoAccion();
	// if (metodoId!=null )
	// try {
	// // se trata de una claseAcciones semanticas, ejecutamos el metodo de la
	// clase
	// if (tipoTransicion ==
	// NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_BLOQ ){
	...
	// return true;
	// } catch (Exception ex) {
	// java.util.logging.Logger.getLogger(InterpreteAutomataEFconGestAcciones.class.getName()).log(Level.SEVERE,
	// null, ex);
	// return false;
	// }
	// }
	// try { // se trata de una claseAcciones semanticas donde no se ha
	// especificado el metodo, o de una accion sincrona o asincron
	...
	public synchronized void interpretarTransicion(
			TransicionAutomataEF transicion) {
		// le manda la accion al ejecutor de acciones para que ejecute y
		// transita al estado que indica la transicion
		Integer tipoTransicion = transicion.getTipoTransicion();
	...
	public synchronized boolean procesaInput(Object input, Object... parametros) {
		...
		// Object[] parametroAccion = new Object[1];
		if (input == null) {
			trazas.aceptaNuevaTraza(new InfoTraza(this.getClass()
					.getSimpleName(),
					"ERROR: El input a procesar no puede ser null ",
					InfoTraza.NivelTraza.error));
			return false;
		}
		...
		// int i=0;
		Object[] valoresParametrosAccion = new Object[(parametros).length
				+ posparametros];
		if (posparametros == 1) {
			valoresParametrosAccion[0] = paramPosicion0;
		}
	...
	private synchronized boolean ejecutarTransicion(String input,
			Object[] params) {
		...
		estadoActual();
		TransicionAutomataEF transicion = this.tablaEstadosAutomata
				.getTransicion(estadoActual() + input);
		if (transicion == null) {
			// mensaje de error el input no es valido en el estado actual
			this.trazas.trazar(this.getClass().getSimpleName(),
		...
		if ((tipoTransicion == NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_BLOQ)
			...
			if (metodoId != null) {
				try {
					// se trata de una claseAcciones semanticas, ejecutamos el
					// metodo de la clase
					if (tipoTransicion == NombresPredefinidos.AUTOMATA_EF_TIPO_TRANSICION_METODO_AS_BLOQ) {
						this.itfGestAcciones.ejecutarMetodo(accion, metodoId,
								params);
					} else {
						this.itfGestAcciones.ejecutarMetodoThread(accion,
								metodoId, params);
					}
					return true;
				} catch (Exception ex) {
					java.util.logging.Logger
							.getLogger(
									InterpreteAutomataEFconGestAcciones.class
		...
		try { // se trata de una claseAcciones semanticas donde no se ha
			...
			java.util.logging.Logger.getLogger(
					InterpreteAutomataEFconGestAcciones.class.getName()).log(
			...
			if (tablaEstadosAutomata.esEstadoValido(nuevoEstado)) {
				...
				if (trazas != null) {
					trazas.trazar(
							...
							"El estado al que se debe transitar : "
									+ nuevoEstado
	...
	/**
	 * Dice si el recurso est en estado activo, es decir, que puede ejecutar
	 * mtodos
	 *
