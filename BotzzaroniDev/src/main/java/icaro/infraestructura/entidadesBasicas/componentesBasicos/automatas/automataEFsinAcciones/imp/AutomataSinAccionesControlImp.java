...
/**
 * Clase que define autmatas de estados finitos, sin acciones semanticas a
...
public class AutomataSinAccionesControlImp extends
		ClaseGeneradoraAutomataEFsinAcciones implements
		ItfUsoAutomataEFsinAcciones, Serializable {
	 ...
	 *            en esta clase)
	 */
	public AutomataSinAccionesControlImp(
			TablaEstadosAutomataEF intpTablaEstados, int nivelTraza) {
	...
	public void transita(String input) {
		...
		if (interpTablaEstados.esInputValidoDeEstado(estadoActual, input)) {
			siguiente = interpTablaEstados.dameEstadoSiguiente(estadoActual,
					input);
			// cambiar al siguiente estado
			// logger.info("Transicion en el ciclo de vida usando input:" +
			// input + "  :" + estadoActual + " -> " + siguiente);
	 ...
	 * tid.tecHabla.agentes.componentes.infraestructura
	 * .automata.AutomataControl.funciones fun = new
	 * tid.tecHabla.agentes.componentes
	 * .infraestructura.automata.AutomataControl.funciones(); // 1.5 Creo objeto
	 * de acciones semnticas AccionesSemanticas as = new
	 ...
	 * tid.tecHabla.agentes
	 * .componentes.infraestructura.automata.AutomataControl.NIVEL_TRAZA_TODO);
	 * 
