...
public class InterpreteEventosSimples {
	...
	public Evidencia generarEvidencia(EventoSimple evento) {
		...
		if (evento == null) {
			...
			evidencia = null;
			trazas.aceptaNuevaTraza(new InfoTraza(this.propietario,
					" El evento recibido para interpretar es null ",
			...
			{
				// Creamos un objeto generico eventInfo que contiene el tipo de
				// evento y la
				...
				// Se crea una evidencia con el contenido del evento sin
				// transformar
	...
	public ExtractedInfo generarExtractedInfo(EventoSimple evento) {
		...
		if (evento == null) {
			...
			evento.getOrigen();
			// Se extrae el contenido del evento y se transforma según sea su
			// tipo
			contenidoEvento = evento.getContenido();
			// if ( contenidoEvento instanceof InfoContEvtMsgAgteReactivo ){
			// PerformativaUsuario infoEvento =
			// transformaInfoControlAgteReactivoEnPerformativaUsuario
			// ((InfoContEvtMsgAgteReactivo)contenidoEvento);
			// if (infoEvento != null){
	...
	public PerformativaUsuario transformaInfoControlAgteReactivoEnPerformativaUsuario(
			InfoContEvtMsgAgteReactivo eventContentInfo) {
		...
		if ((operacion.contains("peticion"))) {
			// la peticion de alta la transformamos en una peticion donde la
			// operacion sera dar de alta y a continuacion pasamos los
			// paramentros de la operacion
			performativa = "peticion";
			// String operacion = contenidoMSG;
			// Object parametros = evento.getMsgElements();
			// Los parametros son los parametros del evento
			// return new PerformativaUsuario ( performativa,operacion,
			// parametros );
		} else {
			trazas.aceptaNuevaTraza(new InfoTraza(
					this.propietario,
					" Hacemos una transformacion estandar  en performativa de usuario ",
					InfoTraza.NivelTraza.debug));
			// return null;
		}
