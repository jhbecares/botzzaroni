 ...
 */
public class FactoriaPercepcionAgenteCognitivoImp2 extends
		FactoriaPercepcionAgenteCognitivo {
	...
	@Override
	public PercepcionAgenteCognitivo crearPercepcion(
			AgenteCognitivo agenteCognitivo,
			ItfProcesadorObjetivos itfProcesadorExtractedInfo) {
		// Primero se se crean los objetos que forman la percepcion y luego se
		// ensamblan segun sus dependencias
		...
		extractedInfoQ = new LinkedBlockingQueue<ExtractedInfo>(
				CAPACIDAD_BUZON_CONT_MESSG);
		percepcionAgteCognitivo = new PercepcionAgenteCognitivoImp();
		// El procesador de items produce elementos extraidos de los mensajes o
		// de los eventos
		procesItems = new ProcesadorItems();

		// itfProcesador = new ProcesadorItems();

		// Ensamblaje de los elementos pasando a cada uno los elementos
		// necesarios para su funcionamiento
		percepcionAgteCognitivo.SetParametrosPercepcionAgenteCognitivoImp(
				buzonMsges, procesItems, agenteCognitivo);
		procesItems.SetParametrosProcesadorItems(agenteCognitivo,
				extractedInfoQ, itfProcesadorExtractedInfo);
