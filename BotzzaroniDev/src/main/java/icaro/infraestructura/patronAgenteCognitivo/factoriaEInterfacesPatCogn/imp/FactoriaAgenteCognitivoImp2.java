...
public class FactoriaAgenteCognitivoImp2 extends FactoriaAgenteCognitivo {
	...
	public void createCognitiveAgent(DescInstanciaAgente descInstanciaAgente)
		...
		String descName = descInstanciaAgente.getDescComportamiento()
				.getNombreComportamiento();
		descName.substring(0, 1).toLowerCase();
		descName.substring(1);

		// Get interface repository
		// ItfUsoRepositorioInterfaces repositorio =
		...
		try {
			...
			String rutaComportamiento = descInstanciaAgente
					.getDescComportamiento().getLocalizacionComportamiento();
			String rutaReglas = rutaComportamiento + "."
					+ NombresPredefinidos.CARPETA_REGLAS;
			rutaReglas = obtenerRutaValidaReglas(rutaReglas);

			// Create cognitive agent
			trazas.aceptaNuevaTraza(new InfoTraza(agentName, agentName
			...
			trazas.aceptaNuevaTraza(new InfoTraza(agentName, agentName
					...
					InfoTraza.NivelTraza.debug));
			repositorioInterfaces.registrarInterfaz(NombresPredefinidos.ITF_USO
					+ agentName, agent);
			repositorioInterfaces.registrarInterfaz(
					NombresPredefinidos.ITF_GESTION + agentName, agent);
		...
		// Damos orden a las factorias de los componentes internos para que
		// creen los elementos

	}

	@Override
	public void crearAgenteCognitivo(DescInstanciaAgente descInstanciaAgente)
		...
		trazas.aceptaNuevaTraza(new InfoTraza(nombreInstanciaAgente,
				tipoEntidad, nombreInstanciaAgente + ": Creando el agente  "
						+ nombreInstanciaAgente, InfoTraza.NivelTraza.debug));
		...
		if ((rutaReglas != null)) {
			try {
				...
				// del agente
				ItfUsoAutomataEFsinAcciones itfAutomataEstadoAgente = ClaseGeneradoraAutomataEFsinAcciones
						.instance(NombresPredefinidos.FICHERO_AUTOMATA_CICLO_VIDA_COMPONENTE);
				// itfAutomataEstadoAgente.transita(NombresPredefinidos.);
				agenteContrl = new AgenteCognitivotImp2(nombreInstanciaAgente);

				// Ordenamos a las factorias que creen los componentes internos

				// Creamos en primer lugar el Procesador de objetivos porque es
				// el mas critico y porque la percepcion
				// envia las evidencias a al procesador de objetivos => necesata
				// conocer su interfaz
				procesObjetivos = FactoriaProcesadorObjetivos.instance()
						.crearProcesadorObjetivos(agenteContrl, rutaReglas);
				percepcion = FactoriaPercepcionAgenteCognitivo.instance()
						.crearPercepcion(agenteContrl, procesObjetivos);

				// Definimos los parametros del objeto que implementa las
				// interfaces del agente

				agenteContrl.setComponentesInternos(itfAutomataEstadoAgente,
						percepcion, procesObjetivos);
				...
				repositorioInterfaces
						.registrarInterfaz(NombresPredefinidos.ITF_GESTION
								+ nombreInstanciaAgente, agenteContrl);
				repositorioInterfaces.registrarInterfaz(
						NombresPredefinidos.ITF_USO + nombreInstanciaAgente,
						agenteContrl);
				// La factoria podria verificar que el estado del automata del
				// ciclo de vida es creado
				...
				logger.error(
						"Error al crear Procesador de Objetivos del agente. La factoria no puede crear la instancia o no se pueden obtener la interfaces : "
								+ nombreInstanciaAgente, exc);
				System.err
						.println(" No se puede crear el Procesador de Objetivos del agente. La factoria no puede crear la instancia.");
				...
				logger.error(
						"Error AL CREAR el Agente Cognitivo. La factoria no puede crear la instancia : "
								+ identAgente, ex);
				agenteContrl.setEstado(NombresPredefinidos.ESTADO_ERROR);
				System.err
						.println(" No se puede crear la Percepcion del agente. La factoria no puede crear la instancia.");
				throw new ExcepcionEnComponente("patronAgenteCognitivo.contol",
						" error al crear los componentes internos", "factoria",
						"");
			}
	...
	private String obtenerRutaValidaReglas(String rutaComportamiento)
			throws ExcepcionEnComponente {
		...
		}
		{
			...
			throw new ExcepcionEnComponente("PatronAgenteCognitivo",
					"No se encuentra el fichero de reglas en la ruta :"
							+ rutaComportamiento,
