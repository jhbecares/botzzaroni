...
public class GestorTareasImp implements ItfGestorTareas {
	...
	public GestorTareasImp(AgenteCognitivo agente,
	...
	public Tarea crearTarea(Class clase) throws Exception {
		...
		// identTarea =
		// identTareaLong.substring(identTareaLong.lastIndexOf(".")+1);
	...
	public TareaSincrona crearTareaSincrona(Class clase) throws Exception {
		...
		if (tareasCreadas.containsKey(identTarea)) {
			return tareasCreadas.get(identTarea);
		}
		...
		if (superclase.equals(tareaSincSimpleName)) {
			...
			this.trazas
					.trazar(this.getClass().getSimpleName(),
							"Error en  la ejecucion de la tarea: "
									+ claseTareaEjecutar.getSimpleName()
									+ " debe extender a TareaSincrona o a TareaAsincrona ",
