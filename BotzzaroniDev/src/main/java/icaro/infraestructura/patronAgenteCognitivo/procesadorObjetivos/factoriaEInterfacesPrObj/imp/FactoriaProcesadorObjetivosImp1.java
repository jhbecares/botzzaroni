 ...
 *
 */
public class FactoriaProcesadorObjetivosImp1 extends
		FactoriaProcesadorObjetivos {
	private Logger log = Logger
			.getLogger(FactoriaProcesadorObjetivosImp1.class);

	@Override
	public ProcesadorObjetivos crearProcesadorObjetivos(AgenteCognitivo agente,
			String ficheroResolucionObjetivos) throws ExcepcionEnComponente {
		String identAgte = agente.getIdentAgente();
		InputStream reglas;
		reglas = this.getClass()
				.getResourceAsStream(ficheroResolucionObjetivos);
		ItfUsoRecursoTrazas trazas = NombresPredefinidos.RECURSO_TRAZAS_OBJ;
		try {
			if (reglas != null) {
				// ItfMotorDeReglas motorDeReglas =
				// FactoriaMotorDeReglas.instance().crearMotorDeReglas(agente,reglas);
				ItfMotorDeReglas motorDeReglas = FactoriaMotorDeReglas
						.instance().crearMotorDeReglas(agente, reglas,
								ficheroResolucionObjetivos);
				...
				ItfGestorTareas taskManager = TaskManagerFactory.instance()
						.createTaskManager(agente, itfProcObj);
				// ItfUsoRepositorioInterfaces repoIntfs =
				...
				// motorDeReglas.addGlobalVariable(NombresPredefinidos.TASK_MANAGER_GLOBAL,
				// taskManager);
				// motorDeReglas.addGlobalVariable(NombresPredefinidos.ITFUSO_RECURSOTRAZAS_GLOBAL,
				// trazas);
				// motorDeReglas.addGlobalVariable(NombresPredefinidos.AGENT_ID_GLOBAL,
				// agentName);
				...
				String msg = "Rules not found for " + identAgte
						+ "\n Check the " + ficheroResolucionObjetivos
						+ " is in classpath";
				trazas.trazar(identAgte, msg, InfoTraza.NivelTraza.error);
				log.error(msg);
				// ExcepcionEnComponente(String identComponente,String causa,
				// String identParteAfectada,String contextoExcepcion )
				throw new ExcepcionEnComponente("FactoriaMotorDeReglas", msg,
						"MotorDeReglas", "Creacion del Motor de Reglas");
			}
		} catch (Exception e) {
			log.error("Errors compiling rules for agent " + identAgte, e);
			throw new ExcepcionEnComponente("FactoriaMotorDeReglas",
					"Errors compiling rules for agent " + identAgte,
					"MotorDeReglas", "Creacion del Motor de Reglas");
		}
	}
