	...
	public static void main(String args[]) {
		...
		if (args.length == 0) {
			...
			try {
				...
				ItfGestIniciador = (ItfGestionAgenteReactivo) ClaseGeneradoraRepositorioInterfaces
						.instance().obtenerInterfaz(
								NombresPredefinidos.ITF_GESTION
										+ NombresPredefinidos.NOMBRE_INICIADOR);
				ItfUsoIniciador = (ItfUsoAgenteReactivo) ClaseGeneradoraRepositorioInterfaces
						.instance().obtenerInterfaz(
								NombresPredefinidos.ITF_USO
										+ NombresPredefinidos.NOMBRE_INICIADOR);
				...
				if ((ItfGestIniciador != null) && (ItfUsoIniciador != null)) {
					...
					// DescDefOrganizacion descOrganizacionaCrear = new
					// DescDefOrganizacion();
					// descOrganizacionaCrear.setIdentFicheroDefOrganizacion(args[0]);
					// ItfUsoIniciador.aceptaEvento( new
					// EventoRecAgte("crearOrganizacion",descOrganizacionaCrear,
					// "main", "iniciador" ));
					// args[0] contiene el identificador del fichero que
					// contiene la definicion de la organizacion a crear
					// ItfUsoIniciador.aceptaEvento( new
					// EventoRecAgte("crearOrganizacion",args[0], "main",
					// "iniciador" ));
				}
