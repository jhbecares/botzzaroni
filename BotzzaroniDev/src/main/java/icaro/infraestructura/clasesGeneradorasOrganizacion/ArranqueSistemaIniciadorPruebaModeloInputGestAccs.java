...
public class ArranqueSistemaIniciadorPruebaModeloInputGestAccs {
	...
	public static void main(String args[]) {
		...
		// "Descripción de Organizacion no encontrado. ¿Desea arrancar el asistente de creación de Descripción de Organización?",
		...
		try {
			...
			ItfUsoRepositorioInterfaces repositorioInterfaces = ClaseGeneradoraRepositorioInterfaces
					.instance();
			recursoTrazas = ClaseGeneradoraRecursoTrazas.instance();
			repositorioInterfaces.registrarInterfaz(NombresPredefinidos.ITF_USO
					+ NombresPredefinidos.RECURSO_TRAZAS, recursoTrazas);
			...
			// NombresPredefinidos.RUTA_DESCRIPCIONES+args[0];
			// NombresPredefinidos.DESCRIPCION_XML_POR_DEFECTO = args[0];
		} catch (Exception e) {
			System.err
					.println("Error. No se pudo crear o registrar el recurso de trazas");
			e.printStackTrace();
			// no es error cr�tico
		}
		...
		try {
			...
			FactoriaComponenteIcaro.instanceAgteReactInpObj()
					.crearAgenteReactivo(identPropietario, rutaCarpetaAcciones);
			itfUsoIniciador = (ItfUsoAgenteReactivo) NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
					.obtenerInterfazUso(identPropietario);
			itfGestIniciador = (ItfGestionAgenteReactivo) NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
					.obtenerInterfazGestion(identPropietario);
			itfGestIniciador.arranca();
			EventoRecAgte eventoPrueba = new EventoRecAgte("comenzar", origen,
					destino);
			// estadoActual
			// =itfUsoIniciador.getItfControl().getEstadoControlAgenteReactivo();
			// recursoTrazas.trazar(identPropietario,
			// "Antes de enviar el evento Estoy en el estado : "+ estadoActual,
			// NivelTraza.debug);
			...
			// =itfUsoIniciador.getItfControl().getEstadoControlAgenteReactivo();
			// recursoTrazas.trazar(identPropietario,
			// "Despues de enviar el evento Estoy en el estado : "+
			// estadoActual, NivelTraza.debug);
			...
			recursoTrazas.trazar(
					NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION, msgUsuario,
					NivelTraza.error);
