...
public class AccionesSemanticasGestorNodo extends
	...
	public AccionesSemanticasGestorNodo() {
		try {
			...
			// ItfGestionRecTrazas = (InterfazGestion)
			// this.itfUsoRepositorio.obtenerInterfaz(
			...
			config = (ItfUsoConfiguracion) this.itfUsoRepositorio
			...
			itfUsoPropiadeEsteAgente = (ItfUsoAgenteReactivo) itfUsoRepositorio
					.obtenerInterfazUso(NombresPredefinidos.ITF_USO
							+ nombreAgente);
		} catch (Exception ex) {
	...
	public void comenzar() {
		try { /* Saco mi configuracion */
			...
			Logger.getLogger(AccionesSemanticasGestorNodo.class.getName()).log(
		...
		if (!AdaptadorRegRMI.addElement2LocalRegRMI("GestorNodo",
				itfUsoPropiadeEsteAgente)) {
			// trazarDebug ("Estado RMI en puerto " + puertoRMI + " ... " +
			// listo);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Arranco el RMI en el puerto:" + puertoRMI,
					InfoTraza.NivelTraza.debug));
			// boolean exportado = ControlRMI.export("GestorNodo",
			// itfUsoPropiadeEsteAgente);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Se añade la interfaz del gestor de nodo :" + nombreAgente
							+ " al RMI registry", InfoTraza.NivelTraza.debug));
		} else {
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"No he podido añadir mi interfaz al registo RMI"
							+ puertoRMI, InfoTraza.NivelTraza.error));
		}
	...
	public void configurarGestor() {
		try {
			 ...
			 */
			tiempoParaNuevaMonitorizacion = Integer
					.parseInt(config
							.getValorPropiedadGlobal(NombresPredefinidos.INTERVALO_MONITORIZACION_ATR_PROPERTY));
			nombreAgente = this.getNombreAgente();
			config.getDescInstanciaGestor(nombreAgente);
			esteNodo = InetAddress.getLocalHost().getHostName();
			this.comunicator = this.getComunicator();

			/* Arranco RMI */
			AdaptadorRegRMI.inicializar();
			if (AdaptadorRegRMI.addElement2LocalRegRMI(nombreAgente,
					itfUsoPropiadeEsteAgente)) {
				// trazarDebug ("Estado RMI en puerto " + puertoRMI + " ... " +
				// listo);
				...
				// itfUsoPropiadeEsteAgente);
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
						"Se añade la interfaz del gestor de nodo :"
								+ nombreAgente + " al RMI registry",
						InfoTraza.NivelTraza.debug));
	...
	public void crearAgenteAplicacion(String identAgente) {
		...
		try {
			DescInstanciaAgenteAplicacion descAgente = config
					.getDescInstanciaAgenteAplicacion(identAgente);
			String tipoAgente = descAgente.getDescComportamiento().getTipo()
					.value();
			if (tipoAgente.equals(NombresPredefinidos.TIPO_REACTIVO)) {
				FactoriaAgenteReactivo.instancia().crearAgenteReactivo(
						descAgente);
			} else if (tipoAgente.equals(NombresPredefinidos.TIPO_COGNITIVO)) {
				FactoriaAgenteCognitivo.instance().crearAgenteCognitivo(
						descAgente);
			} else {

			}
		} catch (Exception e) {
			logger.error("GestorNodo : Fue imposible crear el agente : "
					+ identAgente, e);
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Fue imposible crear el agente : " + identAgente,
					InfoTraza.NivelTraza.error));
			trazas.trazar(nombreAgente, "Fue imposible crear el agente : "
	...
	public void crearGestor(String identGestor) {
		...
		try {
				...
				while (!ok) {
					...
					try {
						...
						logger.error("Error al crear el agente "
								...
								+ intentos + " segundos...\n nodo origen: "
								+ esteNodo + "\t nodo destino: " + nodoDestino);
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, identGestor
					+ " Gestor  creado.", InfoTraza.NivelTraza.debug));

			Set<Object> conjuntoEventos = new HashSet<Object>();
			conjuntoEventos.add(EventoRecAgte.class);

			// indico a quien debe reportar. Por defecto se pone el gestor de
			// nodo
			...
			((ItfGestionAgenteReactivo) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ identGestor)).setGestorAReportar(nombreAgente);
			...
			logger.error(
					"GestorOrganizaci�n: Fue imposible crear los gestores de agentes y recursos en el gestor de la organizaci�n",
					e);
			trazas.aceptaNuevaTraza(new InfoTraza(
					"GestorOrganizacion",
	...
	public void crearGestores() {
		try {
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Creando gestor de agentes ...", InfoTraza.NivelTraza.debug));

			// Gestor de Agentes: local o remoto?
			DescInstanciaGestor descGestor = config
					.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION);
			String esteNodo = descGestor.getNodo().getNombreUso();

			DescInstanciaGestor gestorAgentes = config
					.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Gestor de agentes creado.", InfoTraza.NivelTraza.debug));
			...
			DescInstanciaAgente gestorRecursos = config
					.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_RECURSOS);
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
					"Gestor de recursos creado.", InfoTraza.NivelTraza.debug));
			...
			// EventoRecAgte("gestores_creados",
			// NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION,
			// NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION));

		} catch (Exception e) {
			logger.error(
					"GestorOrganizacion: Fue imposible crear los gestores de agentes y recursos en el gestor de la organizaci�n",
					e);
			trazas.aceptaNuevaTraza(new InfoTraza(
					"GestorOrganizacion",
					"Fue imposible crear los gestores de agentes y recursos en el gestor de la organizaci�n",
	...
	public void arrancarGestorAgentes() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
				"Arrancando Gestor de Agentes.", InfoTraza.NivelTraza.debug));
		try {
	...
	public void arrancarGestorRecursos() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
				"Arrancando Gestor de Recursos.", InfoTraza.NivelTraza.debug));
		try {
			((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ NombresPredefinidos.NOMBRE_GESTOR_RECURSOS))
	...
	public void decidirTratamientoErrorIrrecuperable() {
		...
		/*
		 * try { this.itfUsoAgente.aceptaEvento(new EventoRecAgte(
		 * "tratamiento_terminar_gestores_y_gestor_organizacion",
		 * NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION,
		 * NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION)); } catch (Exception
		 * e) { e.printStackTrace(); }
		 */
	...
	public void monitorizarGestores() {
		...
		// if(DEBUG) System.out.println("GestorOrganizaci�n: Iniciando ciclo
		// de
		// monitorizaci�n");
		...
		try {
			...
			if (errorAlMonitorizar) {
				...
				if ((monitAgentes == InterfazGestion.ESTADO_ERRONEO_IRRECUPERABLE)
						|| (monitAgentes == InterfazGestion.ESTADO_ERRONEO_RECUPERABLE)) {
					logger.error("GestorOrganizaci�n: El GESTOR DE AGENTES ha fallado. Su estado es "
							+ monitAgentes);
					trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
							"El GESTOR DE AGENTES ha fallado. Su estado es "
									+ monitAgentes, InfoTraza.NivelTraza.error));
				} else {
					logger.error("GestorOrganizaci�n: El GESTOR DE RECURSOS ha fallado. Su estado es "
							+ monitRecursos);
					trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
							"El GESTOR DE RECURSOS ha fallado. Su estado es "
									+ monitRecursos, InfoTraza.NivelTraza.error));
	...
	public void terminarGestorAgentes() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
				"Terminando gestor de agentes", InfoTraza.NivelTraza.debug));
	...
	public void terminarGestorRecursos() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza("GestorOrganizacion",
				"Terminando gestor de recursos", InfoTraza.NivelTraza.debug));
	...
	private void crearUnAgenteReactivo(String identAgenteAcrear)
			throws Exception {
		try {
			...
			logger.debug("GestorNodo: Construyendo agente reactivo "
					+ nombreAgente + ".");
			...
			// DescInstanciaGestor descGestorAgentes =
			// config.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
			// String esteNodo = descGestorAgentes.getNodo().getNombreUso();

			DescInstanciaAgenteAplicacion instancia = config
					.getDescInstanciaAgenteAplicacion(identAgenteAcrear);
			...
			if (nodoDestino.equalsIgnoreCase(esteNodo)) {
				...
				// ++intentos;
				// try {
				// ((FactoriaAgenteReactivo)
				// this.itfUsoRepositorio.obtenerInterfaz(
				// NombresPredefinidos.FACTORIA_AGENTE_REACTIVO +
				// nodoDestino)).crearAgenteReactivo(instancia);
				// ok = true;
				// } catch (Exception e) {

				trazas.aceptaNuevaTraza(new InfoTraza("GestorNodo",
						"El nodo especificado:" + nodoDestino
								+ " para el agente " + identAgenteAcrear
								+ " no se corresponde con este nodo  "
								+ esteNodo, InfoTraza.NivelTraza.error));

				logger.error("El nodo especificado:" + nodoDestino
						+ " para el agente " + identAgenteAcrear
						+ " no se corresponde con este nodo  " + esteNodo);
			...
			// ((ItfGestionAgenteReactivo)
			// this.itfUsoRepositorio.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
			// + nombreAgente)).setGestorAReportar(
			// NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
	...
	private void crearUnAgenteCognitivo(String nombreAgente) throws Exception {
		try {
			...
			logger.debug("GestorAgentes: Construyendo agente Cognitivo "
					+ nombreAgente + ".");
			...
			// Agentes de aplicacion: local o remoto?
			DescInstanciaGestor descGestorAgentes = config
					.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_AGENTES);
			String esteNodo = descGestorAgentes.getNodo().getNombreUso();

			DescInstanciaAgenteAplicacion instancia = config
					.getDescInstanciaAgenteAplicacion(nombreAgente);
			String nodoDestino = instancia.getNodo().getNombreUso();
			...
			if (nodoDestino.equals(esteNodo)) {
				...
				while (!ok) {
					++intentos;
					try {
						((FactoriaAgenteCognitivo) this.itfUsoRepositorio
								.obtenerInterfaz(NombresPredefinidos.FACTORIA_AGENTE_COGNITIVO
										+ nodoDestino))
								.crearAgenteCognitivo(instancia);
						ok = true;
					} catch (Exception e) {
	...
	public void crearUnRecursoEnNodoLocal(String identInstRecurso)
			throws Exception {
		try {

			// ItfUsoConfiguracion config = (ItfUsoConfiguracion)
			// ClaseGeneradoraRepositorioInterfaces
			// .instance().obtenerInterfaz(
			// NombresPredefinidos.ITF_USO
			// + NombresPredefinidos.CONFIGURACION);
			if (config == null) {
			...
			DescInstanciaRecursoAplicacion descRecurso = config
					.getDescInstanciaRecursoAplicacion(identInstRecurso);
			...
			logger.debug("GestorRecursos: Recurso " + descRecurso.getId()
					+ " creado.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Recurso "
					+ descRecurso.getId() + " creado.",
					InfoTraza.NivelTraza.debug));
			...
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					...
					InfoTraza.NivelTraza.error));
			logger.error("GestorRecursos: Error al crear el recurso "
					+ identInstRecurso + ".", ex);
	...
	public void crearUnRecursoEnNodoLocalyPublicIntfEnRMIO(
			String identInstRecurso) throws Exception {

		// se crea un recurso local y se publican sus interfaces de uso y de
		// gestión en el rep RMI
		try {
			...
			if (!addEntityIntfs2Local_RMIregistry(identInstRecurso)) {
				...
				trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
						"Error al crear el recurso. No se puede añadir al RMI registry local "
								+ identInstRecurso + ".",
						InfoTraza.NivelTraza.error));
				logger.error("GestorNodo: Error al crear el recurso. No se puede añadir al RMI registry local  "
						+ identInstRecurso + ".");
			} else {

				// Se informa al Gestor de Recursos de la organizacion de la
				// creacion y registro del recurso
				// config.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_RECURSOS).getNodo().getNombreUso();
				ItfUsoAgenteReactivo itfUsoAgteReceptor = (ItfUsoAgenteReactivo) AdaptadorRegRMI
						.getItfAgenteRemoto(
								NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
								NombresPredefinidos.ITF_USO);
				if (itfUsoAgteReceptor != null) {
					comunicator.informaraOtroAgenteReactivo(
							new InfoContEvtMsgAgteReactivo(
									"RecursoRemotoCreadoyRegistrado"),
							NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
							itfUsoAgteReceptor);
				} else {
					trazas.aceptaNuevaTraza(new InfoTraza(
							nombreAgente,
							"No he podido obtener la interfaz   del agente: "
									+ NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
							InfoTraza.NivelTraza.error));
				}
			}
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Error al crear el recurso " + identInstRecurso + ".",
					InfoTraza.NivelTraza.error));
			logger.error("GestorNodo: Error al crear el recurso "
					+ identInstRecurso + ".", ex);
			this.informaraMiAutomata("error_en_creacion_recurso");
			// this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
			// "error_en_creacion_recurso",
	...
	public void crearUnAgenteEnNodoLocalyPublicIntfEnRMIO(
			String identAgenteAcrear) throws Exception {
		...
		try {
			...
			DescInstanciaAgenteAplicacion instanciaActual = config
					.getDescInstanciaAgenteAplicacion(identAgenteAcrear);
			...
			TipoAgente tipoAgenteAcrear = instanciaActual
					.getDescComportamiento().getTipo();
			if (!nodoDestino.equalsIgnoreCase(esteNodo)) {
				...
				if (tipoAgenteAcrear != TipoAgente.COGNITIVO) { // no es ni
					...
					trazas.aceptaNuevaTraza(new InfoTraza(
							...
							"El subtipo del agente "
									+ identAgenteAcrear
									+ " definido en la configuracion no es correcto.",
							InfoTraza.NivelTraza.error));
					logger.error("El subtipo del agente " + identAgenteAcrear
							+ " definido en la configuracion no es correcto.");

					throw new Exception("El subtipo del agente "
							+ identAgenteAcrear
							+ " definido en la configuracion no es correcto.");
				} else {
					FactoriaAgenteCognitivo.instance().crearAgenteCognitivo(
							instanciaActual);
					// Se genera informacion para que transite el automata y se
					// registran las interfaces
				}

				if (!addEntityIntfs2Local_RMIregistry(identAgenteAcrear)) {

					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Error al crear el agente remoto. No se puede añadir al RMI registry local "
									+ identAgenteAcrear + ".",
							InfoTraza.NivelTraza.error));
					logger.error("GestorNodo: Error al crear el agente remoto. No se puede añadir al RMI registry local  "
							+ identAgenteAcrear + ".");
				} else {

					// Se informa al Gestor de Agentes de la organizacion de la
					// creacion y registro del recurso
					// config.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_RECURSOS).getNodo().getNombreUso();
					ItfUsoAgenteReactivo itfUsoAgteReceptor = (ItfUsoAgenteReactivo) AdaptadorRegRMI
							.getItfAgenteRemoto(
					...
					if (itfUsoAgteReceptor != null) {
						...
						comunicator
								.informaraOtroAgenteReactivo(
										new InfoContEvtMsgAgteReactivo(
												"agenteRemotoCreadoyRegistrado",
												param),
										NombresPredefinidos.NOMBRE_GESTOR_AGENTES,
										itfUsoAgteReceptor);
					} else {
					...
					logger.debug("GestorNodo: Recurso " + identAgenteAcrear
							+ " creado.");
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Agente " + identAgenteAcrear + " creado.",
							InfoTraza.NivelTraza.debug));
				}
			}
		} catch (Exception ex) {
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					...
					InfoTraza.NivelTraza.error));
			logger.error("GestorNodo: Error al crear el agente "
					+ identAgenteAcrear + ".", ex);
	...
	public Boolean addEntityIntfs2RMI_ORGregistry(String identEntity) {
		...
		try {
			Remote itfEntity = (Remote) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_USO + identEntity);
			if (itfEntity != null) {
				if (AdaptadorRegRMI.addElementRegRMIOrg(
						NombresPredefinidos.ITF_USO + identEntity, itfEntity)) {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Se añaden las Itfs de uso y de gestion de la entidad :"
									+ identEntity
									+ " -- al RMI registry de la organizacion",
							InfoTraza.NivelTraza.debug));
				} else {
					trazas.aceptaNuevaTraza(new InfoTraza(
							nombreAgente,
							"No he podido añadir la entidad :" + identEntity
									+ " -- al RMI registry de la organizacion.",
							InfoTraza.NivelTraza.error));
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						nombreAgente,
						"No he podido añadir la entidad :"
								+ identEntity
								+ " -- al RMI registry de la organizacion. La entidad no ha sido registrada en el registro local",
						InfoTraza.NivelTraza.error));
				// informaraMiAutomata("error_en_registroRemoto_recurso");
				return interfacesRegistradas;
			}
			itfEntity = (Remote) itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ identEntity);
			if (itfEntity != null) {
				if (AdaptadorRegRMI.addElement2LocalRegRMI(
						NombresPredefinidos.ITF_GESTION + identEntity,
						itfEntity)) {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"Se añaden las Itfs de uso y de gestion de la entidad :"
									+ identEntity
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					"Error al registrar  la entidad " + identEntity
							+ ". en el registro RMI de la organizacion",
					InfoTraza.NivelTraza.error));
			logger.error("GestorRecursos: Error al crear el recurso "
					+ identEntity + ".en el registro RMI de la organizacion",
					ex);
			informaraMiAutomata("error_en_registroRemoto_recurso");
			// throw ex;
		}
	...
	public Boolean addEntityIntfs2Local_RMIregistry(String identEntity) {
		...
		try {
			...
			if (itfEntity != null) {
				if (AdaptadorRegRMI.addElement2LocalRegRMI(
					...
					trazas.aceptaNuevaTraza(new InfoTraza(
							nombreAgente,
							"Se añaden las Itfs de uso y de gestion de la entidad :"
									+ identEntity + " -- al RMI registry local",
							InfoTraza.NivelTraza.debug));
				} else {
					trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
							"No he podido añadir la entidad :" + identEntity
									+ " -- al RMI registry local.",
							InfoTraza.NivelTraza.error));
					informaraMiAutomata("error_en_registroRemoto_recurso");
					return interfacesRegistradas;
				}
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						...
						"No he podido añadir la entidad :"
								+ identEntity
								+ " -- al RMI registry. La entidad no ha sido registrada en el registro local",
						InfoTraza.NivelTraza.error));
				informaraMiAutomata("error_en_registroRemoto_recurso");
				return interfacesRegistradas;
			}
			...
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
					...
					InfoTraza.NivelTraza.error));
			logger.error("GestorRecursos: Error al crear el recurso "
					+ identEntity + ". En el registro RMI local", ex);
			informaraMiAutomata("error_en_registroRemoto_recurso");
			// throw ex;
		}
		return interfacesRegistradas;
	...
	public void arrancarRecurso(String identRecurso) {
		...
		try {
			...
			logger.debug("GestorRecursos: Es necesario arrancar el recurso "
					+ identRecurso + ", recuperando interfaz de gesti�n.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Es necesario arrancar el recurso " + identRecurso
							+ ", recuperando interfaz de gesti�n.",
					InfoTraza.NivelTraza.debug));
			InterfazGestion itfGesAg = (InterfazGestion) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ identRecurso);
			// arrancamos el recurso
			logger.debug("GestorRecursos: Arrancando el recurso "
					+ identRecurso + ".");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
			...
			if (itfGesAg != null) {
				...
				logger.debug("GestorRecursos: No se puede dar la orden de arranque al recurso "
						+ identRecurso + ". Porque su interfaz es null");
				trazas.aceptaNuevaTraza(new InfoTraza(
						nombreAgente,
			...
			logger.error("GestorRecursos: Hubo un problema al acceder al interfaz remoto mientras se arrancaba el recurso "
					+ identRecurso + " en el gestor de recursos.");
			trazas.aceptaNuevaTraza(new InfoTraza(
					"GestorRecursos",
	...
	public void procesarPeticionTerminacion() {
		...
		/*
		 * try { // this.itfUsoAgente.aceptaEvento(new //
		 * EventoRecAgte("termina",null,null));
		 * 
		 * 
		 * ItfGestionAgenteReactivo gestion = (ItfGestionAgenteReactivo)
	...
	public void comenzarTerminacionConfirmada() {
		...
		try {
			this.ctrlGlobalAgenteReactivo.getItfControl()
					.getEstadoControlAgenteReactivo();
			this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ this.nombreAgente);
			// gestion.termina();
			...
			this.ctrlGlobalAgenteReactivo.getItfControl()
					.procesarInfoControlAgteReactivo(
							new InfoContEvtMsgAgteReactivo("termina"));
	...
	/**
	 * Da orden de terminaci�n al gestor de agentes y/o al gestor de recursos
	 * si est�n activos.
	 */
	/*
	 ...
	 * gestores"); try {
	 * ((InterfazGestion)this.itfUsoRepositorio.obtenerInterfaz
	 * (NombresInterfaces.ITF_GES_GESTOR_AGENTES)).termina(); } catch (Exception
	 * ex) { logger.debug("GestorOrganizaci�n: No se pudo acceder al gestor de
	 * agentes."); ex.printStackTrace(); } try {
	 * ((InterfazGestion)this.itfUsoRepositorio
	 * .obtenerInterfaz(NombresInterfaces.ITF_GES_GESTOR_RECURSOS)).termina(); }
	...
	public void terminarGestorOrganizacion() {
		...
		try {
			...
			((ItfGestionAgenteReactivo) this.itfUsoRepositorio
					.obtenerInterfaz(NombresPredefinidos.ITF_GESTION
							+ this.nombreAgente)).termina();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		logger.debug("GestorOrganizacion: Cerrando sistema.");
		trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente,
				"Cerrando sistema.", InfoTraza.NivelTraza.debug));
