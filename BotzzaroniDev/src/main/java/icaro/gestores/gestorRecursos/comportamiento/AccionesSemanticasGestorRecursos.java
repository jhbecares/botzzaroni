...
/**
 * Implementaci�n por defecto de las acciones que se ejecutar�n por parte
...
public class AccionesSemanticasGestorRecursos extends
	...
	public void configurarGestor() {
		try {
			config = (ItfUsoConfiguracion) NombresPredefinidos.REPOSITORIO_INTERFACES_OBJ
	...
	public void listarRecursos() {
		try {
			...
			List<DescInstancia> lista = config.getDescInstanciaGestor(
					NombresPredefinidos.NOMBRE_GESTOR_RECURSOS)
			...
			// EventoRecAgte("recursos_listados",
	 ...
	 * @param indice
	 */
	public void crearRecurso(ArrayList<DescInstanciaRecursoAplicacion> lista,
			Integer indice) {
		Iterator<DescInstanciaRecursoAplicacion> iterador = lista.iterator();
		logger.debug("GestorRecursos: Creando recurso.");
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
				"Creando recurso.", InfoTraza.NivelTraza.debug));
		int j = 0;
		String nombre;
		boolean error = false;
		...
		while (iterador.hasNext() && (encontrado == false)) {
			...
			if (j == indice.intValue()) {// es el que debemos arrancar
				...
				try {
					...
					logger.debug("GestorRecursos: Añadiendo recurso " + nombre
							+ " a la lista de objetos gestionados.");
					trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
							"Añadiendo recurso " + nombre
									+ " a la lista de objetos gestionados.",
							InfoTraza.NivelTraza.debug));
					this.nombresRecursosGestionados.add(nombre);
				} catch (Exception ex) {
					logger.error("GestorRecursos: Hubo problemas al crear el recurso "
							+ nombre
							+ " desde la configuracion. Recuperando errores de creaci�n.");
					trazas.aceptaNuevaTraza(new InfoTraza(
							"GestorRecursos",
							"Hubo problemas al crear el recurso "
									+ nombre
									+ " desde la configuracion. Recuperando errores de creaci�n.",
							InfoTraza.NivelTraza.error));
					ex.printStackTrace();
		...
		try {
			...
			if (error == false && encontrado == true) {
				...
				this.comunicator.enviarInfoAotroAgente("recursos_creados_ok",
			...
			// NombresPredefinidos.NOMBRE_GESTOR_RECURSOS));
				...
				this.informaraMiAutomata("error_en_creacion_recurso", recurso,
						indice);
				// this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
	 ...
	 * @param recurso
	 */
	private void crearUnRecurso(DescInstanciaRecursoAplicacion recurso)
			throws Exception {
		try {
			...
			logger.debug("GestorRecursos: Construyendo recurso "
					+ recurso.getId() + ".");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
			...
			// DescInstanciaGestor descGestorRecursos =
			// config.getDescInstanciaGestor(NombresPredefinidos.NOMBRE_GESTOR_RECURSOS);
			...
			if (nodoDestino.equals(esteNodo)) {
				...
				// Si la organizacion esta desplegada en varios nodos entonces
				// hay que registrarlo en el RMI local
			...
			logger.debug("GestorRecursos: Recurso " + recurso.getId()
					+ " creado.");
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos", "Recurso "
					+ recurso.getId() + " creado.", InfoTraza.NivelTraza.debug));

		} catch (Exception ex) {
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Error al crear el recurso " + recurso.getId() + ".",
					InfoTraza.NivelTraza.error));
			logger.error("GestorRecursos: Error al crear el recurso " + recurso
					+ ".", ex);
			throw ex;
		}
	}

	private void crearUnRecursoEnNodoRemotoConRMI(
			DescInstanciaRecursoAplicacion recurso) throws Exception {
		...
		if (!misInterfacesEstanEnElRegistroRMILocal) { // Las añado
			...
			if (!AdaptadorRegRMI.addElement2LocalRegRMI(
					...
					this.itfUsoPropiadeEsteAgente)) { // No se ha podido añadir
														// la interfaz de uso al
														// registro local
				trazas.aceptaNuevaTraza(new InfoTraza(
						"GestorRecursos",
		...
		try {
			...
			while (!ok & (intentos <= maxNumIntentosCreacionCompGestionados)) {
				...
				try {
					...
					// if ( !
					// AdaptadorRegRMI.addElement2LocalRegRMI(NombresPredefinidos.ITF_USO+nombreAgente,
					// this.itfUsoPropiadeEsteAgente))

					// Buscamos el gestor de nodo del recurso que vamos a crear
					ItfUsoAgenteReactivo itfUsoGestorNodo = (ItfUsoAgenteReactivo) AdaptadorRegRMI
							.getItfAgteReactRemoto(identNodoRecurso,
									NombresPredefinidos.NOMBRE_GESTOR_NODO);
					// ItfUsoAgenteReactivo itfUsoGestorNodo =
					// (ItfUsoAgenteReactivo)
					// AdaptadorRegRMI.getItfAgenteRemoto(NombresPredefinidos.NOMBRE_GESTOR_NODO,
					// NombresPredefinidos.ITF_USO);
					if (itfUsoGestorNodo != null) {
						...
						itfUsoGestorNodo.aceptaMensaje(new MensajeSimple(
								new InfoContEvtMsgAgteReactivo(
										"peticion_crearRecurso",
										(Object) recurso.getId()),
								this.nombreAgente,
								NombresPredefinidos.NOMBRE_GESTOR_NODO));
						ok = true;
			...
			logger.debug("GestorRecursos : Recurso :" + identRecurso
					+ "creado.");
			trazas.aceptaNuevaTraza(new InfoTraza(nombreAgente, "Recurso :"
					+ identRecurso + "creado.", InfoTraza.NivelTraza.debug));
		} catch (Exception e) {
			logger.error(
					"GestorRecursos: Error al crear el recurso "
							+ recurso.getId() + "en el nodo remoto "
							+ identNodoRecurso, e);
			trazas.aceptaNuevaTraza(new InfoTraza(identRecurso,
					"Error al crear el recurso " + identRecurso
							+ "en el nodo remoto " + identNodoRecurso,
					InfoTraza.NivelTraza.error));
			e.printStackTrace();
			try {
	 ...
	 */
	public void recuperarErrorCreacionRecurso(
			List<DescInstanciaRecursoAplicacion> lista,
			DescInstanciaRecursoAplicacion recurso, Integer indice) {
		String idRecurso = recurso.getId();
		Object[] parametros = new Object[] { lista, recurso,
				maxNumIntentosCreacionCompGestionados, indice };
		// por defecto no se implementan politicas de recuperacion
		logger.debug("GestorRecursos: Comenzando los reintentos de creacion para el recurso "
				+ idRecurso + ".");
		trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
				"Comenzando los reintentos de creacion para el recurso "
						+ idRecurso + ".", InfoTraza.NivelTraza.debug));
		try {
			this.informaraMiAutomata("reintenta");
			// this.itfUsoPropiadeEsteAgente.aceptaEvento(new
			// EventoRecAgte("reintenta",
			// parametros, NombresPredefinidos.NOMBRE_GESTOR_RECURSOS,
			// NombresPredefinidos.NOMBRE_GESTOR_RECURSOS));
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	public void reintentarCreacionRecurso(
			List<DescInstanciaRecursoAplicacion> lista,
			DescInstanciaRecursoAplicacion recurso, Integer reintento,
			Integer indice) {
		...
		if (!fin_reintentos) { // se reintenta la creacion
			logger.debug("GestorRecursos: Reintento de creacion " + reintento
					+ " del recurso " + idRecurso);
			trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
					"Reintento de creacion " + reintento + " del recurso "
							+ idRecurso, InfoTraza.NivelTraza.debug));
			try {
				...
				logger.debug("GestorRecursos: Añadiendo recurso " + idRecurso
						+ " a la lista de objetos gestionados.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Añadiendo recurso " + idRecurso
								+ " a la lista de objetos gestionados.",
						InfoTraza.NivelTraza.debug));
				...
				logger.error("GestorRecursos: Hubo problemas al crear el recurso "
						+ idRecurso
						+ " desde la configuraci�n. Recuperando errores de creaci�n.");
				trazas.aceptaNuevaTraza(new InfoTraza(
						"GestorRecursos",
						"Hubo problemas al crear el recurso "
								+ idRecurso
		...
		try {
			...
			if (fin_reintentos) { // ya hay que decidir que se va a hacer una
									...
									// recurso
				logger.debug("GestorRecursos: Agotados los intentos de crear el recurso "
						+ idRecurso + ". Continuando con la creaci�n.");
				trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
						"Agotados los intentos de crear el recurso "
								+ idRecurso
								+ ". Continuando con la creaci�n.",
						InfoTraza.NivelTraza.debug));
				...
				parametros = new Object[] { lista, recurso,
						new Integer((reintento.intValue() - 1)), indice };
				this.informaraMiAutomata("reintento_error", lista, recurso,
						reintento - 1);
				// this.itfUsoPropiadeEsteAgente.aceptaEvento(new EventoRecAgte(
	...
	/**
	 * Monitoriza secuencialmente todos los recursos activos que est�n
	 * definidos como necesarios en la monitorizaci�n.
	 */
	public void monitorizarRecursos() {
		System.out.println("GestorRecursos:Comienza ciclo monitorizaci�n.");

		boolean errorEncontrado = false;
		// recuperar todos los interfaces de gesti�n del repositorio que
		// estamos
		// gestionando
		...
		while (enume.hasMoreElements() && !errorEncontrado) {
			...
			try {
				...
				if (monitoriz == 0) {
					...
					logger.debug("GestorRecursos:Recurso " + nombre
							+ " esta en estado err�neo o terminado.");
					trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
							"Recurso " + nombre
	...
	public void terminarRecursosActivos() {
		...
		while (enumRecursos.hasMoreElements()) {
			try {
				...
				// s�lo se terminan los recursos que se arranquen desde
				// aqu�.

				// para cada recurso, recuperamos su itf de gestion
				logger.debug("GestorRecursos: Recuperando Itf Gesti�n del recurso "
						+ nombre + ".");
				trazas.aceptaNuevaTraza(new InfoTraza(
						"GestorRecursos",
				...
				if (itfGesRec != null) {
					itfGesRec.termina();
					logger.debug("GestorRecursos: Orden de terminacion ha sido dada al recurso "
							+ nombre + ".");
					trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
							"Orden de terminacion ha sido dada al recurso "
									+ nombre + ".", InfoTraza.NivelTraza.debug));
				}
			} catch (Exception ex) {
				logger.error("GestorRecursos: Hubo un problema al acceder a un interfaz remoto mientras se daba orden de terminaci�n al recurso "
						+ nombre + ".");
				trazas.aceptaNuevaTraza(new InfoTraza(
						"GestorRecursos",
						"Hubo un problema al acceder a un interfaz remoto mientras se daba orden de terminaci�n al recurso "
	...
	public void terminarGestorRecursos() {
		...
		try {
			if (this.hebra != null)
				this.hebra.finalizar(); // CUIDADO, SI FALLASE LA CREACION DE
										// LOS
										// RECURSOS ESTA HEBRA
