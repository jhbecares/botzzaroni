 ...
 */
public class AgenteCognitivotImp2 extends AgenteCognitivo implements
		Serializable {
	 ...
	 * @throws Exception
	 */
	public AgenteCognitivotImp2(String nombreAgenteNormalizado)
			throws Exception {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(identAgente, "Estado: "
				+ estadoAgente, InfoTraza.NivelTraza.debug));
	}

	@Override
	public void setComponentesInternos(
			ItfUsoAutomataEFsinAcciones itfAutomataCiclVidaAgente,
			PercepcionAgenteCognitivo percepcion,
			ProcesadorObjetivos procObjetivos) {
		...
		if ((estadoAgente.equals(NombresPredefinidos.ESTADO_ACTIVO))) {
			try {
				...
				java.util.logging.Logger.getLogger(
						AgenteCognitivotImp2.class.getName()).log(Level.SEVERE,
						null, ex);
			}
		} else {
	...
	public void aceptaMensaje(MensajeSimple mensaje) {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(identAgente, "Estado: "
				+ estadoAgente + " recibo el mensaje con contenido:"
				+ mensaje.getContenido().toString(), InfoTraza.NivelTraza.debug));
	}
	...
	public void arranca() {
		...
		trazas.aceptaNuevaTraza(new InfoTraza(identAgente, ": arranca()",
				InfoTraza.NivelTraza.debug));
