...
public class GeneracionInputTimeout extends Thread {
	...
	public GeneracionInputTimeout(long milis,
			ItfUsoAgenteReactivo itfusoAfteRecptor, Informe informeAGenerar) {
		...
		try {
			...
			if (trazar) {
				if (trazas == null) {
					...
					// try{
					// trazas =
					// (ItfUsoRecursoTrazas)ClaseGeneradoraRepositorioInterfaces.instance().obtenerInterfaz(
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						"InputTimeout"
								+ informeAGenerar.getidentEntidadEmisora(),
						"........................   InformeTimeout (objetivo->"
								+ informeAGenerar.getReferenciaContexto()
								+ " , conteido ->"
								+ informeAGenerar.getContenidoInforme()
								+ ")..... antes de hacer el sleep.....Hora->"
								+ hora
								+ " , Minuto->"
								+ minutos
								+ " , segundos->"
								+ segundos
								+ " ... milisegundos desde 1 de enero de 1970 UTC->"
								+ lctm1, InfoTraza.NivelTraza.debug));
				// }
				// catch (Exception ex) {}
			}
			//
			...
			if (trazar) {
				...
				trazas.aceptaNuevaTraza(new InfoTraza(
						"InputTimeout"
								+ informeAGenerar.getidentEntidadEmisora(),
						"........................   InformeTimeout  (ref Contexto->"
								+ informeAGenerar.getReferenciaContexto()
								+ " , contenido->"
								+ informeAGenerar.getContenidoInforme()
								+ ")..... milisegundos despues de hacer el sleep ->"
								+ this.milis
								+ " milisegundos"
								+ ".....Hora->"
								+ hora
								...
								+ segundos
								+ " ... milisegundos desde 1 de enero de 1970 UTC->"
								+ lctm2 + " . DIFERENCIA->" + lctm12,
						InfoTraza.NivelTraza.debug));
				try {
					// // }
					// // catch (Exception ex) {}
					this.itfUsoAgenteReceptor.aceptaEvento(new EventoRecAgte(
							"inputTimeout", informeAGenerar, this.getClass()
									.getSimpleName(), null));
				} catch (RemoteException ex) {
					Logger.getLogger(GeneracionInputTimeout.class.getName())
							.log(Level.SEVERE, null, ex);
				}
			}
			// EventoRecAgte ev = new
			// EventoRecAgte("inputTimeout",informeAGenerar,this.getClass().getSimpleName(),null);
