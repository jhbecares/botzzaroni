...
public abstract class TareaSincrona {
	...
	public TareaSincrona(ItfProcesadorObjetivos envioHechos,
	...
	public void generarInformeConCausaTerminacion(String idTarea,
			Objetivo contxtGoal, String idAgenteOrdenante, Object contenido,
			CausaTerminacionTarea causaTerminacion) {

		String identGoal = null;
		if (contxtGoal != null) {
			identGoal = contxtGoal.getgoalId();
		}
		InformeDeTarea resultadoTarea = new InformeDeTarea(idTarea, identGoal,
				idAgenteOrdenante, contenido, causaTerminacion);
	...
	public void generarInformeOK(String idTarea, Objetivo contxtGoal,
			String idAgenteOrdenante, Object contenido) {
		String goalId = null;
		if (contxtGoal != null) {
			goalId = contxtGoal.getgoalId();
		}
		InformeDeTarea resultadoTarea = new InformeDeTarea(idTarea, goalId,
				idAgenteOrdenante, contenido);
		itfProcObjetivos.insertarHecho(resultadoTarea);
		// envioHechos.insertarHecho(contenido);
	}

	public void generarInformeTemporizado(long milis, String idTarea,
			Objetivo contxtGoal, String idAgenteOrdenante, Object contenido) {
		String goalId = null;
		if (contxtGoal != null) {
			goalId = contxtGoal.getgoalId();
		...
		if (contenido == null) {
			contenido = NombresPredefinidos.PREFIJO_MSG_TIMEOUT;
		}
		InformeDeTarea informeTarea = new InformeDeTarea(idTarea, goalId,
				idAgenteOrdenante, contenido);
		InformeTimeout informeTemporizado = new InformeTimeout(milis,
				itfProcObjetivos, informeTarea);
	...
	public void generarInformeTemporizadoFromConfigProperty(
		...
		try {
			...
			this.generarInformeTemporizado(valorTimeout, identproperty,
					contxtGoal, idAgenteOrdenante, contenido);
			trazas.trazar(idAgenteOrdenante,
					"Se ejecuta la tarea " + this.getIdentTarea()
							+ " Se activa un informe temporizado :  "
							+ contenido, NivelTraza.debug);
			...
			trazas.trazar(
					...
					"Se ejecuta la tarea "
							...
							+ " Verifique el nombre de la propiedad en la descripcion de la organizacion :  "
							+ identproperty, NivelTraza.error);
			Logger.getLogger(TareaSincrona.class.getName()).log(Level.SEVERE,
					null, ex);
	...
	public ItfUsoConfiguracion getItfUsoConfiguracion() {
		if (itfConfig == null) {
			try {
				...
				Logger.getLogger(TareaSincrona.class.getName()).log(
